	...
	class ModelSpimFirst extends Model{
		...
		function getAllLegalList($workorderid){
			...
			$sqlQuery = "SELECT wo_fcl_1st.fcl_id,wo_fcl_1st.fcl_passno,wo_fcl_1st.fcl_newpassno,wo_fcl_1st.fcl_name,mst_nationality.nat_desc,
						 ...
						 wo_fcl_1st.fcl_salary,mst_worktrade.trade_desc,wo_fcl_1st.fcl_wt,mst_wo_type.mst_wo_type_desc,wo_fcl_1st.fcl_plksdate1,wo_fcl_1st.fcl_plksdate2,wo_fcl_1st.fcl_entrydate
						 FROM wo_fcl_1st 
		...
		function getOneDate($workorderid){
					
			$sqlQuery = "SELECT wo_fcl_1st.fcl_entrydate
						 FROM wo_fcl_1st whERE wo_fcl_1st.fcl_woid = '$workorderid' LIMIT 1";	
			
			
			$dentry = $this->db->query($sqlQuery);
			return $dentry ->row();	
		...
		function getWOByWoidOrCompname($searchby, $keyword){
			$sqlQuery = "SELECT wo.wo_id, wo.wo_num, wo.wo_clab_no, wo.wo_fcl_total, ctr.ctr_comp_name, wo_status.wo_latest_progress 
						 ...
						 ON wo.wo_clab_no = ctr.ctr_clab_no
			   			 WHERE wo.wo_spim_status in ('V','T') AND $searchby LIKE '%$keyword%' and wo_latest_progress like '%close%'
