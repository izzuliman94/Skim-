<?php
	class ModelIDCard extends Model{
		...
		function getAllLabor($searchdata){
		    ...
		    $sqlquery = "SELECT wkr_passno,wkr_name,mst_countries.cty_desc,wkr_passexp,wkr_id,wkr_permitexp,wkr_uploaddoc.upload_destfilename
			            FROM workers 
		...
		function getIDList($datefrom,$dateto){
		   ...
		   $sql = "SELECT wkr_id,wkr_passno,wkr_name,mst_countries.cty_desc,wkr_passexp,wkr_permitexp,wkr_uploaddoc.upload_destfilename
			            FROM workers 
						left join mst_countries on mst_countries.cty_id = workers.wkr_country
						left join wkr_uploaddoc on wkr_uploaddoc.upload_wkrid = workers.wkr_id
		...
		function getIDList2($datefrom,$dateto){
		   ...
		   $sql = "SELECT fcl_passno,fcl_name,fcl_wkrid,fcl_photo,fcl_permitexp,fcl_country
						 FROM card_fcl 
		...
		function getAllLaborByID($passno){
		    ...
		    $sqlquery = "SELECT wkr_id,wkr_passno,wkr_name,mst_countries.cty_desc,wkr_passexp,wkr_id,wkr_permitexp
			            FROM workers 
						left join mst_countries on mst_countries.cty_id = workers.wkr_country
