<?php
   ...
   class ModelPaymentold extends model {
		function ModelPaymentold(){
			parent::Model();
			$this->load->database();
		}
		...
		function getsumpayment($type,$datefrom,$dateto){
		   ...
		   $sql = "select pmt_receipt.*,pmt_advise.pmt_no,pmt_type.pmt_type_desc,
				   ...
				   from pmt_receipt 
                   inner join pmt_advise on pmt_advise.pmt_no = pmt_receipt.clab_pa_no
                   inner join pmt_fcl on pmt_fcl.pmt_no = pmt_advise.pmt_no
				   inner join pmt_type on pmt_type.pmt_type_id = pmt_receipt.payment_type
		...
		function getsummarypayment($datefrom,$dateto,$region,$type,$cancel,$cont){
		   ...
		   $sqlQuery = "select pmt_receipt.*,pmt_type.pmt_type_desc,
		           sum(clab_amount + jim_amount + fomema_amount + ins_amount + agency_amount + others_amount) as TotalPaymentCollected
				   from pmt_receipt 
                   inner join pmt_type on pmt_type.pmt_type_id = pmt_receipt.payment_type 
			...
			$sqlQuery .= " GROUP BY pmt_receipt.pmt_receipt_no";
