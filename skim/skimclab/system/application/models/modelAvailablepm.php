<?php
	class ModelAvailablepm extends Model {
		function ModelAvailablepm(){
			parent::Model();
			$this->load->database();
		}	
		...
		function getAvailableDetails($ref_no){
			$query = "SELECT availablepm.*, mst_avstatus.avstatus_desc FROM availablepm";
			$query .= " LEFT JOIN mst_avstatus ON availablepm.avlab_statusid = mst_avstatus.avstatus_id";
		...
		function getAvlabwkr($ref_no){
			$query = "SELECT available_wkr.* FROM available_wkr
			WHERE avlabwkr_ref_no = '" . $ref_no . "'";
			$avlabwkr = $this->db->query($query);
			
			return $avlabwkr->row();
		}
		
		function getAvailableSummary($ref_no){
			$sqlQuery = "SELECT availablepm.avlab_ref_no, ctr1.ctr_clab_no AS comp_from_clabno, ctr1.ctr_comp_name AS comp_from, ctr2.ctr_clab_no AS comp_to_clabno, ctr2.ctr_comp_name AS comp_to from availablepm
						LEFT JOIN contractors ctr1 ON availablepm.avlab_comp_from = ctr1.ctr_clab_no
						LEFT JOIN contractors ctr2 ON availablepm.avlab_comp_to = ctr2.ctr_clab_no
