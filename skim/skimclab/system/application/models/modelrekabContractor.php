<?php
	class ModelrekabContractor extends Model {
		function ModelrekabContractor(){
			parent::Model();
			$this->load->database();	
		}
		...
		function companyExists($companyno){
			$query = "SELECT rekab_contractors.*, mst_ctr_appstatus.appstatus_desc, mst_emp_status.emp_status_desc FROM rekab_contractors";
			$query .= " LEFT JOIN mst_ctr_appstatus ON rekab_contractors.ctr_appstatus = mst_ctr_appstatus.appstatus_id";
			$query .= " LEFT JOIN mst_emp_status ON rekab_contractors.ctr_status = mst_emp_status.emp_statusid";
		...
		function getContractors($num, $offset){
			$this->db->select('*');
			$this->db->from('rekab_contractors');
			$this->db->join('mst_ctr_appstatus', 'rekab_contractors.ctr_appstatus=mst_ctr_appstatus.appstatus_id','LEFT');
			$this->db->join('mst_emp_status', 'rekab_contractors.ctr_status=mst_emp_status.emp_statusid','LEFT');
		...
		function getTotalContractors(){
			$sqlQuery = "SELECT COUNT(*) AS totalctr FROM rekab_contractors";
		...
		function getCtrByAppStatus($appstatus_id, $returnType){
			$query = "SELECT rekab_contractors.*, mst_ctr_appstatus.appstatus_desc FROM rekab_contractors
					 LEFT JOIN mst_ctr_appstatus ON mst_ctr_appstatus.appstatus_id = rekab_contractors.ctr_appstatus
					 WHERE ctr_status = '1'";
			if($appstatus_id > 0){
		...
		function getCtrForApproval($returnType){
			//only get active rekab_contractors
			$query = "SELECT rekab_contractors.*, mst_ctr_appstatus.appstatus_desc FROM rekab_contractors 
					  LEFT JOIN mst_ctr_appstatus ON mst_ctr_appstatus.appstatus_id = rekab_contractors.ctr_appstatus
		...
		function getCtrRegisterOnline($statusType, $statusID, $returnType){
			$ctrQuery = "SELECT rekab_contractors.*, mst_ctr_appstatus.appstatus_desc FROM rekab_contractors 
						 LEFT JOIN mst_ctr_appstatus ON mst_ctr_appstatus.appstatus_id = rekab_contractors.ctr_appstatus
		...
		function getCtrByClabNo($ctr_clab_no){
			$query = "SELECT rekab_contractors.*, mst_ctr_appstatus.appstatus_desc, mst_emp_status.emp_status_desc,  
			...
			mst_wkr_agent.agent_addr3 FROM rekab_contractors ";
			$query .= " LEFT JOIN mst_ctr_appstatus ON rekab_contractors.ctr_appstatus = mst_ctr_appstatus.appstatus_id";
			$query .= " LEFT JOIN mst_emp_status ON rekab_contractors.ctr_status = mst_emp_status.emp_statusid";
			$query .= " LEFT JOIN mst_states ON rekab_contractors.ctr_state = mst_states.state_id";
			$query .= " LEFT JOIN employees ON employees.emp_name = rekab_contractors.ctr_app_name";
			$query .= " LEFT JOIN ctr_printletter_history ON ctr_printletter_history.print_ctr_clabno = rekab_contractors.ctr_clab_no";
			$query .= " LEFT JOIN employees employees2 ON employees2.emp_name = rekab_contractors.ctr_rej_name";
			$query .= " LEFT JOIN wo_agency On wo_agency.agency_id = rekab_contractors.agent_id ";
			$query .= " LEFT JOIN mst_wkr_agent On mst_wkr_agent.agent_id = rekab_contractors.agent_id ";
		...
		function getCancelPermit($ctr_clab_no){
			$query = "SELECT rekab_workers.*, mst_countries.* ,rekab_contractors.*, mst_states.* FROM
			rekab_workers";
			$query .= " LEFT JOIN mst_countries ON rekab_workers.wkr_country = mst_countries.cty_id";
			$query .= " LEFT JOIN rekab_contractors ON rekab_contractors.ctr_clab_no = rekab_workers.wkr_currentemp";
			$query .= " LEFT JOIN mst_states ON rekab_contractors.ctr_state = mst_states.state_id";
			$query .= " WHERE rekab_workers.wkr_status = 2 AND rekab_workers.wkr_transtatus in (12,8) AND rekab_workers.wkr_permitexp BETWEEN '2018-04-01' AND '2018-05-31' and	rekab_contractors.ctr_clab_no = '" . $ctr_clab_no . "'";
			return $this->db->query($query);	
		...
		function searchContractors($searchby, $keyword,$num,$offset){
			$this->db->select('*');
    		$this->db->from('rekab_contractors');
    		$this->db->join('mst_ctr_appstatus', 'rekab_contractors.ctr_appstatus = mst_ctr_appstatus.appstatus_id');
    		$this->db->join('mst_emp_status', 'rekab_contractors.ctr_status = mst_emp_status.emp_statusid');
		...
		function generateClabNo(){
			$sqlQuery = "SELECT CASE WHEN COUNT(*) = 0 THEN 'CLAB000001' 
						 ...
						 AS next_clabno FROM rekab_contractors
		...
		function generateTempClabNo(){
			$sqlQuery = "SELECT CASE WHEN COUNT(*) = 0 THEN 'TEMP000001'
						 ...
						 AS temp_clabno FROM rekab_contractors
		...
		function getCtrNameByClabNo($clabno){
			$sqlQuery = "SELECT ctr_comp_name FROM rekab_contractors WHERE ctr_clab_no = '$clabno'";
		...
		function getCtrByClabExpiry($fieldname, $noOfMonths, $returnType){
			$sqlQuery = "SELECT rekab_contractors.*, tablex.print_date,tablex.print_doctype";
			
			$sqlQuery .= " FROM rekab_contractors
						   LEFT JOIN (SELECT * FROM ctr_printletter_history WHERE ctr_printletter_history.print_isvoid = 0 AND print_docduration = $noOfMonths AND print_doctype = '$fieldname') AS tablex ON rekab_contractors.ctr_clab_no = tablex.print_ctr_clabno 
						  WHERE ";
		...
		function getCtrByClabExpiry1($fieldname, $noOfMonths, $returnType){
			$sqlQuery = "SELECT * FROM rekab_contractors WHERE";
		...
		function listExpiryByCompany($fieldname, $noOfMonths, $returntype, $clabNo){
			...
			if($returntype == 'companylist')
				$sqlQuery .= " wkr_currentemp, COUNT(*) AS totalwkr, rekab_contractors.ctr_comp_name, ctr_printletter_history.print_date";
			else if($returntype == 'listOfWorkers')
				$sqlQuery .= " rekab_workers.*, mst_countries.*,ctr_printletter_history.print_date";
			else {}
			
			$sqlQuery .= " FROM rekab_workers
						  LEFT JOIN mst_countries ON rekab_workers.wkr_country = mst_countries.cty_id
						  LEFT JOIN rekab_contractors ON rekab_contractors.ctr_clab_no = rekab_workers.wkr_currentemp
						  LEFT JOIN ctr_printletter_history ON rekab_contractors.ctr_clab_no = ctr_printletter_history.print_ctr_clabno AND ctr_printletter_history.print_isvoid = '0' AND print_doctype = '$fieldname' AND print_docduration = $noOfMonths
						  WHERE ";
			...
			//only take active rekab_workers
			...
			if($returntype == 'companylist'){
				...
				$sqlQuery .= " AND rekab_workers.wkr_currentemp = '$clabNo'";
				$sqlQuery .= " AND TIMESTAMPDIFF(YEAR,rekab_workers.wkr_entrydate,NOW()) + 1 < '10'";
...
//			echo $sqlQuery;
		...
		function listExpiryByCompanyLast($fieldname, $noOfMonths, $returntype, $clabNo){
			...
			if($returntype == 'companylist')
				...
				$sqlQuery .= " rekab_workers.*, ctr_printletter_history.print_date,mst_countries.cty_desc";
			else {}
			...
			//only take inactive rekab_workers
...
//			echo $sqlQuery;
		...
		function listExpiryByCompany2($fieldname, $noOfMonths, $returntype, $limit, $offset){
			...
			$sqlQuery .= "SELECT  wkr_currentemp, COUNT(*) AS totalwkr, rekab_contractors.ctr_comp_name, ctr_printletter_history.print_date";
			$sqlQuery .= " FROM rekab_workers
						  LEFT JOIN mst_countries ON rekab_workers.wkr_country = mst_countries.cty_id
						  LEFT JOIN rekab_contractors ON rekab_contractors.ctr_clab_no = rekab_workers.wkr_currentemp
						  LEFT JOIN ctr_printletter_history ON rekab_contractors.ctr_clab_no = ctr_printletter_history.print_ctr_clabno AND ctr_printletter_history.print_isvoid = '0' AND print_doctype = '$fieldname' AND print_docduration = $noOfMonths
		...
		function listExpiryByCompany3($fieldname, $noOfMonths, $returntype, $clabNo){
			$sqlQuery = "SELECT ";
			if($returntype == 'companylist')
				$sqlQuery .= " wkr_currentemp, COUNT(*) AS totalwkr, rekab_contractors.ctr_comp_name, tablex.print_date";
			else if($returntype == 'listOfWorkers')
				$sqlQuery .= " rekab_workers.*, ctr_printletter_history.print_date";
			else {}
			
			$sqlQuery .= " FROM rekab_workers
						  LEFT JOIN mst_countries ON rekab_workers.wkr_country = mst_countries.cty_id
						  LEFT JOIN rekab_contractors ON rekab_contractors.ctr_clab_no = rekab_workers.wkr_currentemp
						  LEFT JOIN (SELECT * FROM ctr_printletter_history WHERE ctr_printletter_history.print_isvoid = 0 AND print_docduration = $noOfMonths AND print_doctype = '$fieldname') AS tablex ON rekab_workers.wkr_currentemp = tablex.print_ctr_clabno 
						  WHERE ";
		...
		function getLabourListByCompany($clabno){
			$sqlQuery = "SELECT * FROM rekab_workers 
						LEFT JOIN mst_countries ON mst_countries.cty_id = rekab_workers.wkr_country
						LEFT JOIN mst_nationality ON mst_nationality.nat_id = rekab_workers.wkr_nationality
						LEFT JOIN mst_race ON mst_race.race_id = rekab_workers.wkr_race
						LEFT JOIN mst_gender ON mst_gender.gender_id = rekab_workers.wkr_gender
						LEFT JOIN mst_wkr_availability ON mst_wkr_availability.avlab_id = rekab_workers.wkr_transtatus
						LEFT JOIN rekab_contractors ON rekab_contractors.ctr_clab_no = rekab_workers.wkr_currentemp
						LEFT JOIN mst_emp_status ON mst_emp_status.emp_statusid = rekab_workers.wkr_status
		...
		function getLabourListByCompanyc($clabno){
			$sqlQuery = "SELECT * FROM rekab_workers 
						LEFT JOIN mst_countries ON mst_countries.cty_id = rekab_workers.wkr_country
						LEFT JOIN mst_nationality ON mst_nationality.nat_id = rekab_workers.wkr_nationality
						LEFT JOIN mst_race ON mst_race.race_id = rekab_workers.wkr_race
						...
						LEFT JOIN rekab_contractors ON rekab_contractors.ctr_clab_no = rekab_workers.wkr_currentemp
						LEFT JOIN mst_emp_status ON mst_emp_status.emp_statusid = rekab_workers.wkr_status
						WHERE wkr_currentemp = '$clabno' and confirm = 1
						ORDER BY wkr_country, wkr_permitexp ASC";
						
			return $this->db->query($sqlQuery);		
		}
		
		function getLabourListByCompany0($clabno){
			$sqlQuery = "SELECT * FROM rekab_workers 
						LEFT JOIN mst_countries ON mst_countries.cty_id = rekab_workers.wkr_country
						LEFT JOIN mst_nationality ON mst_nationality.nat_id = rekab_workers.wkr_nationality
						LEFT JOIN mst_race ON mst_race.race_id = rekab_workers.wkr_race
						...
						LEFT JOIN rekab_contractors ON rekab_contractors.ctr_clab_no = rekab_workers.wkr_currentemp
						LEFT JOIN mst_emp_status ON mst_emp_status.emp_statusid = rekab_workers.wkr_status
						WHERE wkr_currentemp = '$clabno' and confirm = 0
						ORDER BY wkr_country, wkr_permitexp ASC";
		...
		function getAvlabDetails($clabno){
			$sqlQuery = "SELECT available.*, rekab_contractors.ctr_comp_name AS comp_to_name FROM available
						 LEFT JOIN rekab_contractors ON rekab_contractors.ctr_clab_no = available.avlab_comp_to
		...
		function getAvlabDetailsltNew($clabno,$ltref){
			$sqlQuery = "SELECT available.*, rekab_contractors.ctr_comp_name AS comp_to_name FROM available
						 LEFT JOIN rekab_contractors ON rekab_contractors.ctr_clab_no = available.avlab_comp_to
						 WHERE avlab_comp_to = '$clabno' and avlab_ref_no = '$ltref'";
			
			return $this->db->query($sqlQuery);
		}
		
		function getAvlabWkrDetails($avlab_ref_no){
			$sqlQuery = "SELECT available_wkr.*, rekab_workers.wkr_passno, rekab_workers.wkr_name, rekab_workers.wkr_permitexp, mst_countries.cty_desc
 						 FROM available_wkr 
						 LEFT JOIN rekab_workers ON avlabwkr_wkrid = rekab_workers.wkr_id
						 LEFT JOIN mst_countries ON mst_countries.cty_id = rekab_workers.wkr_country
