<?php 
	class ModelrekabLabour extends Model {
		function ModelrekabLabour(){
			parent::Model();	
		}	
		...
		function getAllSector(){
			$sqlquery = "SELECT * FROM rekab_sector order by sec_id";	
			
			return $this->db->query($sqlquery);
		}
		
		function getAlltype(){
			$sqlquery = "SELECT * FROM rekab_type order by type_id";	
		...
		function getAllContractors(){
			$sqlquery = "SELECT ctr_clab_no, ctr_comp_name, ctr_addr1, ctr_addr2, ctr_addr3, ctr_pcode, ctr_state FROM rekab_contractors
		...
		function getLabourByWkrID($wkr_id){
			$sqlquery = "SELECT rekab_workers.*,rekab_sector.sec_description,rekab_type.type_description, mst_countries.cty_desc, rekab_contractors.ctr_comp_name, mst_nationality.nat_desc, mst_race.race_desc, mst_gender.gender_desc, mst_wkr_availability.avlab_desc, mst_emp_status.emp_status_desc";
			$sqlquery .= " FROM rekab_workers ";
			$sqlquery .= " LEFT JOIN rekab_sector ON rekab_sector.sec_id = rekab_workers.wkr_sector";
			$sqlquery .= " LEFT JOIN rekab_type ON rekab_type.type_id = rekab_workers.wkr_type";
			$sqlquery .= " LEFT JOIN mst_countries ON mst_countries.cty_id = rekab_workers.wkr_country";
			$sqlquery .= " LEFT JOIN mst_nationality ON mst_nationality.nat_id = rekab_workers.wkr_nationality";
			$sqlquery .= " LEFT JOIN mst_race ON mst_race.race_id = rekab_workers.wkr_race";
			$sqlquery .= " LEFT JOIN mst_gender ON mst_gender.gender_id = rekab_workers.wkr_gender";
			$sqlquery .= " LEFT JOIN mst_wkr_availability ON mst_wkr_availability.avlab_id = rekab_workers.wkr_transtatus";
			$sqlquery .= " LEFT JOIN rekab_contractors ON rekab_contractors.ctr_clab_no = rekab_workers.wkr_currentemp";
			$sqlquery .= " LEFT JOIN mst_emp_status ON mst_emp_status.emp_statusid = rekab_workers.wkr_status";
...
function getCtrByClabNo($ctr_clab_no){
		...
		function labourExists($passportNo){
			$sqlQuery = "SELECT wkr_id, wkr_passno, wkr_name, emp_status_desc, wkr_country, wkr_race, wkr_permitexp, cty_desc, race_desc 
						 FROM rekab_workers
						 LEFT JOIN mst_emp_status ON rekab_workers.wkr_status = mst_emp_status.emp_statusid
		...
		function checkLabourExists($passNo, $country){
			$sqlQuery = "SELECT wkr_id, wkr_passno, wkr_name FROM rekab_workers WHERE wkr_passno = '$passNo' AND wkr_country = '$country'";
		...
		function getLaborByPermitPassExpiry($fieldname, $noOfMonths, $returnType, $country){
			...
			$sqlQuery .= " TIMESTAMPDIFF(YEAR,workers.wkr_entrydate,NOW()) + 1 < '10' AND ";
		...
		function getLaborByPermitPassExpiryOld($fieldname, $noOfMonths, $returnType, $country){
			$sqlQuery = "SELECT workers.*, mst_countries.cty_desc, mst_emp_status.emp_status_desc FROM workers ";
			$sqlQuery .= " LEFT JOIN mst_emp_status ON workers.wkr_status = mst_emp_status.emp_statusid";
			$sqlQuery .= " LEFT JOIN mst_countries ON mst_countries.cty_id = workers.wkr_country";
			$sqlQuery .= " WHERE";
		...
		function getsectorbyid($sec_id){
			$selectQuery = "SELECT sec_description FROM rekab_sector WHERE sec_id = '$sec_id'";	
			$secRecord = $this->db->query($selectQuery);
			$secRow = $secRecord->row();
			//echo $secRow;
			return $secRow->sec_description;
		}
		...
		function getNextWorkerID(){
			$sqlQuery = "SELECT CASE WHEN COUNT(*) = 0 THEN 1 ELSE MAX(wkr_id) + 1 END AS next_wkrid FROM rekab_workers";
			$wkrRecord = $this->db->query($sqlQuery);
		...
		function getUploadFileListPermit($filetype, $wkr_id){
			$sqlQuery = "SELECT * FROM wkr_uploaddoc
						WHERE upload_filetype LIKE '" . $filetype . "%' 
						AND upload_wkrid = $wkr_id
						ORDER BY upload_filetype ASC";
		...
		function getUploadFileListPassport($filetype, $wkr_id){
			$sqlQuery = "SELECT * FROM wkr_uploaddoc
						WHERE upload_filetype LIKE '" . $filetype . "%' 
						AND upload_wkrid = $wkr_id
						ORDER BY upload_filetype ASC";
    ...
    function getVDREmployees(){
			$sqlQuery = "SELECT emp_id, emp_username FROM employees WHERE emp_username != '' and `emp_dpt_id` = '17' ORDER BY emp_username and emp_status='1';";
