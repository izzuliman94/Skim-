	...
	class ModelSpim extends Model{
		...
		function getApprovalList(){
		  ...
		  $sql = "select workorders.*,contractors.ctr_comp_name from workorders 
				  ...
				  where wo_spim_status = 'V' and wo_iswithdrawal='0' and(chk_app_ceo <> '1' or chk_ver_corp <> '1' or chk_ver_fin <> '1')";
		...
		function getAllReceipt($clabno){
			...
			$sql = "select * from pmt_receipt where payment_type = 'V' and clab_no = '$clabno'";
			return $this->db->query($sql);
		...
		function getAllLegalList($workorderid){
			...
			$sqlQuery = "SELECT wo_fcl.fcl_id,wo_fcl.fcl_passno,wo_fcl.fcl_name,mst_nationality.nat_desc,
			             wo_fcl.fcl_bthplace,wo_fcl.fcl_passissue,fcl_passexp,fcl_salary,mst_worktrade.trade_desc,
						 wo_fcl.fcl_next_of_kin,wo_fcl.fcl_relationship,wo_fcl.fcl_salary,wo_fcl.fcl_wt,wo_fcl.fcl_sel
						 FROM wo_fcl 
						 ...
						 WHERE wo_fcl.fcl_woid = '$workorderid'";	
		...
		function getWOByWoidOrCompname($searchby, $keyword){
			$sqlQuery = "SELECT wo.wo_id, wo.wo_num, wo.wo_clab_no, wo.wo_fcl_total, ctr.ctr_comp_name, wo_status.wo_latest_progress 
						 ...
						 ON wo.wo_clab_no = ctr.ctr_clab_no
			   			 WHERE wo.wo_spim_status = 'V' AND $searchby LIKE '%$keyword%' 
		...
		function getWOByWoidOrCompnamePLKS($searchby, $keyword){
			$sqlQuery = "SELECT wo.wo_id, wo.wo_num, wo.wo_clab_no, wo.wo_fcl_total, ctr.ctr_comp_name, wo_status.wo_latest_progress 
						 ...
						 LEFT JOIN contractors ctr ON wo.wo_clab_no = ctr.ctr_clab_no
			   			 WHERE wo.wo_spim_status = 'V' AND $searchby LIKE '%$keyword%' and wo.wo_iswithdrawal = 1
and wo_status.wo_isreceive_visa = 1	 ORDER BY wo.wo_keyin_date DESC";
		...
		$sql = "select wo_fcl.*,mst_countries.cty_id,mst_countries.cty_desc,mst_nationality.nat_id,
		...
		$sql = "select contractors.ctr_comp_name,contractors.ctr_addr1,contractors.ctr_addr2,contractors.ctr_addr3,contractors.ctr_pcode,contractors.ctr_state,
		        ...
		        left join contractors on contractors.ctr_clab_no = workorders.wo_clab_no
				left join mst_states on mst_states.state_id = contractors.ctr_state
				where workorders.wo_spim_status = 'V' and wo_id='$wono'";
		...
		$sql = "select wo_fcl.fcl_name,wo_fcl.fcl_passno,mst_nationality.nat_desc,wo_fcl.fcl_gender,wo_fcl.fcl_dob, wo_fcl.fcl_pass,
		       wo_fcl.fcl_salary,wo_fcl.fcl_bthplace,wo_fcl.fcl_passissue,fcl_passexp,mst_worktrade.trade_desc,mst_countries.cty_period,
				mst_countries.cty_levi,mst_countries.cty_plks,mst_countries.cty_visa,mst_countries.cty_process,mst_countries.cty_fees,wo.wo_personincharge,wo_fcl.fcl_next_of_kin,
				wo_fcl.fcl_add,wo_fcl.fcl_relationship,wo_fcl.fcl_plksno,wo_fcl.fcl_add
		        from wo_fcl 
		...
		/***amik dari contractor skali address untuk handover***/
			...
			employees.emp_position,wo_aggrement.wo_agg_no
						...
						WHERE workorders.wo_spim_status = 'V' and workorders.wo_id = '$workorderid'";
		...
		function getAllAck(){
			$selectQuery = "SELECT * FROM acknowledgement where wo_ack_type = 'V' ORDER BY wo_num ";
			
			return $this->db->query($selectQuery);	
		}
			
		function getAllApproval(){
			$selectQuery = "SELECT * FROM wo_approval ORDER BY wo_approval.app_desc DESC";
		...
		WHERE workorders.wo_spim_status = 'V'  and wo_status.wo_latest_progress NOT IN ('close','closed', 'receive calling visa')
