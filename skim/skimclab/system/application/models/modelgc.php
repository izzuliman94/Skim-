<?php
	...
	/*	function getAllFCLFiles($workorderid){
		...
		function getAllLegalList($workorderid){
			...
			$sqlQuery = "SELECT wo_fcl_gc.fcl_id,wo_fcl_gc.fcl_passno,wo_fcl_gc.fcl_newpassno,wo_fcl_gc.fcl_name,mst_nationality.nat_desc,mst_wkr_availability.avlab_desc,
		...
		/***amik dari contractor skali address untuk handover***/
			$sqlQuery = "SELECT workorders.*, wo_agency.*,gc_consultant.*, mst_regional.*, wo_doc.*, wo_legal.*, wo_payment.*, wo_status.*, gc_app_type.*, ctr.ctr_clab_no, 
						...
						left join gc_app_type on workorders.wo_gc_type = gc_app_type.gc_type_id
