<?php
	class ModelAck extends Model{
		...
		function getApprovalList(){
		  ...
		  $sql = "select acknowledgement.*,contractors.ctr_comp_name from acknowledgement 
		          left join contractors on contractors.ctr_clab_no = acknowledgement.wo_clab_no
				  where wo_spim_status = 'A' and wo_iswithdrawal='0' and (chk_app_ceo <> '1' or chk_ver_corp <> '1' or chk_ver_fin <> '1')";
		  return $this->db->query($sql);       
		...
		function getAllReceipt($clab){
			...
			$sql = "select * from pmt_receipt where payment_type = 'R' and clab_no = '$clab'";
			return $this->db->query($sql);
		...
		function searchAck($searchby, $keyword){
		...
		function getNextWONumber(){
			$sqlQuery = "SELECT MAX(wo_id) as maxid FROM acknowledgement";	
	...
	/*	function getAllFCLFiles($workorderid){
		...
		function getWOByWoidOrCompname($searchby, $keyword){
			$sqlQuery = "SELECT wo.wo_id, wo.wo_num, wo.wo_clab_no, wo.wo_fcl_total, ctr.ctr_comp_name, ack_status.wo_latest_progress 
						 FROM acknowledgement wo 
						 LEFT JOIN ack_status ON wo.wo_id = ack_status.status_woid
						 LEFT JOIN contractors ctr ON wo.wo_clab_no = ctr.ctr_clab_no
						 
			   			 WHERE wo.wo_spim_status = 'A' AND $searchby LIKE '%$keyword%'
		...
		$sql = "select * from wo_fcl_renew where wo_fcl_renew.fcl_id = '$id'";
		...
		$sql = "select contractors.ctr_comp_name,contractors.ctr_addr1,contractors.ctr_addr2,contractors.ctr_addr3,contractors.ctr_pcode,
				...
				where workorders.wo_spim_status = 'R' and wo_id='$wono'";
		...
		$sql = "select wo_fcl_renew.fcl_passno,wo_fcl_renew.fcl_name,mst_nationality.nat_desc,wo_fcl_renew.fcl_salary,wo_fcl_renew.fcl_year_renewal,
		        wo_fcl_renew.fcl_permitno,fcl_add,wo_fcl_renew.fcl_next_of_kin,wo_fcl_renew.fcl_nationality, wo_fcl_renew.fcl_relationship,fcl_passexp,mst_worktrade.trade_desc,mst_countries.cty_period,mst_countries.cty_levi,wo_fcl_renew.fcl_pass,
				wo_fcl_renew.fcl_dob,wo_fcl_renew.fcl_gender,wo_fcl_renew.fcl_newpassno,wo_fcl_renew.fcl_wo_type,wo_fcl_renew.fcl_wo_type,wo_fcl_renew.fcl_plksdate1,wo_fcl_renew.fcl_plksdate2,			mst_countries.cty_plks,mst_countries.cty_visa,workers.wkr_gender,workers.wkr_dob,mst_countries.cty_process,mst_countries.cty_fees,wo.wo_personincharge,wkr_permitexp
		        from wo_fcl_renew
		        LEFT JOIN workorders wo ON wo.wo_id = wo_fcl_renew.fcl_woid
				left join workers on workers.wkr_passno = wo_fcl_renew.fcl_passno 
				left join mst_worktrade on mst_worktrade.trade_id = wo_fcl_renew.fcl_wt
				left join mst_nationality on mst_nationality.nat_id = wo_fcl_renew.fcl_nationality
				left join mst_countries on mst_countries.cty_id = wo_fcl_renew.wkr_country 
				left join mst_wo_type on mst_wo_type.mst_wo_type_id = wo_fcl_renew.fcl_wo_type 
		...
		/***amik dari contractor skali address untuk handover***/
			$sqlQuery = "SELECT acknowledgement.*, wo_agency.*, ack_doc.*, wo_legal.*, ack_payment.*, ack_status.*, ctr.ctr_clab_no, pmt_type.pmt_type_desc,
						 ...
						 wo_agency.agency_name,acknowledgement.wo_visa_date,employees.emp_name,employees.emp_position,wo_aggrement.wo_agg_no
			            FROM acknowledgement 
						LEFT JOIN wo_agency ON acknowledgement.wo_agency = wo_agency.agency_id
						LEFT JOIN ack_doc ON acknowledgement.wo_id = ack_doc.doc_woid
						LEFT JOIN wo_legal ON acknowledgement.wo_id = wo_legal.legal_woid
						LEFT JOIN ack_payment ON acknowledgement.wo_id = ack_payment.pay_woid
						LEFT JOIN ack_status ON acknowledgement.wo_id = ack_status.status_woid
						LEFT JOIN contractors ctr ON acknowledgement.wo_clab_no = ctr.ctr_clab_no
						left join mst_states on ctr.ctr_state = mst_states.state_id 
						LEFT JOIN pmt_type ON acknowledgement.wo_ack_type = pmt_type.pmt_type_id
						LEFT JOIN mst_countries ON acknowledgement.wo_fcl_country = mst_countries.cty_id
						left join employees on employees.emp_username = acknowledgement.wo_personincharge
						left join wo_aggrement on wo_aggrement.wo_no = acknowledgement.wo_id
						WHERE acknowledgement.wo_spim_status = 'A' and acknowledgement.wo_id = $workorderid";
		...
		function getWorkordersByStatus($wo_status, $returnType){
			$sqlQuery = "SELECT wo.wo_id, wo_num, wo.wo_clab_no, wo.wo_fcl_total, wo.wo_keyin_date, contractors.ctr_comp_name, wo_latest_progress
						 ...
						 where wo.wo_spim_status = 'R' ";
						 
			if($wo_status != "overall"){
				if($wo_status == "open")
					$sqlQuery .= " and wo_status.wo_latest_progress NOT IN ('receive calling visa', 'closed')";
		...
		function getAllPhTrackLog($workorderid){
			$sqlQuery = "SELECT * FROM ack_phonetrack 
		...
		function getWoUnderCurrentUser($currentuser){
			$sqlQuery = "SELECT workorders.*, wo_status.wo_latest_progress, wo_status.wo_receivedate, contractors.ctr_comp_name FROM workorders 
						...
						LEFT JOIN contractors ON contractors.ctr_clab_no = workorders.wo_clab_no
						WHERE workorders.wo_spim_status = 'R' and wo_status.wo_latest_progress NOT IN ('closed','close', 'receive calling visa')
						ORDER BY wo_date";
