<?php
	class ModelRenewal extends Model{
		function ModelRenewal(){
			parent::Model();
			$this->load->database();	
		}
		...
		function getAllWorkers(){
			$sqlquery = "SELECT wkr_socso_id AS socsoid FROM workers";
	...
	/*	function getAllFCLFiles($workorderid){
		...
		function getAllLegalList($workorderid){
			...
			function getAllFCLFiles($workorderid){
			$sqlQuery = "SELECT wo_fcl_renew.fcl_id,wo_fcl_renew.fcl_passno,wo_fcl_renew.fcl_newpassno,wo_fcl_renew.fcl_name,mst_nationality.nat_desc, workers.wkr_socso_id,
           			     wo_fcl_renew.fcl_permitno, wo_fcl_renew.fcl_nationality,fcl_passexp,fcl_salary,wo_fcl_renew.fcl_next_of_kin,wo_fcl_renew.fcl_relationship,wo_fcl_renew.fcl_dob,wo_fcl_renew.fcl_recppt,wo_fcl_renew.fcl_recpptdate,
						 ...
						 FROM wo_fcl_renew 					
						 left join workers on workers.wkr_passno = wo_fcl_renew.fcl_newpassno
		...
		function getSelFCLFiles($workorderid){
			$sqlQuery = "SELECT wo_fcl_renew.fcl_id,wo_fcl_renew.fcl_passno,wo_fcl_renew.fcl_newpassno,wo_fcl_renew.fcl_name,mst_nationality.nat_desc,
						 ...
						 left join mst_worktrade on mst_worktrade.trade_id = wo_fcl_renew.fcl_wt 
						 left join mst_wo_type on mst_wo_type.mst_wo_type_id = wo_fcl_renew.fcl_wo_type
						 WHERE wo_fcl_renew.fcl_woid = '$workorderid' and wo_fcl_renew.fcl_recppt = '1'";	
		...
		function getWOByWoidOrCompname($searchby, $keyword){
			$sqlQuery = "SELECT wo.wo_id, wo.wo_num, wo.wo_clab_no, wo.wo_fcl_total, ctr.ctr_comp_name, wo_status.wo_latest_progress 
						 ...
						 ON wo.wo_clab_no = ctr.ctr_clab_no
			   			 WHERE wo.wo_spim_status = 'R' AND $searchby LIKE '%$keyword%'
		...
		$sql = "select workers.wkr_nationality,wo_fcl_renew.fcl_passno,wo_fcl_renew.fcl_name,mst_nationality.nat_desc,wo_fcl_renew.fcl_salary,wo_fcl_renew.fcl_year_renewal,workers.wkr_socso_id,
				...
				wo_fcl_renew.fcl_dob,wo_fcl_renew.fcl_gender,wo_fcl_renew.fcl_wt,wo_fcl_renew.fcl_newpassno,wo_fcl_renew.fcl_wo_type,wo_fcl_renew.fcl_wo_type,wo_fcl_renew.fcl_plksdate1,wo_fcl_renew.fcl_plksdate2,mst_countries.cty_plks,mst_countries.cty_visa,workers.wkr_gender,workers.wkr_dob,mst_countries.cty_process,mst_countries.cty_fees,wo.wo_personincharge,wkr_permitexp
		        from wo_fcl_renew
		...
		/***amik dari contractor skali address untuk handover***/
			$sqlQuery = "SELECT workorders.*, wo_agency.*, wo_doc.*, wo_legal.*, wo_payment.*, wo_status.*, ctr.ctr_clab_no, 
			             ctr.ctr_comp_regno, ctr.ctr_addr1, ctr_addr2, ctr_addr3, ctr_pcode, state_name, ctr_fax, ctr_telno,   
						 ctr.ctr_comp_name, ctr.ctr_contact_name, ctr.ctr_contact_mobileno, ctr.ctr_socso_no, mst_countries.cty_desc,
						...
						WHERE workorders.wo_spim_status = 'R' and workorders.wo_id = $workorderid";
		...
		function getAllAck(){
			$selectQuery = "SELECT * FROM acknowledgement where wo_ack_type = 'R' ORDER BY wo_num";
