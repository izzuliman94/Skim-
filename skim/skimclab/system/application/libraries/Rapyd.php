...
if (!defined('RAPYD_PATH')){
  ...
  include(APPPATH.'config/rapyd'.EXT);
	 ...
	 *****************************************************************************************/
	 $rapyd_apppath = str_replace( str_replace("\\", "/", realpath(dirname(FCPATH))).'/' ,"", APPPATH );
 ...
 */
    ...
    var $data_conn = '';//Should be set by config 
      ...
      //MODIF 0 LANG
			//Finally we keep loading rapyd uri before siniffing lang to keep the lang segment into Rapyd uri_array (rapyd work with or without 
			//the lang into uri_array, because in Rapyd Uri we read the URI from the right and keep the remainig left uri into the 'base_uri')
			//this way rapyd uri_array can propagate the language in links...
			//and then rapyd lang remove it to segments.....If we load lang before uri all work fine except that the language could not be propagate...
