...
<div id="switchsection1" class="switchgroup1"> 
  <form method="POST" action="<?php echo site_url();?>/contrekabLabour/attachMultipleDocSubmit" name="uploadForm" id="uploadForm" enctype="multipart/form-data">
	      	...
	      	<tr>
	      		...
	      		<td>
	      			<SELECT name="seldoctype<?php echo $i;?>" onchange="changeFormElements<?php echo $i;?>();">
	      				...
	      				<option value="CERTIFICATE">Cert(SCC)</option>
	...
	<tr>
		...
		<td align="CENTER">
			...
			if(substr($upload->upload_filetype, 0, 4) == 'PLKS'){
				echo anchor_popup("contrekabLabour/printAttachPermit/PLKS/" . $wkr_id, " Print all Permit");
			}
			else if(substr($upload->upload_filetype, 0, 11) == 'SpecialPass'){
				echo anchor_popup("contrekabLabour/printAttachPermit/SpecialPass/" . $wkr_id, "Print all SpecialPass");
			}
			else{
			 	echo " - ";
		 	}
		...
		<form name="deleteform" method="POST" action="<?php echo site_url();?>/contrekabLabour/deleteUploadedFile/<?php echo $upload->upload_id . "/" . $upload->upload_wkrid;?>">
		<td align="CENTER">
