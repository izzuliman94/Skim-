...
<script type="text/javascript">
	...
	function Inspchs(){
	 ...
	 var url = "<?php echo site_url('contgc/email_item');?>/" + workorderid 
	...
	function newphtracklog(){
		...
		var companyname = document.updateWorkorderForm.txtclabno.value
		var url = "<?php echo site_url('contgc/newPhoneTrackLog');?>/" + workorderid + "/" + companyname
	...
	function registerNewLabor(){
		...
		var url = "<?php echo site_url('contgc/regnewlabour');?>/" + workorderid + "/" + companyname
		...
		var url = "<?php echo site_url('contgc/regnewlegal');?>/" + workorderid + "/" + companyname
	...
	function EditFCL(id){
		...
		var url = "<?php echo site_url('contgc/EditFCL');?>/" + workorderid + "/" + companyname + "/" + id
	...
	function EditLegal(id){
		...
		var url = "<?php echo site_url('contgc/EditLegal');?>/" + workorderid + "/" + companyname + "/" + id
    ...
    function openBatch(clabid){
		...
		var url = "<?php echo site_url('contgc/batchlistwo');?>/" + clabno + "/" + encodeURIComponent(companyname)
	...
	function OpenLampiran(wono){
		var wono = wono;
		var url = "<?php echo site_url('contgc/lampiran_a');?>/" + wono
	...
	function OpenSecondSchedule(wono){
		var wono = wono;
		var url = "<?php echo site_url('contgc/second_schedule');?>/" + wono
	...
	function OpenIM12(wono){
		var wono = wono;
		var url = "<?php echo site_url('contgc/im12');?>/" + wono
	...
	function OpenAddendum(wono){
		var wono = wono;
		var url = "<?php echo site_url('contgc/addendum');?>/" + wono
	...
	function OpenBorangBayaran(wono){
		var wono = wono;
		var url = "<?php echo site_url('contgc/borangbayaran');?>/" + wono
	...
	function openack(wono){
		var wono = wono;
		var url = "<?php echo site_url('contgc/acknowledge');?>/" + wono
	...
	function handover(wono){
	   ...
	   var url = "<?php echo site_url('contgc/handoverletter');?>/" + wono + "/" + workorderid
	...
	function notification(wono){
       var workorderid = document.updateWorkorderForm.workorderno.value
	   var wono = document.updateWorkorderForm.txtwoid.value
	   var url = "<?php echo site_url('contgc/notificationletter');?>/" + wono + "/" + workorderid
	   window.open(url,'handover letter','height=500, width=800, toolbar=yes, directories=no, status=no, location=no, menubar=no, scrollbars=yes');
	}
	...
	function pengesahan(wono){
       var workorderid = document.updateWorkorderForm.workorderno.value
	   var wono = document.updateWorkorderForm.txtwoid.value
	   var url = "<?php echo site_url('contgc/confirmletter');?>/" + wono + "/" + workorderid
	...
	function openprintform(){
	   window.open('<?php echo site_url('contgc/openprintform');?>','Print Form','height=500, width=800, toolbar=no, directories=no, status=no, location=no, menubar=no, scrollbars=yes');
	}
...
window.open(url, 'Payment list', 'height=350,width=700, toolbar=no, directories=no, status=no, location=no, menubar=no, scrollbars=yes') 
	...
	function OpenLampiranExcel(wono){
		var wono = wono;
		var url = "<?php echo site_url('contgc/lampiran_a_excel');?>/" + wono
	...
	function OpenLampiranExcelqbe(wono){
		var wono = wono;
		var url = "<?php echo site_url('contgc/lampiran_qbe');?>/" + wono
		window.open(url, 'Lampiran A', 'height=500, width=800, toolbar=yes, directories=no, status=no, location=no, menubar=no, scrollbars=yes');
...
<?php 
	...
	if($woRow->wo_yearrenew == '2' || $woRow->wo_yearrenew == '3'){
	   ...
	   $strver2 = "disabled";
	   $strver3 = "disabled";
	   $strprocess = "enfcl_pgcdate abled";
	
	}
	
	
?>
<h4><a href="<?php echo site_url();?>/contgc/spimDashbrd">SPIM New Green Card Application</a> <img src="<?php echo base_url();?>/images/right_arrow.gif" width="13" height="14" />Update Work Order </h4>
...
?>
  <form action="<?php echo site_url();?>/contgc/updateWorkOrderPt2Submit" method="POST" name="updateWorkorderForm" id="updateWorkorderForm"  onsubmit="return v.exec();">
    <table width="100%" border="0">
		...
		<tr>
		  <td>Email</td>
		  <td><input type="text" name="txtemail" size="20" value="<?php echo $woRow->wo_email;?>" /></td>
		  <td>&nbsp;</td>
		  <td>&nbsp;</td>
	  </tr>
		<tr>
		  <td>Region Office</td>
		  <td><input type="text" name="txtregion" size="20" value="<?php echo $woRow->regional_desc;?>" /></td>
		  <td>&nbsp;</td>
		  <td>&nbsp;</td>
	  </tr>
		<tr>
		   <td>GreenCard Type Application</td>
		   <td><input type="text" name="txttype" size="20" value="<?php echo $woRow->gc_type_desc;?>" /></td>
		    <td>&nbsp;</td>
		   <td>&nbsp;</td>	
		</tr>
		<!--tr>
        ...
        <!--tr>
		   <td>GreenCard Type Application (Additional)</td>
		    <td><select name="selyearrenew" onchange="fomemacheck(this.value)"><option value='0'>Select Year</option>
		      <option value="1" <?php if($woRow->wo_yearrenew == '1') echo "selected";?>>Old Card</option>
			  <option value="2" <?php if($woRow->wo_yearrenew == '2') echo "selected";?>>Bio Card</option>
			  </select></td>
		    <td>&nbsp;</td>
		   <td>&nbsp;</td>	
		</tr-->			
    ...
    <table width="100%" border="0">
      <tr>
    	...
    	<tr>
    	  <td align="LEFT"><input type="checkbox" name="chkSubang" value="1" <?php if($woRow->chk_trn_subang == '1') echo "CHECKED"; ?>/>
   	      Subang, Shah Alam, Kuala Lumpur</td>
    	  <td align='left'><input type="text" name="txtregion2" size="20" value="<?php echo $woRow->gc_train_desc;?>" /></td>
   	  </tr>
    	<tr>
    	  <td align="LEFT"><input type="checkbox" name="chkjohor" value="1" <?php if($woRow->chk_trn_johor == '1') echo "CHECKED"; ?>/>
    	    Johor</td>
    	  <td align='left'>Date:
            <input type="text" name="dtclass" id="dtclass" value="<?php if($woRow->wo_class_date != "0000-00-00") echo date('d-m-Y', strtotime($woRow->wo_class_date));?>" size="12"/>
          <input id="button" name="button3" type="button" class="scwdatepicker" onclick="scwShow(document.getElementById('dtclass'), this)" /></td>
  	  </tr>
    	<tr>
    	  <td align="LEFT"><input type="checkbox" name="chkpenang" value="1" <?php if($woRow->chk_trn_penang == '1') echo "CHECKED"; ?>/>
   	      Pulau Pinang</td>
    	  <td align='RIGHT'>&nbsp;</td>
  	  </tr>
    	<tr>
    	  <td align="LEFT"><input type="checkbox" name="chkothers" value="1" <?php if($woRow->chk_trn_others == '1') echo "CHECKED"; ?>/>
    	    Others</td>
    	  <td align='RIGHT'>&nbsp;</td>
  	  </tr>
    	<tr>
		...
		<tr>
		    <td><input type="checkbox" name="chkoripass" value="1"  <?php if($woRow->doc_ori_passport == '1') echo "CHECKED"; ?> /> 
		      Payment <font color="red">*</font></td>
		...
		<!--tr>
			...
			<!--td><input type="checkbox" name="chkfomema" value="1" <?php if($woRow->doc_fomema == '1') echo "CHECKED"; ?> /> 
			  Sijil SICW</td>
		...
		<!--tr>
		    <td><input type="checkbox" name="chkappletter" value="1" <?php if($woRow->doc_extplks == '1') echo "CHECKED"; ?>/> 
		      Employer's Apllication Form</td>
    ...
    <br />
       <table width="100%" border="0">
		...
		<tr>
    		<td>OFFICIAL RECEIPT NO<font color="red">*</font>.: <input type="text" name="payrefno" value="<?php echo $woRow->pay_refno;?>" size="60" /><input type="button" value="..." onclick="paymentlist('<?php echo $woRow->wo_clab_no;?>')"/></td>
    	</tr>
    	
    </table>
      <br />
      <table width="100%" border="0" bordercolor=#C92124>
      	<tr>
      		<th colspan="5" align="LEFT">LIST OF FCL</th>
      		<th align="RIGHT" colspan="12"><input type="button" name="btnRegisNewLabour" value="Register New Labour" onclick="registerNewLabor();" <?php if(strpos($accessibility, 'h') == false) echo "DISABLED";  ?><?php echo $close ?>/><!--input type="button" name="btnUploadFCL" value="Upload FCL list" class="smoothbtn1" onclick="openuploadwindow();" /--></th>
      	</tr>
      	<?php if($allFCLFiles->num_rows() == 0){ ?>
      	<tr>
      		<td colspan="17"><font color="red"><strong>No FCL lists has been insert for this workorder.</strong></font></td>
      	</tr>
      	<?php }
      	else{
	    ?>
	    <tr>
            ...
            <!--th>Wages</th-->
			<!--th>Trade</th-->
			<th>No Previous GC</th>
			<th>Labour Status</th>
			<th><p>GC Process</p></th>
			<th>Process Date</th>
			<th>GC Received</th>
			<th>Received Date</th>
	    ...
	    <?php $i = 1;
      	 ...
      	 <tr>
			...
			<td align="center"><input name="chkprgc" type="checkbox" disabled="disabled" value="1" <?php if($fcl->fcl_prgc == '1') echo "CHECKED"; ?>/></td>
			<td><?php echo $fcl->avlab_desc;?></td>
			<td align="center"><input name="chkgcp" type="checkbox" disabled="disabled" value="1" <?php if($fcl->fcl_gcp == '1') echo "CHECKED"; ?>/></td>
			<td align="center"><?php echo $fcl->fcl_gcpdate;?></td>
		   <td align="center"><input name="chkgcr" type="checkbox" disabled="disabled" value="1" <?php if($fcl->fcl_gcr == '1') echo "CHECKED"; ?>/></td>
			<td align="center"><?php echo $fcl->fcl_gcrdate;?></td>
            <td align="center">
         ...
         <tr>
			...
			<input type="button" value="Export FCL (Excel)" onclick="OpenLampiranExcel('<?php echo $woRow->wo_id; ?>')" <?php if($allFCLFiles->num_rows() == 0){ echo "disabled"; }?> />
...
<br />
    <table width="100%" border="0">
      	<tr>
      		...
      		<th align="RIGHT"><!--input type="button" name="btnRegisNewLabour" value="Register New Labour" onclick="registerNewLabor();"/--><input type="button" name="btnUploadFCL" value="Upload FCL list"  onclick="openuploadwindow();" /></th>
      	</tr>
	    ...
	    <?php $i = 1;
      	 ...
      	 <tr>
      	 	<td><?php echo $i++;?></td>
      	 	<td><a href="<?php echo base_url();?>uploads/spimgc/<?php echo $file->upload_destfilename;?>"><?php echo $file->upload_destfilename;?></a></td>
    ...
    <table width="100%" border="0">
    	...
    	<tr>
    		<td>Received by GreenCard Dept: <font color="red">*</font></td>
    	...
    	<tr>
    		...
    		<td>
	    		<table width="100%">
	    			<tr><!--td align="center"><?php echo anchor_popup("contgc/printCheckLits/". $woRow->wo_id,"Checklist");?></td--></tr>
	    			<tr><!--td align="center"><!--input type="button" value="Insurance Purchase" onclick="Inspchs()" /--></td--></tr>
					<tr><!--td align="center"><?php echo anchor_popup("contgc/printPLKSIP/". $woRow->wo_id ."/".$woRow->wo_num,"PLKS in Progress");?></td--></tr>
	    		</table>    		</td>
    		<td>Invoice No:
    		  <input type="text" name="txtprocesscomment" value="<?php echo $woRow->wo_process_comment;?>"/></td>
    	</tr>
		<tr>
		    <td>2nd Step ( RM25 )</td>
			<td><input type="checkbox" name="chkvercorp" value="1" onclick="checktrigger(this, txtvercorp, dtvercorp);" <?php if($woRow->chk_ver_corp == '1') echo "CHECKED DISABLED";?> />
			...
			<td><input type="text" name="dtvercorp" size="12" id="dtvercorp" value="<?php if(strlen($woRow->ver_corp_date) > 0 && $woRow->ver_corp_date != '0000-00-00') echo date('d-m-Y', strtotime($woRow->ver_corp_date));?>" READONLY/>
    			<input id="button14" name="btnDate14" type="button" class="scwdatepicker" onclick="scwShow(document.getElementById('dtvercorp'), this)"/> </td>
			<td>&nbsp;</td>
			<td>Invoice No:
			  <input type="text" name="txtprocesscomment2" value="<?php echo $woRow->wo_process_comment2;?>"/></td>
		</tr>
		<tr>
		    <td>Send Temp Pass To VDR/EXT</td>
			...
			<td><input type="text" name="dtverfin" size="12" id="dtverfin" value="<?php if(strlen($woRow->ver_fin_date) > 0 && $woRow->ver_fin_date != '0000-00-00') echo date('d-m-Y', strtotime($woRow->ver_fin_date));?>" READONLY/>
    			<input id="button14" name="btnDate14" type="button" class="scwdatepicker" onclick="scwShow(document.getElementById('dtverfin'), this)"/> </td>
			<td>&nbsp;</td>
			<td><input type="text" name="txtprocesscomment3" value="<?php echo $woRow->wo_process_comment3;?>"/></td>
		</tr>
		<tr>
		    <td>Received Card From CIDB</td>
			...
			<td><input type="text" name="dtappceo" size="12" id="dtappceo" value="<?php if(strlen($woRow->app_ceo_date) > 0 && $woRow->app_ceo_date != '0000-00-00') echo date('d-m-Y', strtotime($woRow->app_ceo_date));?>" READONLY/>
    			<input id="button14" name="btnDate14" type="button" class="scwdatepicker" onclick="scwShow(document.getElementById('dtappceo'), this)"/> </td>
			<td>&nbsp;</td>
			<td><input type="text" name="txtprocesscomment4" value="<?php echo $woRow->wo_process_comment4;?>"/></td>
		</tr>
		<tr><th colspan="5">&nbsp;</th></tr>
		<tr>
		    <td>Handover and Collection</td>
			...
			<td>&nbsp;</td>
			<td><input type="text" name="txtprocesscomment5" value="<?php echo $woRow->wo_process_comment5;?>"/></td>
		</tr>
		<tr>
		    <td>&nbsp;</td>
			<td colspan="4"><input name="button" type="button" onclick="notification('<?php echo $woRow->wo_clab_no;?>')" value="Notification Letter"/>&nbsp;<input name="button" type="button" onclick="handover('<?php echo $woRow->wo_clab_no;?>')" value="Handover Letter"/>
    ...
    <table width="100%" border="0">
    	<tr>
    		<td align="center">
				...
				<input type="button" name="btnCancel" value=" Back " onclick="location.href='<?php echo site_url();?>/contgc/updateWorkorder'" />    		
    		</td>
    	</tr>
    </table>
</form>
  ...
  <table width="100%" border="0">
  	<tr>
  		...
  		<th colspan="1" align="RIGHT"><input type="button" name="btnNewLog" value=" New Log " onclick="newphtracklog();"  <?php if($woRow->wo_latest_progress == "closed") echo "DISABLED";?>/>
  			<input type="button" name="btnRefresh" value=" Refresh Log " onclick="location.href='<?php echo site_url();?>/contgc/updateWorkOrderPt2/<?php echo $woRow->wo_id;?>/updatephonetrack'"  <?php if($woRow->wo_latest_progress == "closed") echo "DISABLED";?>/>
  		</th>
  	</tr>
  	<tr>
  	    <th>No.</th>
...
<script>
	...
	var a_fields = {
		...
		'chkoripass':{'l':'Payment','r':true,'t':'dtoripass'},
		'chkisreceive':{'l':'Received By GC Department','r':true,'t':'txtreceiveby'},
	},
