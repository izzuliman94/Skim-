	...
	function generateReportexcel(type)
      {
	    ...
	    var url = "<?php echo site_url('contPayment/xlssumallexcel')?>/" + datefrom + "/" + dateto;
        window.open(url);
	  ...
	  function generateReportview(type)
      {
	    ...
	    var datefrom = document.sumvdr.datefrom.value;
		var dateto = document.sumvdr.dateto.value;
	    var url = "<?php echo site_url('contPayment/xlssumallview')?>/" + datefrom + "/" + dateto;
        window.open(url);
...
<body>
<h4><a href="dashboard.php">Payment Summary (ALL PAYMENT RECORDED) </a></h4>
