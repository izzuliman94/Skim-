...
?>
  <form action="<?php echo site_url();?>/contSpim/updateWorkOrderPt2Submit" method="POST" name="updateWorkorderForm" id="updateWorkorderForm"  onsubmit="return v.exec();">
    <table width="100%" border="0">
		<tr>
			<td width="10%">Workorder No</td>
			<td width="18%"><input type="hidden" name="txtwoid" value="<?php echo $woRow->wo_id;?>" />			  <input type="text" name="workorderno" value="<?php if(strlen($woRow->wo_num) == 0) echo $woRow->wo_id; else echo $woRow->wo_num;?>" READONLY/></td>
		  <td width="5%">Ack No</td>
			<td width="27%"><input type="text" name="txtackno" value="<?php if(strlen($woRow->wo_ack) == 0) echo "No Ack No"; else echo $woRow->wo_ack;?>" /></td>
			<td width="8%">Date Submit</td>
		  <td width="32%">
		...
		<tr>
		  ...
		  <td colspan="3"><input type="text" name="txtsenderctc" value="<?php echo $woRow->wo_sender_contact;?>" /></td>
		  <td>Approval No</td>
          ...
          <td width="0%"><!--input id="button2" name="btnDate2" type="button" class="scwdatepicker" onclick="scwShow(document.getElementById('dtvisa'), this)" /--></td>		 
	  </tr>
		<tr>
		  <td>Attending Officer </td>
		  <td colspan="3"><input type="text" name="txtattnoff" value="<?php echo  $woRow->wo_attn_officer; ?>" size="40"/></td>
		  <td>&nbsp;</td>
		  <td>&nbsp;</td>
	  </tr>
    </table>
    ...
    <table width="100%" border="0">
    	...
    	<tr>
    		<td><input type="checkbox" name="chkreqform" value="1" <?php if($woRow->doc_rqform == '1') echo "CHECKED "; ?> />
    		Surat Kelulusan Asal dari Kementerian Dalam Negeri (OSC)&nbsp; &nbsp;&nbsp;</td>
			<td align="RIGHT">
    	...
    	<tr>
    		<td><input type="checkbox" name="chkawardletter" value="1" <?php if($woRow->doc_awardletter == '1') echo "CHECKED ";?> <?php echo $doc ?> /> 
    		Surat Wakil Syarikat (Salinan I/C Wakil Syarikat dan No Telefon), Penyata KWSP (3 Bulan terkini) / Pengarah/ Pemilik Syarikat&nbsp; &nbsp; &nbsp;  &nbsp;&nbsp;</td>
			<td align="RIGHT"><!--input type="button" name="btnUpload" value="Upload Documents" onclick="openuploadwindow();" DISABLED/-->
    	...
    	<tr>
    		...
    		<td><input type="checkbox" name="chksignedpay" value="1" <?php if($woRow->doc_signedpayment == '1') echo "CHECKED ";?> <?php echo $doc ?>/> Penyata PERKESO 3 Bulan terkini    			</td>
			<td align="RIGHT"><!--input type="button" name="btnUpload" value="Upload Documents" onclick="openuploadwindow();" DISABLED/-->
	        ...
	        <input id="button6" name="button5" type="button" class="scwdatepicker" onclick="scwShow(document.getElementById('dtsignedpay'), this)" /></td>
    	</tr>
    	<tr>
			<td><p>
		      <input type="checkbox" name="chksupagree" value="1" <?php if($woRow->doc_supplieragree == '1') echo "CHECKED ";?> <?php echo $doc ?>/> 
		      Biodata Pekerja ( Sijil Kesihatan Asal, 2 Keping Salinan Pasport Pekerja, 2 Keping Gambar Berukuran Pasport, Salinan ISC) 
    			
			<legal> 
				<h5><em>*Tempoh Sah Laku Sijil Kesihatan hendaklah melebihi 30 Hari dari tarikh penghantaran</em></h5>
			  	<h5><em>*Tempoh Sah Laku Pasport mestilah melebihi 18 Bulan semasa proses penghantaran ke CLAB</em></h5>
			  	<h5><em>*Nama Waris, Hubungan serta Alamat Lengkap Waris Diperlukan</em></h5></p>
			 </legal>
				
    		</td>
			<td align="RIGHT"><!--input type="button" name="btnUpload" value="Upload Documents" onclick="openuploadwindow();" DISABLED/-->
		      <input type="text" name="dtsupagree" id="dtsupagree" size="11" value="<?php if($woRow->doc_supplieragreedate != "0000-00-00") echo date('d-m-Y', strtotime($woRow->doc_supplieragreedate));?>" <?php echo $doctext ?>/>
	        <input id="button10" name="btnDate10" type="button" class="scwdatepicker" onclick="scwShow(document.getElementById('dtsupagree'), this)" /></td>
...
<td>FWHS:</td>
  ...
  <table width="100%" border="0">
    	<tr>
    		...
    		<td>&nbsp;</td>
<td><input type="hidden" name="txtcspolicy"  value="<?php echo $woRow->pay_fwcsno;?>"/></td>
    		<td>FWHS:</td>
...
<table width="100%" border="0">
    ...
    <table width="100%" border="0">
		...
		<tr>
	      ...
	      <input id="button" name="button3" type="button" class="scwdatepicker" onclick="scwShow(document.getElementById('dtsubmitlegal'), this)" <?php echo $strprocess ?>/></td>
		  <td><p>&nbsp;</p>
	      <p>Submit  : 
	        <input type="text" name="dtsubmitdl" size="12" id="dtsubmitdl" value="<?php if(strlen($woRow->wo_submitdldate) > 0 && $woRow->wo_submitdldate != '0000-00-00') echo date('d-m-Y', strtotime($woRow->wo_submitdldate));?>" readonly="readonly"/>
            <input id="button8" name="button6" type="button" class="scwdatepicker" onclick="scwShow(document.getElementById('dtsubmitdl'), this)" <?php echo $strprocess ?>/>
	      </p>
	      <p>Collect  : 
	        <input type="text" name="dtcollectdl" size="12" id="dtcollectdl" value="<?php if(strlen($woRow->wo_collectdldate) > 0 && $woRow->wo_collectdldate != '0000-00-00') echo date('d-m-Y', strtotime($woRow->wo_collectdldate));?>" readonly="readonly"/>
            <input id="button9" name="button7" type="button" class="scwdatepicker" onclick="scwShow(document.getElementById('dtcollectdl'), this)" <?php echo $strprocess ?>/>
	      </p>
	      <p>Remarks :
  <input name="txtsentlegalcomment" type="text"  value="<?php echo $woRow->wo_sentlegalcomment;?>"/>
          </p></td>
		  <td>&nbsp;</td>
	  </tr>
		<tr>
    	...
    	<tr>
    		<td>Received Payment From Finance:</td>
    	...
    	<tr>
    		...
    		<td>Total Approve: <input type="text" name="txtapprove" size="4" value="<?php echo $woRow->wo_receivevisa_approve;?>" READONLY/> <br />
                ...
                <!--input type="text" name="txtreject" size="4" value="<?php echo $woRow->wo_receivevisa_balance;?>" READONLY/-->
