...
<script type="text/javascript">
	...
	function EditTrackLog(id){
		...
		window.open(url, 'Edit Tracking Log', 'height=400, width=480, toolbar=no, directories=no, status=no, location=no, menubar=no, scrollbars=yes')
	}
...
?>
  ...
  <table width="100%" border="0">
    ...
    <table width="100%" border="0">
    	...
    	<tr>
          <td>Receive by Corporate Services:</td>
    	  <td><input type="hidden" name="currentuser2" value="<?php echo $this->session->userdata('username');?>" />
              <!--for javascript to get currentuser-->
              <input type="checkbox" name="chkisreceive2" value="1" onclick="checktrigger(this, txtreceiveby, dtreceive);" <?php if($woRow->wo_isreceive == '1') echo "CHECKED DISABLED";?>/>
              <input type="text" name="txtreceiveby2" id="txtreceiveby4" value="<?php echo $woRow->wo_receiveby;?>" readonly="READONLY"/>
          </td>
    	  <td><input type="text" name="dtreceive2" size="12" id="dtreceive4"  value="<?php if(strlen($woRow->wo_receivedate) > 0 && $woRow->wo_receivedate != '0000-00-00') echo date('d-m-Y', strtotime($woRow->wo_receivedate));?>" readonly="READONLY"/>
              <input id="button21" name="button5" type="button" class="scwdatepicker" onclick="scwShow(document.getElementById('dtreceive'), this)" />
          </td>
    	  <td></td>
    	  <td><!--input type="text" name="txtreceivecomment" value="<?php echo $woRow->wo_receive_comment;?>"/--></td>
  	  </tr>
    	<tr>
    		<td rowspan="3">Process by Corporate Services:</td>
	  ...
	  <td><input type="checkbox" name="chkisjimack" onclick="checktrigger(this, txtjimack, dtjimack);" <?php if($woRow->wo_isjim_ack == '1') echo "CHECKED DISABLED";?> <?php echo $strprocess ?>/>
    		...
    		<td>MyIMMS Batch No: &nbsp; <input type="Text" name="txtjimackref"  value="<?php if($woRow->wo_isreplacement == '0') echo $woRow->wo_jimack_refno; else echo $woRow->replacement_desc;?>"/></td>
    	...
    	<tr>
    	  ...
    	  <td><?php echo anchor_popup("contSpim/printVDRApp/". $woRow->wo_id,"Print VDR Application");?>
    	    <p><?php echo anchor_popup("contSpim/printWPList/". $woRow->wo_id,"Print Surat Wakil ");?></p>
    	  <p><?php echo anchor_popup("contSpim/printHOVDR/". $woRow->wo_id,"Print Surat Serahan VDR ");?></p>
...
<input type="text" name="reasonwithd" size="60" value="<?php echo $woRow->wo_withd_reason;?>"/></td>
  ...
  <table width="100%" border="0">
  	...
  	<tr>
  		...
  		<th width="9%">Attend By</th>
  		<th width="23%">Remarks</th>
  		<th width="23%">Action</th>
  		<th width="7%">Action By</th>
	...
	<tr>
		...
		<td width="8%"><?php if($log->track_compdate!= "0000-00-00") echo date('d-m-Y', strtotime($log->track_compdate));?></td>
	    <td width="8%"><input type="button" value="Edit" onclick="EditTrackLog('<?php echo $log->track_id;?>')" <?php echo $close ?>/></td>
	</tr>
	<?php }
	}
	?>
