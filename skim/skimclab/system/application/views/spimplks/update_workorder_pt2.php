...
<script type="text/javascript">
	...
	function openuploadwindow(){
		...
		var url = "<?php echo site_url('contSpimFirst/uploadDocView');?>/" + workorderid + "/" + companyname
	...
	function newphtracklog(){
		...
		var companyname = document.updateWorkorderForm.txtclabno.value
		var url = "<?php echo site_url('contSpimFirst/newPhoneTrackLog');?>/" + workorderid + "/" + companyname
	...
	function registerNewLabor(){
		...
		var url = "<?php echo site_url('contSpimFirst/regnewlabour');?>/" + workorderid + "/" + companyname
	...
	function EditFCL(id){
		...
		var url = "<?php echo site_url('contSpimFirst/EditFCL');?>/" + workorderid + "/" + companyname + "/" + id
	...
	function EditTrackLog(id){
		...
		var url = "<?php echo site_url('contSpimFirst/editPhoneTrackLog');?>/" + workorderid + "/" + companyname + "/" + id
    ...
    function openBatch(clabid){
		...
		var url = "<?php echo site_url('contSpimFirst/batchlistwo');?>/" + clabno + "/" + encodeURIComponent(companyname)
	...
	function OpenLampiran(wono){
		var wono = wono;
		var url = "<?php echo site_url('contSpimFirst/lampiran_a');?>/" + wono
	...
	function OpenSecondSchedule(wono){
		var wono = wono;
		var url = "<?php echo site_url('contSpimFirst/second_schedule');?>/" + wono
	...
	function OpenIM12(wono){
		var wono = wono;
		var url = "<?php echo site_url('contSpimFirst/im12');?>/" + wono
	...
	function OpenAddendum(wono){
		var wono = wono;
		var url = "<?php echo site_url('contSpimFirst/addendum');?>/" + wono
	...
	function OpenBorangBayaran(wono){
		var wono = wono;
		var url = "<?php echo site_url('contSpimFirst/borangbayaran');?>/" + wono
	...
	function openack(wono){
		var wono = wono;
		var url = "<?php echo site_url('contSpimFirst/acknowledge');?>/" + wono
	...
	function handover(wono){
	   ...
	   var url = "<?php echo site_url('contSpimFirst/handoverletter');?>/" + wono + "/" + workorderid
	...
	function pengesahan(wono){
       var workorderid = document.updateWorkorderForm.workorderno.value
	   var wono = document.updateWorkorderForm.txtwoid.value
	   var url = "<?php echo site_url('contSpimFirst/confirmletter');?>/" + wono + "/" + workorderid
	...
	function openprintform(){
	   window.open('<?php echo site_url('contSpimFirst/openprintform');?>','Print Form','height=500, width=800, toolbar=no, directories=no, status=no, location=no, menubar=no, scrollbars=yes');
	}
...
window.open(url, 'Payment list', 'height=350,width=700, toolbar=no, directories=no, status=no, location=no, menubar=no, scrollbars=yes') 
	...
	function OpenLampiranExcel(wono){
		var wono = wono;
		var url = "<?php echo site_url('contSpimFirst/lampiran_a_excel');?>/" + wono
...
?>
<h4><a href="<?php echo site_url();?>/contSpimFirst/spimDashbrd">SPIM (1st Year Renewal)</a> <img src="<?php echo base_url();?>/images/right_arrow.gif" width="13" height="14" />Update Work Order </h4>
...
?>
  <form action="<?php echo site_url();?>/contSpimFirst/updateWorkOrderPt2Submit" method="POST" name="updateWorkorderForm" id="updateWorkorderForm"  onsubmit="return v.exec();">
 ...
 <br />
       <table width="100%" border="0">
		...
		<tr>
    		<td colspan="6">OFFICIAL RECEIPT NO<font color="red">*</font>.: <input type="text" name="payrefno" value="<?php echo $woRow->pay_refno;?>" size="60" /><input type="button" value="..." onclick="paymentlist('<?php echo $woRow->wo_clab_no;?>')"/>
    		  <select name="selSP">
                <option value="0">Select SP</option>
                <option value="1"<?php if($woRow->wo_spcount == '1') echo "selected"; ?>>SP 1st</option>
                <option value="2"<?php if($woRow->wo_spcount == '2') echo "selected"; ?>>SP 2nd</option>
              </select></td>
    	</tr>
		<tr>
		   <td colspan="6">IMIGRATION RECEIPT NO
		     :
		     <i>
		     <input name="txtPersonInCharge2" type="text" value="<?php echo $woRow->wo_jimreceipt;?>" />
		     </i>
		     <input type="checkbox" name="chkimigration" value="1" <?php if($woRow->pay_imigration == '1') echo "checked";?>/></td>
		</tr>
...
<br />
      <table width="100%" border="0">
      	<tr>
      		...
      		<th align="RIGHT" colspan="6"><input type="button" name="btnRegisNewLabour" value="Register New Labour" onclick="registerNewLabor();" <?php echo $close ?>/><!--input type="button" name="btnUploadFCL" value="Upload FCL list" class="smoothbtn1" onclick="openuploadwindow();" /--></th>
      	</tr>
      	<?php if($allFCLFiles->num_rows() == 0){ ?>
      	<tr>
      		<td colspan="11"><font color="red"><strong>No FCL lists has been insert for this workorder.</strong></font></td>
      	</tr>
      	<?php }
      	else{
	    ?>
	    ...
	    <?php $i = 1;
		   ...
		   <td><?php echo $fcl->fcl_next_of_kin;?></td>
			...
			<td><?php echo $fcl->fcl_entrydate;?></td>
			<td><?php echo $fcl->mst_wo_type_desc;?></td>
            <td align="center">
         ...
         <tr>
            <td colspan="11">
            <input type="button" value="Data" onclick="OpenLampiran('<?php echo $woRow->wo_id; ?>')" <?php if($allFCLFiles->num_rows() == 0){ echo "disabled"; }?> />
        ...
        <!--input type="button" value="Borang Bayaran" onclick="OpenBorangBayaran('<?php echo $woRow->wo_id; ?>')" <?php if($allFCLFiles->num_rows() == 0){ echo "disabled"; }?> />            </td-->        </tr>
      </table>
<br />
	   <table width="100%" border="0">
      	<tr>
      		<th colspan="3" align="LEFT">Attachment</th>
      		<th align="RIGHT"><!--input type="button" name="btnRegisNewLabour" value="Register New Labour" onclick="registerNewLabor();"/--><input type="button" name="btnUploadFCL" value="Upload Attachment"  onclick="openuploadwindow();" /></th>
      	</tr>
      	<?php if($allFCLUpload->num_rows() == 0){ ?>
      	<tr>
      		<td colspan="4"><font color="red"><strong>No attachment has been uploaded for this workorder.</strong></font></td>
      	</tr>
      	<?php }
      	else{
	    ?>
	    ...
	    <?php $i = 1;
      	 ...
      	 <tr>
      	 	<td><?php echo $i++;?></td>
      	 	<td><a href="<?php echo base_url();?>uploads/spimplks/<?php echo $file->upload_destfilename;?>"><?php echo $file->upload_destfilename;?></a></td>
    ...
    <table width="100%" border="0">
    	...
    	<tr>
    		...
    		<td>
	    		<table width="100%">
	    			<tr><!--td align="center"><?php echo anchor_popup("contSpimFirst/printCheckLits/". $woRow->wo_id,"Checklist");?></td--></tr>
	    			<tr><td align="center"><!--input type="button" value="Insurance Purchase" onclick="Inspchs()" /--></td></tr>
					<tr><td align="center"><?php echo anchor_popup("contSpimFirst/printPLKSIP/". $woRow->wo_id ."/".$woRow->wo_num,"PLKS in Progress");?>
				  <p><?php echo anchor_popup("contSpimFirst/printSPList/". $woRow->wo_id,"Print SP Application Letter");?></p></td></tr>
	    		</table>    		</td>
		...
		<tr>
			...
			<td align="center"><?php echo anchor_popup("contSpimFirst/printHOVDR/". $woRow->wo_id,"Print Hand Over Acknowledgement ");?></td>
		  <td>&nbsp;</td>
		</tr>
		<tr>
		    <td>&nbsp;</td>
    	...
    	<tr>
    	  ...
    	  <td colspan="4"><input type="checkbox" name="iswithd" <?php if($woRow->wo_iswithdrawal == '1') echo "CHECKED DISABLED";?> <?php echo $strprocess ?>/>
Date:
...
<input type="text" name="reasonwithd" size="60" value="<?php echo $woRow->wo_withd_reason;?>"/></td>
    ...
    <table width="100%" border="0">
    	<tr>
    		<td align="center">
				...
				<input type="button" name="btnCancel" value=" Back " onclick="location.href='<?php echo site_url();?>/contSpimFirst/updateWorkorder'" />    		
    		</td>
    	</tr>
    </table>
  </form>
  ...
  <table width="100%" border="0">
  	<tr>
  		...
  		<th colspan="2" align="RIGHT"><input type="button" name="btnNewLog" value=" New Log " onclick="newphtracklog();"  <?php if($woRow->wo_latest_progress == "closed") echo "DISABLED";?>/>
  			<input type="button" name="btnRefresh" value=" Refresh Log " onclick="location.href='<?php echo site_url();?>/contSpimFirst/updateWorkOrderPt2/<?php echo $woRow->wo_id;?>/updatephonetrack'"  <?php if($woRow->wo_latest_progress == "closed") echo "DISABLED";?>/>  		</th>
  	</tr>
  	<tr>
  	    <th width="5%">No.</th>
	...
	<tr>
		...
		<td width="23%"><?php if($log->track_compdate!= "0000-00-00") echo date('d-m-Y', strtotime($log->track_compdate));?></td>
	    <td width="9%"><input type="button" value="Edit" onclick="EditTrackLog('<?php echo $log->track_id;?>')" <?php echo $close ?>/></td>
	</tr>
	<?php }
	}
	?>
