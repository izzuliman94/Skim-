...
<link href="<?php echo base_url();?>js/scwdatepicker/scw.css" rel="stylesheet" type="text/css" />
	...
	function generateReportexcel(type){
	  ...
	  function generateReportview(type){
	    ...
	    var datefrom = document.summary.datefrom.value;
		var dateto = document.summary.dateto.value;
	    var url = "<?php echo site_url('contPayment/summaryview')?>/" + datefrom + "/" + dateto + "/" + region + "/" + ptype + "/" + cancel + "/" + cont;
        window.open(url);
...
<form method="POST" action="" name="summary">
  <table width="80%" border="0">
		...
		<tr>
			<td>Payment Type</td>
			<td>
				<select name="selpayment" onchange="valid(this.value)">
				<option value="0">All</option>
				...
				<label>
					<input name="radcancel" type="radio" id="radio" value="0" checked="checked" />All Recorded Payment
					<input type="radio" name="radcancel" id="radio" value="1" />Cancelled Payment Only
				</label>
			</td>
    </tr>
		<tr>
