...
var url = "<?php echo site_url('contPayment/addlocalworker');?>/" + pmno
window.open(url, 'Add Local Worker', 'height=350,width=700, toolbar=no, directories=no, status=no, location=no, menubar=no, scrollbars=yes') 

}

function editlocalworker(id){

var url = "<?php echo site_url('contPayment/editlocalworker');?>/" + id
window.open(url, 'Edit Local Worker', 'height=350,width=700, toolbar=no, directories=no, status=no, location=no, menubar=no, scrollbars=yes') 
}

function printpayment(){

var pmno = document.frmpayment.txtpmno.value;
var url = "<?php echo site_url('contPayment/displayprintlocal');?>/" + pmno
window.open(url, 'Print Payment', 'height=350,width=700, toolbar=no, directories=no, status=no, location=no, menubar=no, scrollbars=yes') 
}
</script>
</head>
<body>
<?php $accessibility = $this->session->userdata('emp_accessibility'); ?>
<h4><a href="addpayment">Payment Advice</a> <img src="<?php echo base_url();?>/images/right_arrow.gif" width="13" height="14" /> Update Local Worker Payment </h4>
<form name="frmpayment" method="post" action="<?php echo site_url();?>/contPayment/UpdatePaymentLW">
<table  width="100%">
<tr>
...
<table width="100%">
	<tr>
	    <th colspan="3" align="right"><input type="button" onclick="addlocalworker()" value="Add Local Worker Payment" /></th> 
	</tr>
	<tr>
	    <th>No</th>
		<th>No Of Local Worker</th>
	  <th>Action</th>	
	</tr>
		<?php if($allPaymentFCL->num_rows() == 0){ ?>
      	<tr>
      		<td colspan="3" align="center"><font color="red"><strong>No Local Worker lists has been insert for this Payment.</strong></font></td>
      	</tr>
      	<?php }else{ ?>
		 <?php 
		    $i = 1;
		...
		<tr>
		  ...
		  <td align="center"><input type="button" value="Edit / Delete" onclick="editlocalworker('<?php echo $fcl->id; ?>')"/></td>
		</tr>
		 <?php }
  	 	 }
  	 	 ?>
