...
var url = "<?php echo site_url('contPayment/addextfcl');?>/" + pmno
window.open(url, 'Add EXT FCL', 'height=650,width=700, toolbar=no, directories=no, status=no, location=no, menubar=no, scrollbars=yes') 

}

function editextfcl(id){

var url = "<?php echo site_url('contPayment/editextfcl');?>/" + id
window.open(url, 'Edit VDR FCL', 'height=650,width=700, toolbar=no, directories=no, status=no, location=no, menubar=no, scrollbars=yes') 
}
...
window.open(url, 'Print Payment', 'height=650,width=1100, toolbar=no, directories=no, status=no, location=no, menubar=no, scrollbars=yes') 
}
</script>
</head>
<body>
<?php $accessibility = $this->session->userdata('emp_accessibility'); ?>
<h4><a href="addpayment">Payment Advice</a> <img src="<?php echo base_url();?>/images/right_arrow.gif" width="13" height="14" /> Update EXT Payment </h4>
<form name="frmpayment" method="post" action="<?php echo site_url();?>/contPayment/updatepaymentext">
<table  width="100%">
<tr>
...
<table width="100%">
	<tr>
	    <th colspan="5" align="right"><input type="button" onclick="addextfcl()" value="Add FCL Payment" /></th> 
	</tr>
	<tr>
	    <th>No</th>
		<th>No Of FCL</th>
		...
		<tr>
			...
			<td align="center"><input type="button" value="Edit / Delete" onclick="editextfcl('<?php echo $fcl->id; ?>')"/></td>
		</tr>
		 <?php }
  	 	 }
  	 	 ?>
