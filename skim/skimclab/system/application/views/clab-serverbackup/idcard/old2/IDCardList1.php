	...
	function generateReportexcel(type)
      {
	    ...
	    var url = "<?php echo site_url('contIDcard/idlistexcel')?>/" + datefrom + "/" + dateto;
        window.open(url);
	  ...
	  function generateReportview(type)
      {
	    ...
	    var datefrom = document.sumvdr.datefrom.value;
		var dateto = document.sumvdr.dateto.value;
	    var url = "<?php echo site_url('contIDcard/idlistview')?>/" + datefrom + "/" + dateto;
        window.open(url);
...
<body>
<h4><a href="../payment/dashboard.php">Generate Card ID List By Entry Date</a></h4>
