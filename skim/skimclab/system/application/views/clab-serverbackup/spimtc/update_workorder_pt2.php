...
<script type="text/javascript">
	...
	function Inspchs(){
	 ...
	 var url = "<?php echo site_url('contSpimtc/email_item');?>/" + workorderid 
	...
	function handover(wono){
	   ...
	   var url = "<?php echo site_url('contSpimtc/handoverletter');?>/" + wono + "/" + workorderid
	...
	function pengesahan(wono){
       var workorderid = document.updateWorkorderForm.workorderno.value
	   var wono = document.updateWorkorderForm.txtwoid.value
	   var url = "<?php echo site_url('contSpimtc/confirmletter');?>/" + wono + "/" + workorderid
	...
	function openuploadwindow(){
		...
		var url = "<?php echo site_url('contSpimtc/uploadDocView');?>/" + workorderid + "/" + companyname
	...
	function newphtracklog(){
		...
		var companyname = document.updateWorkorderForm.txtclabno.value
		var url = "<?php echo site_url('contSpimtc/newPhoneTrackLog');?>/" + workorderid + "/" + companyname
	...
	function registerNewLabor(){
		...
		var url = "<?php echo site_url('contSpimtc/regnewlabour');?>/" + workorderid + "/" + companyname
		...
		var url = "<?php echo site_url('contSpimtc/regnewlegal');?>/" + workorderid + "/" + companyname + "/" + currentuser + "/" + wono
	...
	function EditFCL(id){
		...
		var url = "<?php echo site_url('contSpimtc/EditFCL');?>/" + workorderid + "/" + companyname + "/" + id
	...
	function EditLegal(id){
		...
		var url = "<?php echo site_url('contSpimtc/EditLegal');?>/" + workorderid + "/" + companyname + "/" + id
    ...
    function openBatch(clabid){
		...
		var url = "<?php echo site_url('contSpimtc/batchlistwo');?>/" + clabno + "/" + companyname
	...
	function OpenLampiran(wono){
		var wono = wono;
		var url = "<?php echo site_url('contSpimtc/lampiran_a');?>/" + wono
		...
		var wono = wono;
		var url = "<?php echo site_url('contSpimtc/lampiran_a_excel');?>/" + wono
		window.open(url, 'Lampiran A', 'height=500, width=800, toolbar=yes, directories=no, status=no, location=no, menubar=no, scrollbars=yes');
	...
	function OpenSecondSchedule(wono){
		var wono = wono;
		var url = "<?php echo site_url('contSpimtc/second_schedule');?>/" + wono
	...
	function OpenIM12(wono){
		var wono = wono;
		var url = "<?php echo site_url('contSpimtc/im12');?>/" + wono
	...
	function OpenAddendum(wono){
		var wono = wono;
		var url = "<?php echo site_url('contSpimtc/addendum');?>/" + wono
	...
	function OpenBorangBayaran(wono){
		var wono = wono;
		var url = "<?php echo site_url('contSpimtc/borangbayaran');?>/" + wono
	...
	function openack(wono){
		var wono = wono;
		var url = "<?php echo site_url('contSpimtc/acknowledge');?>/" + wono
	...
	function openprintform(fclid){
	   var url = "<?php echo site_url('contSpimtc/openprintform');?>/" + fclid
   ...
   function paymentlist(clabno){
	  var frm = "update";
	  var url = "<?php echo site_url('contSpimtc/openreceipt');?>/" + frm + "/" + clabno;
...
?>
<h4><a href="<?php echo site_url();?>/contSpimtc/spimDashbrd">SPIM Transit Centre</a> <img src="<?php echo base_url();?>/images/right_arrow.gif" width="13" height="14" />Update Work Order </h4>
...
?>
  <form action="<?php echo site_url();?>/contSpimtc/updateWorkOrderPt2Submit" method="POST" name="updateWorkorderForm" id="updateWorkorderForm"  onsubmit="return v.exec();">
    ...
    <table width="100%" border="0">
    	...
    	<tr>
    		...
    		<td><input type="checkbox" name="chksignedpay" value="1" <?php if($woRow->doc_signedpayment == '1') echo "CHECKED ";?> <?php echo $doc ?>/>Signed Payment Advice 
    			<input type="text" name="dtsignedpay" id="dtsignedpay" size="11" value="<?php if($woRow->doc_signedpaymentdate != "0000-00-00") echo date('d-m-Y', strtotime($woRow->doc_signedpaymentdate));?>" <?php echo $doctext ?>/>
   ...
   <br />
     <table width="100%" border="0">
      ...
      <table width="100%" border="0">
      	<tr>
      		...
      		<th align="RIGHT" colspan="7"><input type="button" name="btnRegisNewLabour" value="Register New Labour" onclick="registerNewLabor();" <?php echo $close ?> /><!--input type="button" name="btnUploadFCL" value="Upload FCL list" class="smoothbtn1" onclick="openuploadwindow();" /--></th>
      	</tr>
	    ...
	    <?php $i = 1;
           ...
           <td align="center"><input type="checkbox" name="chkconfirm" value="1"/></td>
      	 </tr>
      	 <?php }
  	 	 }
  	 	 ?>
	  ...
	  <br />
	   <table width="100%" border="0">
	    ...
	    <?php $i = 1;
      	 ...
      	 <tr>
      	 	<td><?php echo $i++;?></td>
      	 	<td><a href="<?php echo base_url();?>uploads/spimtc/<?php echo $file->upload_destfilename;?>"><?php echo $file->upload_destfilename;?></a></td>
    ...
    <table width="100%" border="0">
    	...
    	<tr>
          <td rowspan="3">Received by Corporate Services:</td>
    	  <td rowspan="3"><input name="chkisprocess" type="checkbox" id="chkisprocess" onclick="checktrigger(this, txtprocessby, dtprocess);" value="1" <?php if($woRow->wo_isprocess == '1') echo "CHECKED DISABLED";?>/>
              <input type="text" name="txtprocessby" id="txtprocessby"  value="<?php echo $woRow->wo_processby;?>" readonly="READONLY"/>          </td>
    	  <td rowspan="3"><input type="text" name="dtprocess" size="12" id="dtprocess" value="<?php if(strlen($woRow->wo_processdate) > 0 && $woRow->wo_processdate != '0000-00-00') echo date('d-m-Y', strtotime($woRow->wo_processdate));?>" readonly="READONLY"/>
              <input id="button12" name="button5" type="button" class="scwdatepicker" onclick="scwShow(document.getElementById('dtprocess'), this)" />          </td>
    	  <td align="center"><?php echo anchor_popup("contSpimtc/printCheckLits/". $woRow->wo_id,"PrintChecklist");?></td>
    	  <td rowspan="3"><input name="txtprocesscomment" type="text" id="txtprocesscomment" value="<?php echo $woRow->wo_process_comment;?>"/></td>
  	  </tr>
    	<tr>
          <td align="center">&nbsp;</td>
  	  </tr>
    	...
    	<tr>
    		...
    		<td><div align="center">
    		  <input type="button" value="Insurance Purchase" onclick="Inspchs()" />
  		  </div></td>
		...
		<tr>
		    <td>Verification From Corporate Service</td>
		...
		<tr>
		  ...
		  <td><input type="text" name="txtsentlegalcomment"  value="<?php echo $woRow->wo_sentlegalcomment;?>"/></td>
		  <td>&nbsp;</td>
	  </tr>
		<tr>
    	...
    	<tr>
    		...
    		<td>Total Approve: <input type="text" name="txtapprove" size="4" value="<?php echo $woRow->wo_receivevisa_approve;?>" READONLY/> <br />
    			...
    			<input type="button" name="btnRejectHistory" value="Reject History"  class="smoothbtn1" onclick="window.open('<?php echo site_url();?>/contSpimtc/viewRejectHistory/<?php echo $woRow->wo_clab_no ."/". $woRow->wo_id;?>', 'Workorder Reject History', 'height=480, width=420, toolbar=no, directories=no, status=no, location=no, menubar=no, scrollbars=yes')" <?php echo $close ?>/>    		</td>
    	...
    	<tr>
    	  ...
    	  <td align="left"><?php echo anchor_popup("contSpimtc/printHandoverList/". $woRow->wo_id,"Print Handover Letter");?>
   	      <p><?php echo anchor_popup("contSpimtc/printSPList/". $woRow->wo_id,"Print SP Application Letter");?></p>
   	      <p><?php echo anchor_popup("contSpimtc/printCOMList/". $woRow->wo_id,"Print COM Application Letter");?></p>
   	      <p><?php echo anchor_popup("contSpimtc/printHandoverArr/". $woRow->wo_id,"Print Handover Letter (Arrival)");?></p>
          <p><?php echo anchor_popup("contSpimtc/printPLKS1year/". $woRow->wo_id,"Print PLKS First Year Reminder");?></p></td>
    	  <td><?php echo anchor_popup("contSpimtc/printVDRApp/". $woRow->wo_id,"Print VDR Application");?>
    	    <p><?php echo anchor_popup("contSpimtc/printWPList/". $woRow->wo_id,"Print Surat Wakil ");?></p>
    	  <p><?php echo anchor_popup("contSpimtc/printHOVDR/". $woRow->wo_id,"Print Hand Over Acknowledgement ");?></p>
    	  <p><?php echo anchor_popup("contSpimtc/printPLKSIP/". $woRow->wo_id ."/".$woRow->wo_num,"PLKS in Progress");?><span class="style1">New!</span></p></td>
  	  </tr>
    	<tr>
    	  <td>&nbsp;</td>
    ...
    <table width="100%" border="0">
    	<tr>
    		<td align="center">
				...
				<input type="button" name="btnCancel" value=" Back " onclick="location.href='<?php echo site_url();?>/contSpimtc/updateWorkorder'" />    		
    		</td>
    	</tr>
    </table>
  </form>
  ...
  <table width="100%" border="0">
  	<tr>
  		...
  		<th colspan="1" align="RIGHT"><input type="button" name="btnNewLog" value=" New Log " onclick="newphtracklog();"  <?php if($woRow->wo_latest_progress == "closed") echo "DISABLED";?> <?php echo $close ?>/>
  			<input type="button" name="btnRefresh" value=" Refresh Log " onclick="location.href='<?php echo site_url();?>/contSpimtc/updateWorkOrderPt2/<?php echo $woRow->wo_id;?>/updatephonetrack'"  <?php if($woRow->wo_latest_progress == "closed") echo "DISABLED";?> <?php echo $close ?>/>
  		</th>
  	</tr>
  	<tr>
  	    <th>No.</th>
