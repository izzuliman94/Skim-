<?php
	class ContRenewal extends Controller{
		function ContRenewal(){
			parent::Controller();
			$this->load->library('session');
			$this->load->library('rapyd');	
			$this->load->Model('ModelRenewal');	
		...
		function openreceipt(){
		   ...
		   $data['receiptlist'] = $this->ModelRenewal->getAllReceipt();
		   $data['form'] = $frm;
		...
		function empfcl_list(){
			...
			$searchdata = $_POST["txtsearch"];
		   	...
		   	$data['labour'] = $this->ModelRenewal->getAllLabor($searchdata);
		...
		$sql = "insert into wo_fcl_renew
		        (fcl_woid,fcl_passno,fcl_passissue,fcl_passexp,fcl_salary,fcl_wt,fcl_next_of_kin,fcl_relationship)
                values
                ('$workorderid','$passno','$passissue','$passexp','$salary','$wt','$nok','$relationship')"; 
        $this->db->trans_begin();
		$this->db->query($sql);	
        $this->db->trans_commit();
	...
	/*	function form_new_labour(){
		...
		function newWorkOrder(){
			$this->load->view('renewal/new_workorder_pt1');	
		}
		...
		function newWOSearchCtr(){
			...
			$data['ctrRecord'] = $this->ModelRenewal->searchCtr($searchby, $keyword);
			
			$this->load->view('renewal/new_workorder_pt1', $data);
		}
		...
		function newWorkOrderPt2(){
			...
			if(strlen($currentuserid) > 0){
				...
				$ctrRecord = $this->ModelRenewal->getCtrByClabNo($clabno);
				$data['ctrRow'] = $ctrRecord->row();
				$data['allCountries'] = $this->ModelRenewal->getAllCountries();
				$data['allAgencies'] = $this->ModelRenewal->getAllAgencies();
				
				$this->load->view('renewal/new_workorder_pt2', $data);
			}
			else{
				$this->load->view('session_expired.php');	
			}
		...
		function newWorkOrderPt2Submit(){
			$currentuserid = $this->session->userdata('emp_id');
			if(strlen($currentuserid) > 0){
				$wo_id = $this->ModelRenewal->getNextWONumber();
				...
				//PAYMENT
				$wo_pay_imi = $_POST['chkimigration'];
				$wo_pay_clb = $_POST['chkclab'];
				$wo_pay_insg = $_POST['chkins_guarante'];
				$wo_pay_insh = $_POST['chkins_hospital'];
				...
				/*$woQuery = "INSERT INTO workorders (wo_id, wo_num, wo_date, wo_clab_no, wo_fcl_total, wo_fcl_country, wo_agency, wo_isreplacement, wo_replace_date, wo_replace_reason, 
							...
							*/
				$woQuery = "INSERT INTO workorders(wo_id, wo_num, wo_date, wo_clab_no, wo_fcl_total, wo_fcl_country,wo_iswithdrawal, wo_withd_date, wo_withd_reason, wo_personincharge, wo_datesubmit, wo_keyin_by, wo_keyin_date,wo_yearrenew,wo_spim_status,wo_jtk,wo_jtk_ref,wo_sender_name,wo_sender_ic,wo_sender_contact) VALUES ($wo_id, '$workorderno', '$wo_date', '$wo_clab_no',$wo_fcl_total, '$wo_fcl_country','$wo_iswithdrawal', '$wo_withd_date', '$wo_withd_reason', '$wo_personincharge', '$wo_datesubmit', '$wo_keyin_by', '$wo_keyin_date','$wo_yearrenew','R','$wo_jtk_check','$wo_jtk_ref','$wo_sendername','$wo_senderic','$wo_senderctc')";
				...
				$paymentQuery = "INSERT INTO wo_payment (pay_woid, pay_refno, pay_adminfee, pay_levy, pay_plks, pay_agencyfee, 
				                 pay_fomema, pay_visa, pay_ig, pay_fwcs, pay_fwhs,pay_imigration,pay_clab,
								 pay_ins_guarante,pay_ins_hospital) 
	            			     VALUES ($wo_id, '$wo_pay_refno', '$wo_pay_adminfee', '$wo_pay_levy', '$wo_pay_plks', 
								 '$wo_pay_agencyfee', '$wo_pay_fomema', '$wo_pay_visa', '$wo_pay_ig', '$wo_pay_fwcs', 
								 '$wo_pay_fwhs','$wo_pay_imi','$wo_pay_clb','$wo_pay_insg','$wo_pay_insh')";	        
				...
				redirect('contRenewal/updateWorkOrderPt2/'.$wo_id .'/add');
			}
			else{
				$this->load->view('session_expired.php');	
			}
		...
		function updateWorkOrderPt2(){
			...
			if(strlen($currentuserid) > 0){
				...
				$this->load->view('renewal/update_workorder_pt2', $data);
			}
			else{
				$this->load->view('session_expired.php');	
			}
		...
		function updateWorkOrderPt2Submit(){
			...
			if(strlen($currentuserid) > 0){
				...
				$woQuery = "UPDATE workorders SET wo_fcl_total = $wo_fcl_total, wo_fcl_country = '$wo_fcl_country', wo_isreplacement = '$wo_isreplacement',     
				            wo_iswithdrawal = '$wo_iswithdrawal', wo_withd_date = '$wo_withd_date', wo_withd_reason = '$wo_withd_reason', 
							wo_personincharge = '$wo_personincharge', wo_modifiedby = '$modifiedby', wo_modifieddate = NOW(),
							wo_yearrenew = '$wo_yearrenew',wo_jtk = '$wo_jtk_check',wo_jtk_ref = '$wo_jtk_ref',wo_sender_name = '$wo_sendername', wo_sender_ic = 
							'$wo_senderic',wo_sender_contact='$wo_senderctc' WHERE wo_id = $wo_id";
	        ...
	        /*    $paymentQuery = "UPDATE wo_payment SET pay_refno = '$wo_pay_refno', pay_adminfee = '$wo_pay_adminfee', pay_levy = '$wo_pay_levy', pay_plks = '$wo_pay_plks', 
				...
				$paymentQuery = "UPDATE wo_payment set pay_refno = '$wo_pay_refno', pay_adminfee = '$wo_pay_adminfee', 
				                 pay_levy = '$wo_pay_levy', pay_plks = '$wo_pay_plks',pay_agencyfee = '$wo_pay_agencyfee', 
								 pay_fomema = '$wo_pay_fomema', pay_visa = '$wo_pay_visa', pay_ig = '$wo_pay_ig', pay_fwcs = 
								 '$wo_pay_fwcs', pay_fwhs = '$wo_pay_fwhs',pay_imigration = '$wo_pay_imi',pay_clab = '$wo_pay_clb',
				...
				//echo $legalQuery;
				//echo $statusQuery;
				redirect('contRenewal/updateWorkOrderPt2/'.$wo_id .'/update');
			}
			else{
				$this->load->view('session_expired.php');	
			}
		...
		function listAllWorkorders(){
			...
			$this->load->view('renewal/workorder_lists', $data);	
		}
