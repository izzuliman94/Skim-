...
<div id="switchsection1" class="switchgroup1"> 
 <?php if(isset($successMsg)) echo "<font color=\"red\">" . $successMsg . "</font>";
       if(isset($clabno)) $clabno = $clabno;
  ?>
  <br />
  <?php echo "Contractor Name: " . $ctr_compname;?>
  <br />
  <?php echo "CLAB No.: " . $clabno;?>
  
  <form action="<?php echo site_url();?>/contContractor/uploadDoc" method="POST" name="uploadForm" id="uploadForm">
  	<table width="80%" border="0">
      	<tr>
      		<td>File Type:</td>
    		<td><input type="radio" name="filetype" value="form24" CHECKED/>Form 24
    			<input type="radio" name="filetype" value="form49" />Form 49
    			<input type="radio" name="filetype" value="copyofCIDBcert" />Copy of CIDB certificate <br />
    			<input type="radio" name="filetype" value="others" />Others
    			<input type="text" name="otherFileName" size="30" />
