<?php
	class ContrekabContractor extends Controller {
		function ContrekabContractor(){
			parent::Controller();
			$this->load->database();
		...
		$data['process'] = $this->uri->segment(3);
		$data['agent'] = $this->ModelrekabContractor->getAllAgent($searchdata);
		$this->load->view('rekabcontractor/agent_list', $data);
		
		}
		
		function registrationForm(){
			$this->load->view('rekabcontractor/registration_pt1.php');
		}
		...
		function checkCompanyNumber(){
			...
			if(strlen($currentuserid) > 0){
				...
				$companyExists = $this->ModelrekabContractor->companyExists($companyno);
				if($companyExists->num_rows() > 0)
				{
					...
					$this->load->view('rekabcontractor/registration_pt1.php', $data);
				}
				else{
					$data['allStates'] = $this->ModelrekabContractor->getAllStates();
					$data['allGrades'] = $this->ModelrekabContractor->getAllCtrGrades();
					$data['allCategories'] = $this->ModelrekabContractor->getAllCtrCategories();
					$data['allSpec'] = $this->ModelrekabContractor->getAllCtrSpec();
				
					$data['companyno'] = $companyno;
					$this->load->view('rekabcontractor/registration_pt2.php', $data);
				}
			}
			else{
		...
		function registrationFormSubmit(){
			$clabno = $this->ModelrekabContractor->generateClabNo();
			...
			$companygstno = trim($_POST['companygstno']);	
			$companysocsono = trim($_POST['companysocsono']);
			$companyregno = str_replace(" ", "", $companyregno);
			$agentid = $_POST["txtagid"];
			...
			if(isset($_POST['formD'])) $formD = $_POST['formD'];	
			else $formD = '0';
			...
			$ctr_status = '1'; //rekabcontractor status = active
			
			$insertQuery = "INSERT INTO rekab_contractors (ctr_clab_no,ctr_gst_no,ctr_socso_no, ctr_datereg, ctr_validdate, ctr_comp_name, ctr_comp_regno, ctr_addr1, ctr_addr2, ctr_addr3, ctr_pcode, ctr_state, ctr_telno, 
							...
							ctr_attach_form24, ctr_attach_form49,ctr_attach_formD, ctr_attach_copycidb, ctr_attach_iccopy, ctr_attach_others, ctr_attach_specify, 
							ctr_dir_name, ctr_dir_mobileno, ctr_contact_name, ctr_contact_desg, ctr_contact_mobileno, ctr_status, ctr_clabexp_date,agent_id) 
							VALUES ('$clabno','$companygstno','$companysocsono', NOW(), NOW(), " . $this->db->escape($companyname) .",'$companyregno', ". $this->db->escape($address1) .", ". $this->db->escape($address2).", 
							...
							'$form24', '$form49','$formD', '$cidb_certcopy', '$ic_copy', '$others', '$attach_others', 
							".$this->db->escape($director).", '$directorMobile', ".$this->db->escape($contactperson).", ".$this->db->escape($contactDesig).", '$contactMobile', '$ctr_status', '$clabExpDate','$agentid');";
			
			/**********START TRANSACTION**********/
			$this->db->trans_begin();
			$this->db->query($insertQuery);
			if($periodreg != '' && $amount > 0){
			$paymentid = $this->ModelrekabContractor->getNextPaymentID();
			...
			redirect('contrekabContractor/editContractor/'. $clabno .'/add');
		}
		...
		function regisAttachDoc(){
			...
			if(strlen($currentuserid) > 0){
				...
				$data['clabno'] = $clab_no;
				$data['ctr_compname'] = $this->ModelrekabContractor->getCtrNameByClabNo($clab_no);
				$data['uploadHistory'] = $this->ModelrekabContractor->getUploadHistory($clab_no);
				
				$data['accessibility'] = $this->session->userdata('emp_accessibility');
				$this->load->view('rekabcontractor/registration_attachdoc', $data);	
			}
			else{
		...
		function uploadDocSubmit(){
			...
			if(strlen($currentuserid) > 0){
				...
				else if($filetype == 'formD') $fileTypeName = "Form D";
				else if($filetype == 'iccopy') $fileTypeName = "IC Copy";
				else if($filetype == 'agreement') $fileTypeName = "Supplier Agreement";
				...
				else
				{
					...
					$attachment_id = $this->ModelrekabContractor->getNextAttachmentID();
				...
				$data['uploadHistory'] = $this->ModelrekabContractor->getUploadHistory($clabno);
				$data['accessibility'] = $this->session->userdata('emp_accessibility');
				$this->load->view('rekabcontractor/registration_attachdoc', $data);	
			}
			else{
		...
		function ctrDashboard(){
			$data['overallCtr'] = $this->ModelrekabContractor->getTotalContractors();
			$data['openCtr'] = $this->ModelrekabContractor->getCtrByAppStatus(1, 'total');
			$data['verifiedCtr'] = $this->ModelrekabContractor->getCtrByAppStatus(2, 'total');
			$data['approvedCtr'] = $this->ModelrekabContractor->getCtrByAppStatus(3, 'total');
			$data['rejectedCtr'] = $this->ModelrekabContractor->getCtrByAppStatus(4, 'total');
			$data['withdrawCtr'] = $this->ModelrekabContractor->getCtrByAppStatus(5, 'total');
			$data['activeCtr'] = $this->ModelrekabContractor->getCtrByStatus(1, 'total');
			$data['inactiveCtr'] = $this->ModelrekabContractor->getCtrByStatus(2, 'total');
			$data['suspendedCtr'] = $this->ModelrekabContractor->getCtrByStatus(3, 'total');
			$data['registerOnlineCtr'] = $this->ModelrekabContractor->getCtrRegisterOnline('', 0, 'total');
			
			$data['clabexpiry1mth'] = $this->ModelrekabContractor->getCtrByClabExpiry('ctr_clabexp_date', 1, 'total');
			$data['clabexpiry2mth'] = $this->ModelrekabContractor->getCtrByClabExpiry('ctr_clabexp_date', 2, 'total');
			$data['clabexpiry3mth'] = $this->ModelrekabContractor->getCtrByClabExpiry('ctr_clabexp_date', 3, 'total');
			$data['clabexpiryexpired'] = $this->ModelrekabContractor->getCtrByClabExpiry('ctr_clabexp_date', -1, 'total');
			
			$data['cidbexpiry1mth'] = $this->ModelrekabContractor->getCtrByClabExpiry('ctr_cidbexp_date', 1, 'total');
			$data['cidbexpiry2mth'] = $this->ModelrekabContractor->getCtrByClabExpiry('ctr_cidbexp_date', 2, 'total');
			$data['cidbexpiry3mth'] = $this->ModelrekabContractor->getCtrByClabExpiry('ctr_cidbexp_date', 3, 'total');
			$data['cidbexpiryexpired'] = $this->ModelrekabContractor->getCtrByClabExpiry('ctr_cidbexp_date', -1, 'total');
			...
			$data['plksexp1mth'] = $this->ModelrekabLabour->getLaborByPermitPassExpiryOld('wkr_permitexp', -1, 'total', 'allcountry');
			$data['plksexp2mth'] = $this->ModelrekabLabour->getLaborByPermitPassExpiryOld('wkr_permitexp', -2, 'total', 'allcountry');
			$data['plksexp3mth'] = $this->ModelrekabLabour->getLaborByPermitPassExpiryOld('wkr_permitexp', -3, 'total', 'allcountry');
			...
			$this->load->view('rekabcontractor/dashboard', $data);
		}
		...
		function ctrListByClabExpiry(){
			...
			$data['contractorList'] =  $this->ModelrekabContractor->getCtrByClabExpiry($fieldType, $noOfMonths, 'list');
			
			$this->load->view('rekabcontractor/ctr_listby_clabexpiry', $data);
		}
		...
		function listByExpiryAndCompany(){
			...
			$success = $this->ModelrekabContractor->updatePrintLetterHistory($noOfMonths, $fieldName);
			...
			$data['listByCompany'] = $this->ModelrekabContractor->listExpiryByCompany($fieldName, $noOfMonths, 'companylist', '');
			
			$data['total'] = $total;
			$data['fieldName'] = $fieldName;
			$data['noOfMonths'] = $noOfMonths;
			
			$this->load->view('rekabcontractor/ctr_listby_plksexpiry', $data);
		}
		...
		function listByExpiryAndCompany2(){
			...
			$totalCompanies = $this->ModelrekabContractor->listExpiryByCompany2($fieldName, $noOfMonths, 'total', 0, 0);
			...
			$data['listByCompany'] = $this->ModelrekabContractor->listExpiryByCompany2($fieldName, $noOfMonths, 'companylist', $config['per_page'], $offset);
			
			$data['fieldName'] = $fieldName;
			$data['noOfMonths'] = $noOfMonths;
			$data['offset'] = $offset;
			$this->load->view('rekabcontractor/ctr_listby_plksexpiry2', $data);
		}
		...
		function listByExpiryAndCompany3(){
			...
			$data['listByCompany'] = $this->ModelrekabContractor->listExpiryByCompany3($fieldName, $noOfMonths, 'companylist', '');
			
			$data['total'] = $total;
			$data['fieldName'] = $fieldName;
			$data['noOfMonths'] = $noOfMonths;
			
			$this->load->view('rekabcontractor/ctr_listby_plksexpiry3', $data);
		}
		...
		function listbydate(){
			...
			$data['listByCompany'] = $this->ModelrekabContractor->listExpiryByCompanyDate($fieldName, $fromDate, $toDate ,'companylist', '');
			...
			$this->load->view('rekabcontractor/ctr_listby_plksexpiry4', $data);
		}
				
				
		function listByExpiryAndCompanyOld(){
			$fieldName = $this->uri->segment(3);
			$noOfMonths = $this->uri->segment(4);
			$total = $this->uri->segment(5);
			
			print_r ($fieldname,1);
			var_dump ($noofMonths,1);
			echo $total;
			...
			$data['listByCompany'] = $this->ModelrekabContractor->listExpiryByCompanyOld($fieldName, $noOfMonths, 'companylist', '');
		...
		function ctrList(){
		    ...
		    //grid
		    $grid = new DataGrid("", "rekab_contractors");
		    $grid->db->join("mst_ctr_appstatus","rekab_contractors.ctr_appstatus=mst_ctr_appstatus.appstatus_id","LEFT");
		    $grid->db->join("mst_emp_status","rekab_contractors.ctr_status=mst_emp_status.emp_statusid","LEFT");
		    ...
		    /**********end grid****************/
			$this->load->view('rekabcontractor/contractor_lists', $data);	
		}
		
		function ctrPrintHistoryList(){
			/********Grid for labor list*******/
			$css_src = $this->rapyd->get_elements_path("css/sippsstyle.css","css");
			$this->rapyd->load("datafilter","datagrid");
		    $this->rapyd->uri->keep_persistence();
		    ...
		    //grid
		    $grid = new DataGrid("", "ctr_printletter_history");
		    $grid->db->join("contractors","contractors.ctr_clab_no=ctr_printletter_history.print_ctr_clabno","LEFT");
		    $grid->db->join("employees","employees.emp_id=ctr_printletter_history.print_by","LEFT");
		    $grid->db->order_by("print_date", "DESC");
			...
			//$grid->column("CLAB NO","$link_edit", 'width="80"');
			$grid->column("PRINT ID","print_id",'width="60"');
			$grid->column("COMPANY","ctr_comp_name");
			$grid->column("PRINT DATE","<callback_displaydate><#print_date#></callback_displaydate>");
			$grid->column("PRINT BY","emp_name");
			$grid->column("DOC TYPE","print_doctype");
			$grid->column("DOC DUR","print_docduration");
		    ...
		    $data["crud"] = $grid->output;
		    /**********end grid****************/
			$this->load->view('rekabcontractor/ctr_printhistory_lists', $data);	
		}
		...
		function listContractorByStatus(){
			...
			if($statusType == 'ctr_appstatus'){
				$data['appStatus'] = $this->ModelrekabContractor->getAppStatusByStatusID($appStatusID);
				$data['contractorList'] = $this->ModelrekabContractor->getCtrByAppStatus($appStatusID, 'list');
			}
			else if($statusType == 'ctr_status'){ //ctr_status
				$data['appStatus'] = $this->ModelrekabContractor->getStatusByStatusID($appStatusID);
				$data['contractorList'] = $this->ModelrekabContractor->getCtrByStatus($appStatusID, 'list');
			}
			else if($statusType == 'approval'){
				$data['appStatus'] = "";
				$data['titleString'] = "require approval";
				$data['contractorList'] = $this->ModelrekabContractor->getCtrForApproval('list');
			}
			else{
				$data['appStatus'] = "REGISTERED ONLINE";
				$data['contractorList'] = $this->ModelrekabContractor->getCtrRegisterOnline($statusType, $appStatusID, 'list');
			}
			$this->load->view('rekabcontractor/ctr_listby_status', $data);
		}
		
		function editContractor(){
			$ctr_clab_no = $this->uri->segment(3);
			$successMsg = $this->uri->segment(4);
			$data['ctr'] = $this->ModelrekabContractor->getCtrByClabNo($ctr_clab_no);
			...
			$this->load->view('rekabcontractor/ctr_edit', $data);	
		}
		...
		function editCtrSubmit(){
			...
			$category = trim($_POST['txtcategory']);	
			$specialization = trim($_POST['txtspec']);	
			$clabvalidate = date('Y-m-d', strtotime($_POST['clab_validdate']));
			...
			if(isset($_POST['isapproved'])){
				...
				if($registrationType == '2') $newClabNo = $this->ModelrekabContractor->generateClabNo();
			}
			...
			$updateCtrQuery = "UPDATE rekab_contractors SET
							   ctr_comp_name = " . $this->db->escape($companyname) .",
			                   ctr_comp_regno = '$companyregno', 
							   ctr_gst_no = '$companygstno',
			                   ctr_socso_no = '$companysocsono',
			...
			redirect('contrekabContractor/editContractor/' . $ctr_clab_no . '/update');
		}
		
		function assignLabourCompSearch(){
			$this->load->view('rekabcontractor/assign_labor_pt1');	
		}
		...
		function assignLabourCompSearchResult(){
			...
			$data['ctrRecord'] = $this->ModelrekabContractor->searchContractors($searchby, $keyword,0,0);
			
			$this->load->view('rekabcontractor/assign_labor_pt1', $data);
		}
		...
		function assignLabourPt2(){
			...
			if(strlen($currentuserid) > 0){
				...
				$ctr = $this->ModelrekabContractor->getCtrByClabNo($clab_no);
				$data['ctr_comp_name'] = $ctr->ctr_comp_name;
				$data['ctr_clab_no'] = $ctr->ctr_clab_no;
				
				$this->load->view('rekabcontractor/assign_labor_pt2', $data);
			}
			else{
				$this->load->view('session_expired.php');	
			}
		...
		function searchPrintLetterHistory(){
			if(isset($_POST['searchby'])){
				$searchby = $_POST['searchby'];
				$keyword = trim($_POST['keyword']); 
		...
		function printApproveRejLetter(){
			...
			$ctr = $this->ModelrekabContractor->getCtrByClabNo($clabno);
			$data['ctr'] = $ctr;
			...
			if($ctr->ctr_approve == "1"){
				...
				$this->load->view('rekabcontractor/ctr_print_approve', $data);	
			}
			else if($ctr->ctr_reject == "1"){
				$this->load->view('rekabcontractor/ctr_print_reject', $data);
			}
			else{
				$data['errorMsg'] = "This application is still under processing. Wating for approval.";
				$this->load->view('rekabcontractor/ctr_print_errorpage', $data);
			}
		}
		
		function printOPR1(){
			$clabno = $this->uri->segment(3);
			$data['ctr'] = $this->ModelrekabContractor->getCtrByClabNo($clabno);
			
			$this->load->view('rekabcontractor/ctr_print_opr1', $data);
		}
		
		//renewal reminder letter
		function viewRegisRenewal(){
			$clabno = $this->uri->segment(4);
			$reminderType = $this->uri->segment(5);
			$fieldname = "ctr_clabexp_date";
			...
			$this->load->view('rekabcontractor/ctr_print_regisRenewal', $data);
		}
		...
		function printRegisRenewal(){
			...
			$isAlreadyPrinted = $this->ModelrekabContractor->checkExistingPrintRecord($clabno, $reminderType, $fieldname);
			if($isAlreadyPrinted->num_rows() == 0){
				$nextprintID = $this->ModelrekabContractor->getNextPrintLetterID();
				$currentuserid = $this->session->userdata('emp_id');
		...
		function printRenewal(){
			...
			$periodreg = $ctr->ctr_periodreg;
	if($periodreg == "12") $txtAmount = "<u>Two Hundred Only</u>(<u>RM 200.00</u>) for <u>twelve(12)</u> years.";
	
	else if($periodreg == "11") $txtAmount = "<u>One Hundred Ninety Only</u>(<u>RM 190.00</u>) for <u>eleven(11)</u> years.";
	else if($periodreg == "10") $txtAmount = "<u>One Hundred Seventy Only</u>(<u>RM 170.00</u>) for <u>ten(10)</u> years.";
	else if($periodreg == "9") $txtAmount = "<u>One Hundred Fifty Only</u>(<u>RM 150.00</u>) for <u>nine(9)</u> years.";
	
	else if($periodreg == "8") $txtAmount = "<u>One Hundred Fourty Only</u>(<u>RM 140.00</u>) for <u>eight(8)</u> years.";
	else if($periodreg == "7") $txtAmount = "<u>One Hundred Twenty Only</u>(<u>RM 120.00</u>) for <u>seven(7)</u> years.";
	else if($periodreg == "6") $txtAmount = "<u>One Hundred Only</u>(<u>RM 100.00</u>) for <u>six(6)</u> years.";
	else if($periodreg == "5") $txtAmount = "<u>Ninety Only</u>(<u>RM 90.00</u>) for <u>five(5)</u> years.";
	else if($periodreg == "4") $txtAmount = "<u>Seventy Only</u>(<u>RM 70.00</u>) for <u>four(4)</u> years.";
	else if($periodreg == "3") $txtAmount = "<u>Fifty Only</u>(<u>RM 50.00</u>) for <u>three(3)</u> years.";
	else if($periodreg == "2") $txtAmount = "<u>Forty Only</u>(<u>RM 40.00</u>) for <u>two(2)</u> years.";
	else $txtAmount = "<u>Twenty Only</u>(<u>RM 20.00</u>) for <u>one(1)</u> years.";
				
			$data['txtAmount'] = $txtAmount;
				
			$this->load->view('rekabcontractor/ctr_print_renewal', $data);
		}
		...
		function printCert(){
			...
			$sqlQuery = "UPDATE ctr_payment SET 
			pay_cert_date = NOW(),
			pay_cert_pic = '$pic',
			pay_cert_print = 'Printed'
			WHERE pay_ctr_id = '$clabno'";
			
			$this->db->query($sqlQuery);
			
			$this->load->view('rekabcontractor/ctr_print_certificate', $data);
		}
		...
		function printCert2(){
			...
			$this->load->view('rekabcontractor/ctr_print_certificate2', $data);
		}
		...
		function printVDR(){
			...
			$this->load->view('rekabcontractor/ctr_print_agreement_vdr', $data);
		}
		...
		function printExt(){
			...
			$this->load->view('rekabcontractor/ctr_print_agreement_ext', $data);
		}
		...
		function printTransferApp(){
			$clabno = $this->uri->segment(3);
			$data['ctr'] = $this->ModelrekabContractor->getCtrByClabNo($clabno);
			$avlabDetails = $this->ModelrekabContractor->getAvlabDetails($clabno);
			if($avlabDetails->num_rows() > 0){
				$avlabRow = $avlabDetails->row();
				$data['avlabRow'] = $avlabRow;
				$data['avlabWkrDetails'] = $this->ModelrekabContractor->getAvlabWkrDetails($avlabRow->avlab_ref_no);
				$this->load->view('rekabcontractor/ctr_print_transfer_app', $data);
			}
			else{
		...
		function printApplicationRekab(){
			$clabno = $this->uri->segment(3);
			$data['ctr'] = $this->ModelrekabContractor->getCtrByClabNo($clabno);
			//$avlabDetails = $this->ModelrekabContractor->getAvlabDetails($clabno);
			$data['listofLabours'] = $this->ModelrekabContractor->getLabourListByCompany($clabno);
			$this->load->view('rekabcontractor/ctr_print_rekab_app', $data);	
		}
		
		function printVerifikasiRekab(){
			$clabno = $this->uri->segment(3);
			$data['ctr'] = $this->ModelrekabContractor->getCtrByClabNo($clabno);
			//$avlabDetails = $this->ModelrekabContractor->getAvlabDetails($clabno);
			$data['listofLabours'] = $this->ModelrekabContractor->getLabourListByCompany($clabno);
			$this->load->view('rekabcontractor/ctr_print_rekab_validate', $data);	
		}
		
		function printlebihanRekab(){
			$clabno = $this->uri->segment(3);
			$data['ctr'] = $this->ModelrekabContractor->getCtrByClabNo($clabno);
			//$avlabDetails = $this->ModelrekabContractor->getAvlabDetails($clabno);
			$data['listofLabours'] = $this->ModelrekabContractor->getLabourListByCompanyc($clabno);
			$this->load->view('rekabcontractor/ctr_print_rekab_validate_extra', $data);	
		}
		
		function printnoackRekab(){
			$clabno = $this->uri->segment(3);
			$data['ctr'] = $this->ModelrekabContractor->getCtrByClabNo($clabno);
			//$avlabDetails = $this->ModelrekabContractor->getAvlabDetails($clabno);
			$data['listofLabours'] = $this->ModelrekabContractor->getLabourListByCompany0($clabno);
			$this->load->view('rekabcontractor/ctr_print_rekab_noack', $data);	
		}
		...
		function printcard(){
			...
			$this->load->view('rekabcontractor/ctr_print_card_wkr', $data);
		}
		...
		function printsalary(){
			...
			$this->load->view('rekabcontractor/ctr_print_salary', $data);
		}
		
		function printArrivalWorkers(){
			$clabno = $this->uri->segment(3);
			$data['ctr'] = $this->ModelrekabContractor->getCtrByClabNo($clabno);
			
			$this->load->view('rekabcontractor/ctr_print_handover_arrival', $data);
		}
		
		function printCancelPermit(){
			$clabno = $this->uri->segment(3);
			
			//$data['expiredWorkersRecord'] = $this->ModelrekabContractor->listExpiryByCompany($fieldname, $noOfMonths, 'listOfWorkers', $clabNo);
			$data['expiredWorkersRecord'] = $this->ModelrekabContractor->getCancelPermit($clabno);
			$data['ctr'] = $this->ModelrekabContractor->getCtrByClabNo($clabno);
			
			$this->load->view('rekabcontractor/ctr_print_plksexp_warning', $data);
		}
		
		function printPeringatanLaporan(){
			$clabno = $this->uri->segment(3);
			...
			$this->load->view('rekabcontractor/peringatan_laporan', $data);
		}
		function printPeringatanTerakhir(){
			$clabno = $this->uri->segment(3);
			...
			$this->load->view('rekabcontractor/peringatan_terakhir', $data);
		}
		
		function printhandoverAck(){
			$clabno = $this->uri->segment(3);
			$data['ctr'] = $this->ModelrekabContractor->getCtrByClabNo($clabno);
		...
		function printTransferValidate(){
			...
			if($avlabDetails->num_rows() > 0){
				...
				$data['avlabRow'] = $avlabRow;
				$data['avlabWkrDetails'] = $this->ModelrekabContractor->getAvlabWkrDetails($avlabRow->avlab_ref_no);
				$this->load->view('rekabcontractor/ctr_print_transfer_valid', $data);
			}
			else{
				$data['errorMsg'] = "No local transfer application has been made under this company.";
				$this->load->view('rekabcontractor/ctr_print_errorpage', $data);
		...
		function printLetterMain(){
			...
			$this->load->view('rekabcontractor/ctr_print_main', $data);
		}
		...
		function printRenewalForm(){
			...
			$this->load->view('rekabcontractor/ctr_print_renewalform.php', $data);
		}
		
		function changeCtrStatus(){
			$this->load->view('rekabcontractor/change_status_pt1');
		}
		...
		function changeCtrStatusSearch(){
			...
			$this->load->view('rekabcontractor/change_status_pt1', $data);
		}
		
		function changeCtrStatusPt2(){
			$clabno = $this->uri->segment(3);
			$data['ctrStatus'] = $this->ModelrekabContractor->getAllCtrStatus();
			$data['ctr'] = $this->ModelrekabContractor->getCtrByClabNo($clabno);
			
			$this->load->view('rekabcontractor/change_status_pt2', $data);
		}
		...
		function changeCtrStatusPt2Submit(){
			...
			$data['successMsg'] = "<font color=\"red\"><strong>The rekabcontractor status has been updated.</strong></font>";
		...
		function paymentForm(){
			...
			if(strlen($currentuserid) > 0){
				$clabno = $this->uri->segment(3);
				$data['ctr'] = $this->ModelrekabContractor->getCtrByClabNo($clabno);
				$data['paymentHistory'] = $this->ModelrekabContractor->getPreviousPayments($clabno);
				$data['clabno'] = $clabno;
				$data['comp_name'] = $this->ModelrekabContractor->getCtrNameByClabNo($clabno);
				
				$this->load->view('rekabcontractor/ctr_payment', $data);
			}
			else{
				$this->load->view('session_expired_popup.php');	
			}
		...
		function paymentSubmit(){
			...
			if(strlen($currentuserid) > 0){
				...
				if($periodreg == '1') $clabExpDate = date('Y-m-d', strtotime('+1 year', strtotime($validdate)));
				else if($periodreg == '2') $clabExpDate = date('Y-m-d', strtotime('+2 year', strtotime($validdate)));
				else if($periodreg == '3') $clabExpDate = date('Y-m-d', strtotime('+3 year', strtotime($validdate)));
				else if($periodreg == '4') $clabExpDate = date('Y-m-d', strtotime('+4 year', strtotime($validdate)));
				else if($periodreg == '5') $clabExpDate = date('Y-m-d', strtotime('+5 year', strtotime($validdate)));
				else if($periodreg == '6') $clabExpDate = date('Y-m-d', strtotime('+6 year', strtotime($validdate)));
				else if($periodreg == '7') $clabExpDate = date('Y-m-d', strtotime('+7 year', strtotime($validdate)));
				else if($periodreg == '8') $clabExpDate = date('Y-m-d', strtotime('+8 year', strtotime($validdate)));
				else if($periodreg == '9') $clabExpDate = date('Y-m-d', strtotime('+9 year', strtotime($validdate)));
				else if($periodreg == '10') $clabExpDate = date('Y-m-d', strtotime('+10 year', strtotime($validdate)));
				else if($periodreg == '11') $clabExpDate = date('Y-m-d', strtotime('+11 year', strtotime($validdate)));
				else if($periodreg == '12') $clabExpDate = date('Y-m-d', strtotime('+12 year', strtotime($validdate)));
				else{	
				}
		...
		function viewExpiryLetter(){
			...
			if($noOfMonths == '3'){
				...
				$this->load->view('rekabcontractor/ctr_print_plksexp', $data);	
			}
			else if($noOfMonths == '2'){
				$data['reminderType'] = "Second Reminder";
				$this->load->view('rekabcontractor/ctr_print_plksexp_reminder', $data);	
			}
				...
				$this->load->view('rekabcontractor/ctr_print_plksexp_finalremind', $data);	
			}
			else if($noOfMonths == '-1'){
		...
		function viewExpiryLetterOld(){
			$fieldname = $this->uri->segment(3);
			$noOfMonths = $this->uri->segment(4);
			$clabNo = $this->uri->segment(5);	
			
			$data['expiredWorkersRecord'] = $this->ModelrekabContractor->listExpiryByCompanyLast($fieldname, $noOfMonths, 'listOfWorkers', $clabNo);
			//echo $expiredWorkersRecord->num_rows();
			$data['ctr'] = $this->ModelrekabContractor->getCtrByClabNo($clabNo);
			$data['fieldname'] = $fieldname;
			...
			else if($noOfMonths == '-1' or $noOfMonths == '-2' or $noOfMonths == '-3'){
				$data['reminderType'] = "Warning";
				$this->load->view('rekabcontractor/ctr_print_plksexp_warning', $data);	
			}
			else{
		...
		function printExpiryLetter(){
			...
			//save to database, mark as printed
			$isAlreadyPrinted = $this->ModelrekabContractor->checkExistingPrintRecord($clabNo, $noOfMonths, $fieldname);
			if($isAlreadyPrinted->num_rows() == 0){
				$nextprintID = $this->ModelrekabContractor->getNextPrintLetterID();
				$currentuserid = $this->session->userdata('emp_id');
		...
		function viewListOfLabours(){
			...
			$data['companyname'] = $this->ModelrekabContractor->getCtrNameByClabNo($clabno);
			$data['listofLabours'] = $this->ModelrekabContractor->getLabourListByCompany($clabno);
			
			$this->load->view('rekabcontractor/ctr_listofLabours', $data);	
		}
		
		function deleteUploadedFile(){
			$upload_id = $this->uri->segment(3);
			$ctr_id = $this->uri->segment(4);
			$filepath = $this->ModelrekabContractor->getFilePathByUploadID($upload_id, 'att_desturl');
		...
		function ctrStatusHistory(){
			$clabNo = $this->uri->segment(3);
			$data['ctr'] = $this->ModelrekabContractor->getCtrByClabNo($clabNo);
			$data['statusHistory'] = $this->ModelrekabContractor->getCtrStatusHistory($clabNo);
			
			$this->load->view('rekabcontractor/change_status_history', $data);
		}
		...
		function listFifthYearLetter(){
			...
			$this->load->view('rekabcontractor/ctr_listby_fifthyr', $data);	
		}
		
		function printFifthYearLetter(){
			$this->load->view('rekabcontractor/ctr_print_fifthyr');	
		}
	}
?>