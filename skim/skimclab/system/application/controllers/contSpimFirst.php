<?php
	class ContSpimFirst extends Controller{
		...
		$sql = "delete from wo_fcl_1st where fcl_id = '$id'";
		$this->db->trans_begin();
		$this->db->query($sql);	
		...
		$entrydate=date('Y-m-d', strtotime($_POST["txtentrydate"]));
		...
		$sql = "insert into wo_fcl_1st
				...
				fcl_relationship,fcl_year_renewal,fcl_wo_type,fcl_pass,fcl_name,fcl_nationality,fcl_newpassno,fcl_dob,fcl_gender,wkr_country,fcl_entrydate)
                values
                ('$workorderid','$passno','$permitno','$passexp','$salary','$wt','$nok','$relationship','$yearrenew',			 '$wotype','$chkpass',".$this->db->escape($name).",'$nationality','$newpassno','$dob','$gender','$country','$entrydate')"; 
        $this->db->trans_begin();
		$this->db->query($sql);	
        $this->db->trans_commit();
	...
	/*	function form_new_labour(){
		...
		function confirmletter(){
		      $wono = $this->uri->segment(3); 
			  $workorderid = $this->uri->segment(4);
			  $data['dentry']  = $this->ModelSpimFirst->getOneDate($wono);
			  $data['woRow'] = $this->ModelSpimFirst->getWObyWOid($wono);
			  $data['allFCLFiles'] = $this->ModelSpimFirst->getAllFCLFiles($workorderid);
		...
		/*		$legalQuery = "INSERT INTO wo_legal (legal_woid, legal_agree_receivedate, legal_date_ho) 
				...
				$trackid = $this->ModelSpimFirst->getNextPhTrackLogID();
		...
		function editPhoneTrackLog(){
			...
			$id = $data['id'];
			//echo $id;
			$data['woLog'] = $this->ModelSpimFirst->getLogbyLogid($id);
			$this->load->view('spimplks/edit_workorder_phtracklog', $data);	
		}
		...
		function printSPList(){
			...
			$this->load->view('spimplks/spim_print_SPApp.php', $data);
		}
