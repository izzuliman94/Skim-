<?php
	class ContSpim extends Controller{
		...
		function Send_Report(){
           ...
           // $data['allFCLlampiran'] = $this->ModelSpim->getLampiranFCLbyID($wono);	
            ...
            $this->email->to('akmar@clab.com.my , mrizal@clab.com.my , hartini@clab.com.my');
			$this->email->cc('rafik@clab.com.my , hayati@clab.com.my');
		...
		$data['allworktrade'] = $this->ModelLabour->getAllWorkTrade(); 
		$data['allCountries'] = $this->ModelLabour->getAllCountries();
		$data['lampiran'] = $this->ModelSpim->getLampiranData($workorderid);
		...
		$sql_workers = "insert into workers
		               (wkr_id,wkr_incharge,wkr_passno,wkr_name,wkr_currentemp ,wkr_dob,wkr_homeaddr,wkr_country,wkr_passexp,wkr_address1,wkr_address2,wkr_address3,wkr_pcode,wkr_state,wkr_status,wkr_wtrade,wkr_plksno,wkr_next_of_kin,wkr_relationship,wkr_gender)
					   values
					 ('$workerid','$currentuserid','$passno',".$this->db->escape($name).",'$companyname','$dob','$birthplace','$country','$passexp', '$add1', '$add2','$add3','$pcode','$state','1','$wt','$plksno',".$this->db->escape($nok).",'$relationship','$gender')";
		
		
		$sql = "insert into wo_fcl		            
		       (fcl_woid,fcl_passno,fcl_name,fcl_dob,fcl_bthplace,fcl_nationality,fcl_gender,fcl_passissue,
		        fcl_passexp,fcl_salary,fcl_wt,fcl_country,fcl_next_of_kin,fcl_add,fcl_relationship)
				values
                ('$workorderid','$passno',".$this->db->escape($name).",'$dob','$birthplace','$nationality','$gender','$passissue',
				'$passexp','$salary','$wt','$country',".$this->db->escape($nok).",'$add','$relationship')"; 
	...
	/*	function form_new_labour(){
		...
		function handoverletter(){
			  ...
			  $data['woRow'] = $this->ModelSpim->getWObyWOid($wono);
			  $data['allFCLFiles'] = $this->ModelSpim->getAllFCLFiles($workorderid);
			  $this->load->view('spim/handover_letter', $data);
		}
		// added by azh
		function confirmletter(){
			  ...
			  $this->load->view('spim/confirm_letter', $data);
		}
		...
		function newWorkOrderPt2(){
			...
			if(strlen($currentuserid) > 0){
				...
				$data['allCountries'] = $this->ModelSpim->getAllCountries();
				$data['allAgencies'] = $this->ModelSpim->getAllAgencies();
				$data['allAck'] = $this->ModelSpim->getAllAck();
				$data['allapproval'] = $this->ModelSpim->getAllApproval();
		...
		function newWorkOrderPt2Submit(){
			...
			if(strlen($currentuserid) > 0){
				...
				//$wo_ack = $_POST['selack'];
				$wo_ack = $_POST['txtackno'];
				
				$wo_app = $_POST['selapproval'];
				...
				$woQuery = "INSERT INTO workorders (wo_id, wo_num, wo_date, wo_clab_no, wo_fcl_total, wo_fcl_country, wo_agency, wo_ack, wo_appid, wo_isreplacement, wo_replace_date, wo_replace_reason, 
		    				...
		    				$wo_fcl_total, '$wo_fcl_country', '$wo_agency','$wo_ack','$wo_app','$wo_isreplacement', '$wo_replace_date', '$wo_replace_reason', 
	...
	/*			$legalQuery = "INSERT INTO wo_legal (legal_woid, legal_agree_receivedate, legal_date_ho) 
			   ...
			   $LogQuery = "INSERT INTO wo_phonetrack (track_id,track_wo_id,track_remarks,track_datetime,track_attendby) 
	            			     VALUES ('$trackid','$wo_id','Create Workorder','$datetime','$wo_keyin_by')";				
	/*			$statusQuery = "INSERT INTO wo_status (status_woid, wo_isreceive, wo_receiveby, wo_receivedate, wo_receive_comment, wo_isprocess, wo_processby, 
		...
		function updateWorkOrderPt2(){
			...
			if(strlen($currentuserid) > 0){
				...
				$data['allapproval'] = $this->ModelSpim->getAllApproval($workorderid);
		...
		function updateWorkOrderPt2Submit1(){
			$currentuserid = $this->session->userdata('emp_id');
			if(strlen($currentuserid) > 0){
				$wo_id = $_POST['txtwoid'];
				...
				$wo_pay_fwcs = $_POST['selfwcs'];
				$wo_pay_fwhs = $_POST['selfwhs'];
				$wo_pay_igno= $_POST['txtigpolicy'];
				//$wo_pay_fwcsno = $_POST['txtcspolicy'];
				$wo_pay_fwhsno = $_POST['txthspolicy'];
				...
				if(isset($_POST['chkissubmit2'])){
					$wo_issentto_legal = '1';
					$wo_issentto_legal_ischecked = '1';
					$wo_sentlegalby = $_POST['txtsubmitlegal'];
					$wo_sentlegaldate = $_POST['dtsubmitlegal'];
						if($wo_sentlegaldate != "") $wo_sentlegaldate = date('Y-m-d', strtotime($wo_sentlegaldate));
						else $wo_sentlegaldate = "0000-00-00";
				}
				$wo_sentlegalcomment = $_POST['txtsentlegalcomment'];
				...
				else if($wo_isreceive_visa == '1') $wo_latest_progress = "receive calling visa";
				...
				//$this->db->query($sqlQuery);
				$woQuery = "UPDATE workorders SET wo_fcl_total = $wo_fcl_total, wo_fcl_country = '$wo_fcl_country', wo_agency = $wo_agency, wo_appid = $wo_app, wo_ack = '$wo_ack', wo_isreplacement = '$wo_isreplacement', wo_replace_date = '$wo_replace_date', wo_replace_reason = '$wo_replace_reason', 
	            ...
	            $paymentQuery = "UPDATE wo_payment SET pay_refno = '$wo_pay_refno', pay_adminfee = '$wo_pay_adminfee', pay_levy = '$wo_pay_levy', pay_plks = '$wo_pay_plks', pay_ig = '$wo_pay_ig', 
				 pay_igno = '$wo_pay_igno', pay_fwhsno = '$wo_pay_fwhsno',
				...
				if($wo_issentto_legal_ischecked == '1'){
					$statusQuery .= ", wo_issentto_legal = '$wo_issentto_legal', wo_sentlegalby = '$wo_sentlegalby', wo_sentlegaldate = '$wo_sentlegaldate'";
				}
				...
				$statusQuery .= ", wo_receive_comment = '$wo_receive_comment', 
								   ...
								   wo_sentlegalcomment = '$wo_sentlegalcomment', 
		...
		function updateWorkOrderPt2Submit(){
			...
			if(strlen($currentuserid) > 0){
				...
				$wo_submitdldate = $_POST['dtsubmitdl'];
						if($wo_submitdldate != "") $wo_submitdldate = date('Y-m-d', strtotime($wo_submitdldate));
						else $wo_submitdldate = "0000-00-00";
				$wo_collectdldate = $_POST['dtcollectdl'];
						if($wo_collectdldate != "") $wo_collectdldate = date('Y-m-d', strtotime($wo_collectdldate));
						else $wo_collectdldate = "0000-00-00";
				$wo_sentlegalcomment = $_POST['txtsentlegalcomment'];
				...
				else if($wo_issentto_legal == '1') $wo_latest_progress = "submit to Legal";
				...
				//$this->db->query($sqlQuery);
				$woQuery = "UPDATE workorders SET 
				wo_fcl_total = $wo_fcl_total, 
				wo_fcl_country = '$wo_fcl_country', 
				wo_ack = '$wo_ack', 
				wo_agency = $wo_agency, 
				wo_appid = $wo_app,
				wo_isreplacement = '$wo_isreplacement', 
				wo_replace_date = '$wo_replace_date', 
				wo_replace_reason = '$wo_replace_reason', 
		    	wo_iswithdrawal = '$wo_iswithdrawal', 
				wo_withd_date = '$wo_withd_date', 
				wo_withd_reason = '$wo_withd_reason', 
				wo_personincharge = '$wo_personincharge', 
				wo_modifiedby = '$modifiedby', 
				wo_modifieddate = NOW(),
				wo_visa_date = '$wo_visa_date',
				wo_sender_name = '$wo_sendername', 
				wo_sender_ic = '$wo_senderic',
				wo_sender_contact = '$wo_senderctc',
				chk_ver_corp = '$wo_ver_corp',
				ver_corp_by='$vercorpby',
				ver_corp_date='$vercorpdate',
				chk_ver_fin='$wo_ver_fin',
				ver_fin_by='$verfinby',
				ver_fin_date='$verfindate',
				chk_app_ceo='$wo_app_ceo',
				app_ceo_by='$appceoby',
				app_ceo_date='$appceodate',
				wo_attn_officer = '$wo_attn_off' 
				WHERE wo_id = $wo_id";
				//die($woQuery);
		    	$docQuery = "UPDATE wo_doc SET 
				doc_rqform = '$wo_doc_rqform', 
				doc_empletter = '$wo_doc_empletter', 
				doc_awardletter = '$wo_doc_awardletter', 
				doc_supplieragree = '$wo_doc_supplieragree', 
				doc_acco = '$wo_doc_acco', 
				 doc_signedpayment = '$wo_doc_signedpayment', 
				 doc_datecomplete = '$wo_doc_datecomplete', 
				 doc_rqformdate = '$doc_rqformdate', 
				 doc_empletterdate = '$doc_empletterdate', 
				 doc_awardletterdate = '$doc_awardletterdate', 
				 doc_supplieragreedate = '$doc_supplieragreedate', 
				 doc_accodate = '$doc_accodate', 
				 doc_signedpaymentdate= '$doc_signedpaymentdate'
				 WHERE doc_woid = $wo_id";
				
	            $paymentQuery = "UPDATE wo_payment SET 
				pay_refno = '$wo_pay_refno', 
				pay_adminfee = '$wo_pay_adminfee', 
				pay_levy = '$wo_pay_levy', 
				pay_plks = '$wo_pay_plks', 
				 pay_agencyfee = '$wo_pay_agencyfee', 
				 pay_fomema = '$wo_pay_fomema', 
				 pay_visa = '$wo_pay_visa', 
				 pay_ig = '$wo_pay_ig', 
				 pay_igno = '$wo_pay_igno', 
				 pay_fwhs = '$wo_pay_fwhs' , 
				 pay_fwhsno = '$wo_pay_fwhsno' 
				 WHERE pay_woid = $wo_id";
	            			     
				$legalQuery = "UPDATE wo_legal SET 
				legal_agree_receivedate = '$wo_agree_receivedate' 
				WHERE legal_woid = $wo_id";
	            			    
				$statusQuery = "UPDATE wo_status SET 
				wo_latest_progress = '$wo_latest_progress',wo_submitdldate = '$wo_submitdldate',wo_collectdldate = '$wo_collectdldate'";
				if($wo_isreceive_ischecked == '1'){
					$statusQuery .= ", wo_isreceive = '$wo_isreceive', wo_receiveby = '$wo_receiveby', wo_receivedate = '$wo_receivedate'";
				}
				if($wo_isprocess_ischecked == '1'){
		...
		function listAllWorkorders(){
		    ...
		    //$grid->db->join("contractors","workorder.wo_clab_no = contractors.ctr_clab_no","LEFT");
		    //$grid->db->order_by("wo_id");
			$grid->db->join("wo_approval","workorders.wo_appid = wo_approval.app_id", "LEFT");
		    $grid->db->join("wo_agency","workorders.wo_agency = wo_agency.agency_id", "LEFT");
		    $grid->db->join("wo_doc","workorders.wo_id = wo_doc.doc_woid", "LEFT");
		    $grid->db->join("wo_legal","workorders.wo_id = wo_legal.legal_woid", "LEFT");
		    $grid->db->join("wo_payment","workorders.wo_id = wo_payment.pay_woid", "LEFT");
		...
		function searchWorkorders(){
		    ...
		    if($searchby == "ctr_comp_name" || $searchby == "ctr_clab_no" || $searchby == "wo_ack" || $searchby == "fcl_passno"  || $searchby == "pay_refno" || $searchby == "wo_jimack_refno")
      ...
      $grid->column("Workorder Number","wo_num");
		...
		function printWPList_H(){
			$workorderid = $this->uri->segment(3);
			$data['woRow'] = $this->ModelSpim->getWObyWOid($workorderid);
			
			$this->load->view('spim/spim_print_SuratWakil - Hazlim.php', $data);
		}
		
		function printWPList_R(){
			$workorderid = $this->uri->segment(3);
			$data['woRow'] = $this->ModelSpim->getWObyWOid($workorderid);
			
			$this->load->view('spim/spim_print_SuratWakil - Rizal.php', $data);
		}
		...
		function printHOVDR(){
			...
			$this->load->view('spim/spim_print_handover_ack.php', $data);
		}
