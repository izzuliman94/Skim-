<?php
	class ContAvailablepm extends Controller{
		function ContAvailablepm(){
			parent::Controller();
			$this->load->library('rapyd');
			$this->load->library('session');
			$this->load->Model('ModelContractor');
			$this->load->Model('ModelAvailablepm');
			$this->load->Model('ModelLabour');
		}	
		
		function availableForm(){
			$data['allContractors'] = $this->ModelAvailablepm->getAllContractors();
			$this->load->view('availablepm/newavailable_form', $data);
		...
		function availableFormWithCompDetails(){
			...
			$ctrData1 = $this->ModelAvailablepm->getCtrByClabNo($compFrom);
			$ctrData2 = $this->ModelAvailablepm->getCtrByClabNo($compTo);
			...
			$data['compTo'] = $compTo;
			$this->load->view('availablepm/availableFormDetails', $data);
		}
		...
		function availableFormSubmit(){
			...
			$nextrefno = $this->ModelAvailablepm->getNewAvlabRefNo();
			$avlabrefno = "PM-" . date('y') . "-" . $nextrefno;
			
			$insertQuery = "INSERT INTO availablepm (avlab_ref_no, avlab_submit_date, avlab_comp_from, avlab_comp_to, avlab_statusid, avlab_entered_by, avlab_entered_date, avlab_qty)
							VALUES ('$avlabrefno', '$dateSubmit', '$compFrom', '$compTo', 1, '$keyinby', '$keyindate', $totalQty);";
			
			$this->db->query($insertQuery);
			
			redirect('contAvailablepm/editAvailable/' . $avlabrefno . '/add');
		}
		...
		function displayavlaburi($refno){
			$hrefStr = "<a href=\"" .site_url() . "/contAvailablepm/editAvailable/" . $refno . "\">" . $refno . "</a>";
			return $hrefStr;	
		}
		
		function displayuri($wkr_passno){
			$hrefString = anchor_popup("contLabour/labourDetails/". $wkr_passno, $wkr_passno);
		    $hrefString2 = "<a href=\"" . site_url(). "/contAvailablepm/wkrDetails/" . $wkr_passno . "\">" . $wkr_passno . "</a>";
		...
		function availableDashbrd(){
		    ...
		    //grid
		    $grid = new DataGrid("", "availablepm");
		    $grid->db->join("contractors ctr","availablepm.avlab_comp_from = ctr.ctr_clab_no","LEFT");
		    $grid->db->join("mst_avstatus", "mst_avstatus.avstatus_id = availablepm.avlab_statusid", "LEFT");
			...
			//$grid->column("COMPANY CLAB No To","avlab_comp_to");
			...
			$baseuri = "contAvailablepm";
  			$grid->build();

		    $data["crud"] = $grid->output;
		    /**********end grid****************/
			$this->load->view('availablepm/availableDashboard', $data);
		}
		...
		function printTransferValidate(){
			...
			$data['avlabwkr'] = $this->ModelAvailablepm->getAvlabwkr($ltref);
			if($avlabDetails->num_rows() > 0){
				...
				$this->load->view('availablepm/ctr_print_transfer_valid', $data);
			}
			else{
				$data['errorMsg'] = "No local transfer application has been made under this company.";
				$this->load->view('availablepm/ctr_print_errorpage', $data);
			}
		}
		...
		function editAvailable(){
			...
			$avlabDetails= $this->ModelAvailablepm->getAvailableDetails($refno);
			$data['ctrData1'] = $this->ModelAvailablepm->getCtrByClabNo($avlabDetails->avlab_comp_from);
			if($avlabDetails->avlab_comp_to!= '')
				$data['ctrData2'] = $this->ModelAvailablepm->getCtrByClabNo($avlabDetails->avlab_comp_to);
			$data['avlabDetails'] = $avlabDetails;
			$registeredWorkers = $this->ModelAvailablepm->getAvlabWkrByStatus($refno, 1);
			 $data['registeredWorkers'] = $registeredWorkers;
			//$data['avlabWkrDetails'] = $this->ModelAvailablepm->getAvailableWkrDetails($refno);
			$data['refno'] = $refno;
			
		    $data['allPhTrackLog'] = $this->ModelAvailablepm->getAllPhTrackLog($refno);
			/********Grid for labor list*******/
			$css_src = $this->rapyd->get_elements_path("css/sippsstyle.css","css");
			$this->rapyd->load("datafilter","datagrid");
		    $this->rapyd->uri->keep_persistence();
			...
			$this->load->view('availablepm/editAvailable', $data);
		}
		...
		function editPhoneTrackLog(){
			...
			$id = $data['id'];
			//echo $id;
			$data['woLog'] = $this->ModelAvailablepm->getLogbyLogid($id);
			$this->load->view('availablepm/edit_lt_phtracklog', $data);	
		}
		...
		function newPhoneTrackLog(){
			...
			$this->load->view('availablepm/update_lt_phtracklog', $data);	
		}
		...
		function newPhoneTrackLogSubmit(){
			...
			if(strlen($currentuserid) > 0){			
				...
				//phone track log
				$trackid = $this->ModelAvailablepm->getNextPhTrackLogID();
		...
		function editAvailableSubmit(){
			...
			$updateQuery = "UPDATE availablepm 
			...
			redirect('contAvailablepm/editAvailable/' . $avlab_ref_no . '/update');
		}
		...
		function regisNewLabourPt1(){
			...
			$data['avlabRecord'] = $this->ModelAvailablepm->getAvailableSummary($ref_no);
			
			$this->load->view('availablepm/editavailable_regislabor_pt1', $data);	
		}
		...
		function regisNewLabourPt2(){
			...
			if($existingLabour->num_rows() == 0){
				$siteuri = "contAvailablepm/regisNewLabourNew/" . $passportNo . "/" . $ref_no;
				...
				$isWkrRegistered = $this->ModelAvailablepm->checkRegisteredWkr($passportNo, $ref_no);
		...
		function regisNewLabourNew(){
			...
			$data['avlabRecord'] = $this->ModelAvailablepm->getAvailableSummary($avlabrefno);
			...
			$this->load->view('availablepm/editavailable_regislabor_pt2', $data);	
		}
		...
		function regisExistingLabour(){
			...
			$data['avlabRecord'] = $this->ModelAvailablepm->getAvailableSummary($avlab_refno);
		...
		function regisNewLaborPt2Submit(){
			...
			{
			    $this->db->trans_commit();
			    redirect('contavailablepm/regisNewLabourPt1/' . $avlab_refno . '/' . $wkr_passno);
			}
			/****************END TRANSACTION****************/
		}
		...
		function viewLaborDetails(){
			...
			$this->load->view('availablepm/availableLabourDetails', $data);
		}
...
function printCheckLits(){
	...
	$data['avlabDetails']= $this->ModelAvailablepm->getAvailableDetails($refno);
	
	$data ['ctrData1'] = $this->ModelAvailablepm->getCtrByClabNo($compFrom);
	
	//$this->load->view('availablepm/lt-checklist.php', $data);
	$this->load->view('availablepm/lt-checklist.php', $data);
		...
		function removeLabour(){
			...
			{
			    $this->db->trans_commit();
			    redirect('contavailablepm/editAvailable/' . $ref_no);
			}
			/****************END TRANSACTION****************/
		}
	}
