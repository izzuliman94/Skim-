<?php
	class ContSpimtc extends Controller{
		function ContSpimtc(){
			parent::Controller();
			$this->load->library('session');
			$this->load->library('rapyd');	
			$this->load->Model('ModelSpimtc');	
		...
		$wono = $this->uri->segment(3);
		    $data['lampiran'] = $this->ModelSpimtc->getLampiranData($wono);
            $data['allFCLlampiran'] = $this->ModelSpimtc->getLampiranFCLbyID($wono);
			$msg = $this->load->view('spimtc/email_items', $data);
		...
		function Insurance_purchase(){
		    ...
		    $data['lampiran'] = $this->ModelSpimtc->getLampiranData($wono);
            $data['allFCLlampiran'] = $this->ModelSpimtc->getLampiranFCLbyID($wono);	
			$msg = $this->load->view('spimtc/email_items', $data, true);
		...
		function listAllApproval(){
		   $data["ApprovalList"] = $this->ModelSpimtc->getApprovalList();
		   $this->load->view('spimtc/approvallist',$data);	 
		}
		
		function getcountry(){
		   $frm = $this->uri->segment(3);
		   $data['countrylist'] = $this->ModelSpimtc->getAllCountry();
		     $data['form'] = $frm;
		   $this->load->view('spimtc/countrylist',$data);	 
		}
		...
		function openreceipt(){
		   ...
		   $data['receiptlist'] = $this->ModelSpimtc->getAllReceipt($clabno);
		   $data['form'] = $frm;
	       $this->load->view('spimtc/receiptlist',$data);		   
		...
		$data['im12'] = $this->ModelSpimtc->getFCLbyID($fclid);
		$this->load->view('spimtc/im12_indv',$data);		
		}
		...
		/********Grid for labor list*******/
			...
			$grid->db->where("wo_clab_no ='$clabid' and wo_spim_status = 'T'");
			...
			$this->load->view('spimtc/batch_application_list', $data);
			...
			if(strlen($currentuserid) > 0){
				...
				$data['woRow'] = $this->ModelSpimtc->getWObyWOid($workorderid);
				$data['allCountries'] = $this->ModelSpimtc->getAllCountries();
				$data['allAgencies'] = $this->ModelSpimtc->getAllAgencies();
				$data['allPhTrackLog'] = $this->ModelSpimtc->getAllPhTrackLog($workorderid);
				$data['allFCLFiles'] = $this->ModelSpimtc->getAllFCLFiles($workorderid);
				
				$this->load->view('spimtc/view_wo_batch_app', $data);
			}
			else{
		...
		$this->load->view('spimtc/reg_new_labour', $data);	
		...
		$this->load->view('spimtc/reg_new_legal', $data);
		}
		...
		$data['lampiran'] = $this->ModelSpimtc->getLampiranData($wono); 
		$id = $data['id'];
		$data['fcl'] = $this->ModelSpimtc->getFCLbyID($id);
		$data['strhide'] = "update";
		
		$this->load->view('spimtc/edit_fcl', $data);
		...
		$id = $data['id'];
		$data['legal'] = $this->ModelSpimtc->getLegalbyID($id);
		$data['strhide'] = "update";
		
		$this->load->view('spimtc/edit_legal', $data);
		...
		function lampiran_a(){
			...
			$data['allFCLlampiran'] = $this->ModelSpimtc->getLampiranFCLbyID($wono);
			$data['woRow'] = $this->ModelSpimtc->getWObyWOid($wono);
			
			$this->load->view('spimtc/lampiran_a', $data);
		...
		function lampiran_a_excel(){
			...
			$this->load->view('spimtc/lampiran_a_excel', $data);
		...
		function second_schedule(){
			...
			$this->load->view('spimtc/second_schedule', $data);
		...
		function im12(){
		   $wono = $this->uri->segment(3);
		   $data['allFCLlampiran'] = $this->ModelSpimtc->getLampiranFCLbyID($wono);			
		   $this->load->view('spimtc/im12', $data);
		}
		...
		function addendum(){
		   ...
		   $data['allFCLlampiran'] = $this->ModelSpimtc->getLampiranFCLbyID($wono);	
		   $data['woRow'] = $this->ModelSpimtc->getWObyWOid($wono);		
		   $this->load->view('spimtc/addendum', $data);
		}
		
	    function borangbayaran(){
		   $wono = $this->uri->segment(3);
		   $data['lampiran'] = $this->ModelSpimtc->getLampiranData($wono);
		   $data['TotalFCL'] = $this->ModelSpimtc->getTotalFCL($wono);	
		   $data['allFCLlampiran'] = $this->ModelSpimtc->getLampiranFCLbyID($wono);			
		   $this->load->view('spimtc/borang_bayaran', $data);
		}
		
		function acknowledge(){
		   $wono = $this->uri->segment(3);
		   $data['woRow'] = $this->ModelSpimtc->getWObyWOid($wono);
		   $this->load->view('spimtc/acknowledge', $data);
		}
		...
		$data['allworktrade'] = $this->ModelLabour->getAllWorkTrade(); 
		$data['allCountries'] = $this->ModelLabour->getAllCountries();
		$data['lampiran'] = $this->ModelSpimtc->getLampiranData($workorderid);
		...
		$sql = "insert into wo_fcl		            
		       (fcl_woid,fcl_passno,fcl_name,fcl_dob,fcl_bthplace,fcl_nationality,fcl_gender,fcl_passissue,
		        fcl_passexp,fcl_salary,fcl_wt,fcl_country,fcl_next_of_kin,fcl_relationship,fcl_pass,fcl_add,fcl_plksno)
				values
                ('$workorderid','$passno','$name','$dob','$birthplace','$nationality','$gender','$passissue',
				'$passexp','$salary','$wt','$country','$nok','$relationship','$chkpass','$add','$plksno')"; 
        $this->db->trans_begin();
		$this->db->query($sql);	
	...
	/*	function form_new_labour(){
		...
		function spimDashbrd(){
			$data['overall'] = $this->ModelSpimtc->getWorkordersByStatus("overall", "sum");
			$data['closed'] = $this->ModelSpimtc->getWorkordersByStatus("closed", "sum");
			$data['open'] = $this->ModelSpimtc->getWorkordersByStatus("open", "sum");
			$data['received'] = $this->ModelSpimtc->getWorkordersByStatus("received by Operation", "sum");
			$data['processed'] = $this->ModelSpimtc->getWorkordersByStatus("processed by Operation", "sum");
			$data['submithr'] = $this->ModelSpimtc->getWorkordersByStatus("submit to HR", "sum");
			$data['receivehr'] = $this->ModelSpimtc->getWorkordersByStatus("receive by HR", "sum");
			$data['jim'] = $this->ModelSpimtc->getWorkordersByStatus("JIM acknowledgement", "sum");
			$data['receiveVisa'] = $this->ModelSpimtc->getWorkordersByStatus("receive calling visa", "sum");
			
			$data['lt14days'] = $this->ModelSpimtc->getWoByJIMAckDate("less14", "sum"); //need to correct the sql script
			$data['mt14days'] = $this->ModelSpimtc->getWoByJIMAckDate("more14", "sum");
			
			$currentuser = $this->session->userdata('username');
			$data['woUnderCurrentUser'] = $this->ModelSpimtc->getWoUnderCurrentUser($currentuser);
			
			$this->load->view('spimtc/spim_dashboard', $data);	
		}	
		...
		function dashboardList(){
			...
			if($type == "overall") {
				...
				$progress = "overall";
				$data['woRecord'] = $this->ModelSpimtc->getWorkordersByStatus($progress, "record");
			}
			else if($type == "closed") {
			...
			if($type == "lessFourteen") {
				...
				$progress = "less14";
				$data['woRecord'] = $this->ModelSpimtc->getWoByJIMAckDate($progress, "record");	
			}
			...
			$this->load->view('spimtc/spim_dashboard_lists', $data);
		}
		
		function newWorkOrder(){
			$this->load->view('spimtc/new_workorder_pt1');	
		}
		...
		function newWOSearchCtr(){
			...
			$data['ctrRecord'] = $this->ModelSpimtc->searchCtr($searchby, $keyword);
			
			$this->load->view('spimtc/new_workorder_pt1', $data);
		}
		...
		function newWorkOrderPt2(){
			...
			if(strlen($currentuserid) > 0){
				...
				$ctrRecord = $this->ModelSpimtc->getCtrByClabNo($clabno);
				$data['ctrRow'] = $ctrRecord->row();
				$data['allCountries'] = $this->ModelSpimtc->getAllCountries();
				$data['allAgencies'] = $this->ModelSpimtc->getAllAgencies();
				
				$this->load->view('spimtc/new_workorder_pt2', $data);
			}
			else{
				$this->load->view('session_expired.php');	
			}
		...
		function newWorkOrderPt2Submit(){
			$currentuserid = $this->session->userdata('emp_id');
			if(strlen($currentuserid) > 0){
				$wo_id = $this->ModelSpimtc->getNextWONumber();
				...
				$woQuery = "INSERT INTO workorders (wo_id, wo_num, wo_date, wo_clab_no, wo_fcl_total, wo_fcl_country, wo_agency, wo_isreplacement, wo_replace_date, wo_replace_reason, 
		    				...
		    				$wo_fcl_total, '$wo_fcl_country', $wo_agency, '$wo_isreplacement', '$wo_replace_date', '$wo_replace_reason', 
		    				'$wo_iswithdrawal', '$wo_personincharge', '$wo_datesubmit', '$wo_keyin_by', '$wo_keyin_date','$wo_visa_date','T','$wo_sendername','$wo_senderic','$wo_senderctc','$wo_attn_off')";				
	...
	/*			$statusQuery = "INSERT INTO wo_status (status_woid, wo_isreceive, wo_receiveby, wo_receivedate, wo_receive_comment, wo_isprocess, wo_processby, 
			...
			/*	if($wo_isjim_ack !== '0' &&$wo_receivevisa_approve !== '0' && $wo_receivevisa_reject!=='0' && strlen(wo_receivevisa_comment) > 0){
				...
				redirect('contSpimtc/updateWorkOrderPt2/'.$wo_id .'/add');
			}
			else{
				$this->load->view('session_expired.php');	
			}
		...
		function uploadDocView(){
			...
			if(strlen($currentuserid) > 0){
				...
				$this->load->view('spimtc/new_workorder_upload', $data);	
			}
			else{
		...
		function uploadDoc(){
			...
			if(strlen($currentuserid) > 0){
				...
				//upload documents
				$config['upload_path'] = './uploads/spimtc';
				...
				else
				{
					...
					$upload_id = $this->ModelSpimtc->getNextUploadID();
		...
		function updateWorkOrder(){
			$this->load->view('spimtc/update_workorder_pt1');
		}
		...
		function updateWorkOrderSearch(){
			...
			$data['ctrRecord'] = $this->ModelSpimtc->getWOByWoidOrCompname($searchby, $keyword);
			
			$this->load->view('spimtc/update_workorder_pt1', $data);
		}
		...
		function updateWorkOrderPt2(){
			...
			if(strlen($currentuserid) > 0){
				...
				$data['allPhTrackLog'] = $this->ModelSpimtc->getAllPhTrackLog($workorderid);
				$data['allFCLFiles'] = $this->ModelSpimtc->getAllFCLFiles($workorderid);
				$data['allFCLUpload'] = $this->ModelSpimtc->getAllFCLUpload($workorderid);
				$data['allLegalFiles'] = $this->ModelSpimtc->getAllLegalList($workorderid);		
				
						
				
				$this->load->view('spimtc/update_workorder_pt2', $data);
			}
			else{
				$this->load->view('session_expired.php');	
			}
		...
		function updateWorkOrderPt2Submit(){
			...
			if(strlen($currentuserid) > 0){
				...
				//echo $legalQuery;
				//echo $statusQuery;
				redirect('contSpimtc/updateWorkOrderPt2/'.$wo_id .'/update');
			}
			else{
				$this->load->view('session_expired.php');	
			}
		...
		function newPhoneTrackLog(){
			...
			$this->load->view('spimtc/update_workorder_phtracklog', $data);	
		}
		...
		function newPhoneTrackLogSubmit(){
			...
			if(strlen($currentuserid) > 0){			
				...
				//phone track log
				$trackid = $this->ModelSpimtc->getNextPhTrackLogID();
		...
		function listAllWorkorders(){
			...
			$grid->db->where("wo_spim_status ='T'");
		    $grid->db->orderby("wo_id", "desc");
			...
			$this->load->view('spimtc/workorder_lists', $data);	
		}
      ...
      $grid->column("Workorder Number","wo_num");
		...
		function printCheckLits(){
			...
			$this->load->view('spimtc/spim_printchecklist.php', $data);
		}
		...
		function printHandoverList(){
			...
			$this->load->view('spimtc/spim_print_handover_.php', $data);
		}
		...
		function printHandoverArr(){
			...
			$this->load->view('spimtc/spim_print_handover_arrival.php', $data);
		}
			...
			$this->load->view('spimtc/spim_print_VDRapp.php', $data);
		}
			...
			$this->load->view('spimtc/spim_print_SuratWakil.php', $data);
		}
			...
			$this->load->view('spimtc/spim_print_handover_ack.php', $data);
		}
		...
		function handoverletter(){
			  ...
			  $data['woRow'] = $this->ModelSpimtc->getWObyWOid($wono);
			  $data['allFCLFiles'] = $this->ModelSpimtc->getAllFCLFiles($workorderid);
			  $this->load->view('spimtc/handover_letter', $data);
		}
		// added by azh
		...
		function confirmletter(){
			  ...
			  $this->load->view('spimtc/confirm_letter', $data);
		}
		...
		function printPLKS1year(){
			...
			$this->load->view('spimtc/spim_print_PLKS1styear.php', $data);
		}
		...
		function printSPList(){
			...
			$this->load->view('spimtc/spim_print_SPApp.php', $data);
		}
		...
		function printCOMList(){
			...
			$this->load->view('spimtc/spim_print_COMApp.php', $data);
		}
		function viewRejectHistory(){
			$clabno = $this->uri->segment(3);
			$wo_id = $this->uri->segment(4);
			$data['ctr_comp_name'] = $this->ModelSpimtc->getCtrNameByClabNo($clabno);;
			$data['clab_no'] = $clabno;
			$data['wo_id'] = $wo_id;
			...
			$this->load->view('spimtc/vdr_reject_history', $data);
		}
		...
		function updateRejectHistory(){
			...
			if(strlen($currentuserid)>0){
				...
				$nextRejectHistID = $this->ModelSpimtc->getNextRejectHistoryId();
				...
				redirect('contSpimtc/viewRejectHistory/' . $clab_no. '/' . $wo_id); 
			}
			else{
				$this->load->view('session_expired.php');
			}
