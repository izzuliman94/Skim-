<?php
	class contPaymentold extends Controller {
		function contPaymentold(){
			parent::Controller();
			$this->load->library('session');
			$this->load->library('rapyd');	
			$this->load->helper('url');
			$this->load->library('session');
			$this->load->Model('ModelPaymentold');
		...
		function summarylist(){
			$data['allRegion'] = $this->ModelPaymentold->getRegion();
			$data['allContractors'] = $this->ModelLabour->getAllContractors();
			$this->load->view('paymentold/paymentsummary',$data);
		}
		...
		function summaryview(){
			 ...
			 $data['paymenttype']= $this->ModelPaymentold->getpaymenttype($type);
			 //$paymenttype= $this->ModelPayment->getpaymenttype($type);
		     $data['summary'] = $this->ModelPaymentold->getsummarypayment($datefrom,$dateto,$region,$type,$cancel,$cont);
			 
			 //echo $paymenttype;
		     $this->load->view('paymentold/summary_view',$data);
		...
		function summaryexcel(){
		     ...
		     $this->load->view('paymentold/summary_excel',$data);
		}
		...
		function printreceipt(){
		   ...
		   $data['payment'] = $this->ModelPaymentold->getAllPaymentByID($pmtno);
		   $this->load->view('paymentold/print_receipt',$data);			
		}
		
		function printreceipt2(){
		   $pmtno = $this->uri->segment(3);
		   $data['payment'] = $this->ModelPaymentold->getAllPaymentByID($pmtno);
		   $this->load->view('paymentold/print_receipfull',$data);			
		}
		
		function printinvoice(){
		   $pmtno = $this->uri->segment(3);
		   $data['payment'] = $this->ModelPaymentold->getAllPaymentByID($pmtno);
		   $this->load->view('paymentold/print_invoice',$data);			
		}
		
		function printinvoice3(){
		   $pmtno = $this->uri->segment(3);
		   $data['payment'] = $this->ModelPaymentold->getAllPaymentByID($pmtno);
		   $this->load->view('paymentold/print_invoice2',$data);			
		}
		
		
		function printinvoiceOD(){
		   $pmtno = $this->uri->segment(3);
		   $data['invoice'] = $this->ModelPaymentold->getAllInvTaxByID($pmtno);
		   $this->load->view('paymentold/print_invoiceOD',$data);			
		}
		...
		function searchReceiptList(){
			...
			$this->load->view('paymentold/payment_receipt_list', $data);	
		}
		...
		function taxinvlist(){
		   /********Grid for labor list*******/
			...
			$this->load->view('paymentold/payment_invoice_list', $data);	
		}
	...
	function savetaxinvoice(){
	 ...
	 function updatepaymentrcpt(){
			...
			$sql = "update pmt_receipt set 
					...
					chk_clab_adm ='$chkclabadm',
					chk_clab_mship ='$chkothersfees2',
					clab_reg_amount = '$clabregamt',
					clab_pa_no ='$clabpa',
					...
					chk_transit_cost='$chktransitcost',
					agency_pa_no='$agencypa',
					agency_amount='$agencyamount',
					transit_fees = '$transfees',
					transit_cost = '$transcost',
		...
		redirect('contPaymentold/GetPaymentReceipt/'.$pmtno.'/update');		
		...
		function GetPaymentReceipt(){
			...
			$data['payment'] = $this->ModelPaymentold->getAllPaymentByID($pmtno);
		    $this->load->view('paymentold/update_payment_rcpt',$data);
		}
		...
		function GetPaymentInvTax(){
			...
			$data['successMsg'] = $this->uri->segment(4);
			$data['invoice'] = $this->ModelPaymentold->getAllInvTaxByID($pmtno);
		    $this->load->view('paymentold/update_tax_inv',$data);
		}
	 ...
	 function GetPaymentDetailsEXT(){
		 ...
		 $pmno = $this->uri->segment(3)."/".$this->uri->segment(4)."/".$this->uri->segment(5);		
	     $data['allPaymentFCL'] = $this->ModelPaymentold->getAllFCLpaymentbyPMNO($pmno);
		 $data['PMrow'] = $this->ModelPaymentold->getPaymentByID($pmno);  
		 $this->load->view('paymentold/extpayment_2',$data);
	 ...
	 /********Grid for labor list*******/
			...
			$this->load->view('paymentold/payment_list_EXT', $data);	
	...
	function paymentrcpt(){
	         $data['allRegion'] = $this->ModelPaymentold->getRegion();
			 $this->load->view('paymentold/payment_rcpt',$data);		        
	}
