<?php
	class ContContractor extends Controller {
		...
		$data['agent'] = $this->ModelContractor->getAllAgent($searchdata);
		$this->load->view('contractor/agent_list', $data);
		...
		function registrationFormSubmit(){
			...
			$companyregtype = $_POST['comp_reg_type'];	
			...
			$insertQuery = "INSERT INTO contractors (ctr_clab_no,ctr_gst_no,ctr_reg_type,ctr_socso_no, ctr_datereg, ctr_validdate, ctr_comp_name, ctr_comp_regno, ctr_addr1, ctr_addr2, ctr_addr3, ctr_pcode, ctr_state, ctr_telno, 
							...
							VALUES ('$clabno','$companygstno','$companyregtype','$companysocsono', NOW(), NOW(), " . $this->db->escape($companyname) .",'$companyregno', ". $this->db->escape($address1) .", ". $this->db->escape($address2).", 
			...
			$insertQuery2 = "INSERT INTO rekab_contractors (ctr_clab_no,ctr_gst_no,ctr_socso_no, ctr_datereg, ctr_validdate, ctr_comp_name, ctr_comp_regno, ctr_addr1, ctr_addr2, ctr_addr3, ctr_pcode, ctr_state, ctr_telno, 
							ctr_telno1, ctr_fax, ctr_email, ctr_cidb_regno, ctr_cidbexp_date, ctr_grade, ctr_category, ctr_spec, ctr_periodreg, ctr_mbramount,
							ctr_paymentype, ctr_cshdate, ctr_chqno, ctr_chqdate, ctr_procsby, ctr_procdate, ctr_appstatus, 
							ctr_attach_form24, ctr_attach_form49,ctr_attach_formD, ctr_attach_copycidb, ctr_attach_iccopy, ctr_attach_others, ctr_attach_specify, 
							ctr_dir_name, ctr_dir_mobileno, ctr_contact_name, ctr_contact_desg, ctr_contact_mobileno, ctr_status, ctr_clabexp_date,agent_id) 
		...
		function ctrDashboard(){
			...
			$data['plksexp1mth'] = $this->ModelLabour->getLaborByPermitPassExpiryOld('wkr_permitexp', -1, 'total', 'allcountry');
			$data['plksexp2mth'] = $this->ModelLabour->getLaborByPermitPassExpiryOld('wkr_permitexp', -2, 'total', 'allcountry');
			$data['plksexp3mth'] = $this->ModelLabour->getLaborByPermitPassExpiryOld('wkr_permitexp', -3, 'total', 'allcountry');
		...
		function listbydate(){
			...
			$data['listByCompany'] = $this->ModelContractor->listExpiryByCompanyDate($fieldName, $fromDate, $toDate ,'companylist', '');
		...
		function listByExpiryAndCompanyOld(){
			...
			$data['listByCompany'] = $this->ModelContractor->listExpiryByCompanyOld($fieldName, $noOfMonths, 'companylist', '');
		...
		function ctrPrintHistoryList(){
		    ...
		    $data["crud"] = $grid->output;
		    /**********end grid****************/
			$this->load->view('contractor/ctr_printhistory_lists', $data);	
		}
		...
		function printRegisRenewal(){
			...
			$isAlreadyPrinted = $this->ModelContractor->checkExistingPrintRecord($clabno, $reminderType, $fieldname);
	...
	if($periodreg == "12") $txtAmount = "<u>Two Hundred Only</u>(<u>RM 200.00</u>) for <u>twelve(12)</u> years.";
		...
		function printCert2(){
			...
			$this->load->view('contractor/ctr_print_certificate2', $data);
		}
		...
		function printcard(){
			...
			$this->load->view('contractor/ctr_print_card_wkr', $data);
		}
		...
		function printsalary(){
			...
			$this->load->view('contractor/ctr_print_salary', $data);
		}
		...
		function printArrivalWorkers(){
			...
			$this->load->view('contractor/ctr_print_handover_arrival', $data);
		}
		...
		function printCancelPermit(){
			...
			$data['expiredWorkersRecord'] = $this->ModelContractor->getCancelPermit($clabno);
			$data['ctr'] = $this->ModelContractor->getCtrByClabNo($clabno);
		...
		function printPeringatanLaporan(){
			...
			$this->load->view('contractor/peringatan_laporan', $data);
		}
		function printPeringatanTerakhir(){
			...
			$this->load->view('contractor/peringatan_terakhir', $data);
		}
		...
		function viewExpiryLetterOld(){
			...
			$data['expiredWorkersRecord'] = $this->ModelContractor->listExpiryByCompanyLast($fieldname, $noOfMonths, 'listOfWorkers', $clabNo);
			//echo $expiredWorkersRecord->num_rows();
			$data['ctr'] = $this->ModelContractor->getCtrByClabNo($clabNo);
			$data['fieldname'] = $fieldname;
