<?php
	class ContRenewal extends Controller{
		...
		function Insurance_purchase(){
            ...
            $this->email->cc('lonpac@clab.com.my');
            $this->email->cc('nieza@clab.com.my');
            $this->email->cc('maliah@clab.com.my');
		...
		*/
		...
		$sql = "insert into wo_fcl_renew
		        (fcl_woid,fcl_passno,fcl_passissue,fcl_passexp,fcl_salary,fcl_wt,fcl_next_of_kin,
				fcl_relationship,fcl_year_renewal,fcl_wo_type,fcl_pass,fcl_name,fcl_nationality,fcl_newpassno)
                values
                ('$workorderid','$passno','$passissue','$passexp','$salary','$wt','$nok','$relationship','$yearrenew',
				 '$wotype','$chkpass','$name','$nationality','$newpassno')"; 
        $this->db->trans_begin();
		$this->db->query($sql);	
        $this->db->trans_commit();
	...
	/*	function form_new_labour(){
		...
		function searchWorkorders(){
		    ...
		    if($searchby == "ctr_comp_name" || $searchby == "ctr_clab_no" || $searchby == "fcl_newpassno")
