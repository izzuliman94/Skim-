<?php
	class Contgc extends Controller{
		...
		function spimDashbrd(){
			$data['overall'] = $this->Modelgc->getWorkordersByStatus("overall", "sum");
			$data['closed'] = $this->Modelgc->getWorkordersByStatus("closed", "sum");
			$data['open'] = $this->Modelgc->getWorkordersByStatus("open", "sum");
			$data['received'] = $this->Modelgc->getWorkordersByStatus("received by Operation", "sum");
			$data['processed'] = $this->Modelgc->getWorkordersByStatus("processed by Operation", "sum");
			$data['submithr'] = $this->Modelgc->getWorkordersByStatus("submit to HR", "sum");
			$data['receivehr'] = $this->Modelgc->getWorkordersByStatus("receive by HR", "sum");
			$data['jim'] = $this->Modelgc->getWorkordersByStatus("JIM acknowledgement", "sum");
			
			$data['receiveVisa'] = $this->Modelgc->getWorkordersByStatus("receive calling visa", "sum");
			
			$data['lt14days'] = $this->Modelgc->getWoByJIMAckDate("less14", "sum"); //need to correct the sql script
			$data['mt14days'] = $this->Modelgc->getWoByJIMAckDate("more14", "sum");
			
			$currentuser = $this->session->userdata('username');
			$data['woUnderCurrentUser'] = $this->Modelgc->getWoUnderCurrentUser($currentuser);
			
			$this->load->view('spimgc/spim_dashboard', $data);	
		}	
		function handoverletter(){
		      $wono = $this->uri->segment(3); 
			  $workorderid = $this->uri->segment(4);
			  $data['woRow'] = $this->Modelgc->getWObyWOid($wono);
			  $data['allFCLFiles'] = $this->Modelgc->getSelFCLFiles($workorderid);
			  $this->load->view('spimgc/handover_letter', $data);
		}	
		function notificationletter(){
		      $wono = $this->uri->segment(3); 
			  $workorderid = $this->uri->segment(4);
			  $data['woRow'] = $this->Modelgc->getWObyWOid($wono);
			  $data['allFCLFiles'] = $this->Modelgc->getSelFCLFiles($workorderid);
			  $this->load->view('spimgc/notification_letter', $data);
		}
		
		function summarylist(){
			$data['allRegion'] = $this->Modelgc->getAllregion();
			$data['allContractors'] = $this->ModelLabour->getAllContractors();
			$this->load->view('spimgc/gcsummary',$data);
		}		
		...
		function summaryview(){
			 ...
			 $data['paymenttype']= $this->Modelgc>getAllgctype($type);
			 //$paymenttype= $this->ModelPayment->getpaymenttype($type);
		     $data['summary'] = $this->Modelgc->getsummarygc($datefrom,$dateto,$region,$type,$cancel,$cont);
			 
			 //echo $paymenttype;
		     $this->load->view('spimgc/summary_view',$data);
		...
		if(isset($_POST["chkprgc"])){
		$chkprgc = $_POST["chkprgc"];
		}else{
		$chkprgc = '0';
		}
		
		$dtgcp=date('Y-m-d', strtotime($_POST["dategcp"]));
		if(isset($_POST["chkgcp"])){
		$chkgcp = $_POST["chkgcp"];
		}else{
		$chkgcp = '0';
		}
			
		$dtgcr=date('Y-m-d', strtotime($_POST["dategcr"]));	
		if(isset($_POST["chkgcr"])){
		$chkgcr = $_POST["chkgcr"];
		}else{
		$chkgcr = '0';
		}
		...
		*/
		...
		$add = $_POST["txtaddarea"];
		$passexp = date('Y-m-d', strtotime($_POST["txtpassexp"]));
		$plksexp = date('Y-m-d', strtotime($_POST["txtpermitdate"]));
		$wt = $_POST['txtworktrade'];
		$nok = $_POST['txtnok'];
		$relationship = $_POST['txtrelationship'];
		$wotype = $_POST["selwotype"];
		...
		$sql = "insert into wo_fcl_gc
		        (fcl_woid,fcl_passno,fcl_permitno,fcl_add,fcl_passexp,fcl_plksdate2,fcl_salary,fcl_wt,fcl_next_of_kin,
                ...
                values
                ('$workorderid','$passno','$permitno','$add','$passexp','$plksexp','$salary','$wt','$nok','$relationship','$yearrenew',
				 '$wotype','$chkpass',".$this->db->escape($name).",'$nationality','$newpassno','$dob','$gender','$country')"; 
			
		$wkrQuery = "UPDATE workers SET wkr_gcdate = '$plksexp', wkr_greencard = '1' WHERE wkr_passno = '$passno'";
		$wkrQuery2 = "UPDATE workers SET wkr_gcbiodate = '$plksexp', wkr_biogreencard = '1' WHERE wkr_passno = '$passno'";
		
        $this->db->trans_begin();
		$this->db->query($sql);	
		$this->db->query($wkrQuery);
		$this->db->query($wkrQuery2);		
        $this->db->trans_commit();
	...
	/*	function form_new_labour(){
		...
		function dashboardList(){
			...
			if($type == "overall") {
				...
				$progress = "overall";
				$data['woRecord'] = $this->Modelgc->getWorkordersByStatus($progress, "record");
			}
			else if($type == "closed") {
				$hdr = "Total Workorders (closed)";
				$progress = "receive GreenCard";
				$data['woRecord'] = $this->Modelgc->getWorkordersByStatus($progress, "record");
			}
			else if($type == "open") {
			...
			$this->load->view('spimgc/spim_dashboard_lists', $data);
		}
		...
		function newWorkOrderPt2Submit(){
			...
			if(strlen($currentuserid) > 0){
				...
				/*$woQuery = "INSERT INTO workorders (wo_id, wo_num, wo_date, wo_clab_no, wo_fcl_total, wo_fcl_country, wo_agency, wo_isreplacement, wo_replace_date, wo_replace_reason, 
							...
							*/
				$woQuery = "INSERT INTO workorders(wo_id, wo_num, wo_date, wo_clab_no, wo_fcl_total, wo_fcl_country,wo_gc_consult,wo_gc_type,wo_gc_remark,wo_region,wo_iswithdrawal, wo_personincharge, wo_datesubmit, wo_keyin_by, wo_email,wo_keyin_date,wo_spim_status,wo_sender_name,wo_sender_contact,chk_trn_subang,chk_trn_johor,chk_trn_penang,chk_trn_others,wo_class_date) VALUES ($wo_id, '$workorderno', '$wo_date', '$wo_clab_no',$wo_fcl_total, '$wo_fcl_country','$wo_gc_consult','$wo_gc_type','$wo_gc_remark','$wo_region','$wo_iswithdrawal','$wo_personincharge', '$wo_datesubmit', '$wo_keyin_by','$wo_email', '$wo_keyin_date','X','$wo_sendername','$wo_senderctc','$chk_trn_subang','$chk_trn_johor','$chk_trn_penang','$chk_trn_others','$wo_class_date')";
				
				$docQuery = "INSERT INTO wo_doc (doc_woid, doc_ori_passport, doc_fomema, doc_extplks, doc_details,
				             doc_ori_passport_date)
		...
		function updateWorkOrderPt2Submit(){
			...
			if(strlen($currentuserid) > 0){
				...
				//if(isset($_POST['chkgc'])) $fcl_greencard = '1';
				//else $fcl_greencard = '0';
				...
				$woQuery = "UPDATE workorders SET wo_fcl_total = $wo_fcl_total, wo_fcl_country = '$wo_fcl_country', wo_isreplacement = '$wo_isreplacement',     
				            wo_iswithdrawal = '$wo_iswithdrawal', wo_withd_date = '$wo_withd_date', wo_withd_reason = '$wo_withd_reason', 
							wo_personincharge = '$wo_personincharge', wo_email = '$wo_email', wo_modifiedby = '$modifiedby', wo_modifieddate = NOW(),
							...
							app_ceo_by='$appceoby',app_ceo_date='$appceodate' WHERE wo_id = $wo_id";
