<?php
	class ContAck extends Controller{
		function ContAck(){
			parent::Controller();
			$this->load->library('session');
			$this->load->library('rapyd');	
			$this->load->Model('ModelAck');	
			$this->load->Model('ModelLabour');
			
		}
		
		function newAck(){
			$this->load->view('ack/new_ack_pt1');	
		}
		
		function newAckSearchCtr(){
			$searchby = trim($_POST['searchby']);
			$keyword = trim($_POST['keyword']);
			
			$data['ctrRecord'] = $this->ModelAck->searchAck($searchby, $keyword);
			
			//print_r ($data['ctrRecord']);
			$this->load->view('ack/new_ack_pt1', $data);
		}
		...
		function Insurance_purchase(){
            ...
            $this->email->to('lonpac@clab.com.my , fradila@clab.com.my');
			$this->email->cc('fadil@clab.com.my , hayati@clab.com.my , clabict@clab.com.my');
		...
		function listAllApproval(){
		   $data["ApprovalList"] = $this->ModelAck->getApprovalList();
		   $this->load->view('ack/approvallist',$data);	 
		}
		...
		function openreceipt(){
		   ...
		   $data['receiptlist'] = $this->ModelRenewal->getAllReceipt($clab);
		   $data['form'] = $frm;
	       $this->load->view('renewal/receiptlist',$data);		   
		...
		function handoverletter(){
			  ...
			  $this->load->view('renewal/handover_letter', $data);
		}
		// added by azh
		...
		function regnewlabour(){
			...
			$data['allWOTYPE'] = $this->ModelRenewal->getWOtype();
			//$data['allLabour'] = $this->ModelRenewal->getAllLabor();		
		
		$this->load->view('renewal/reg_new_labour', $data);	
		...
		//$data['allLabour'] = $this->ModelRenewal->getAllLabor();
		$id = $data['id'];
		$data['fcl'] = $this->ModelRenewal->getFCLbyID($id);
		$data['strhide'] = "update";
		
		$this->load->view('renewal/edit_fcl', $data);
		...
		$sql = "delete from wo_fcl_renew where fcl_id = '$id'";
		$this->db->trans_begin();
		$this->db->query($sql);	
		...
		function lampiran_a(){
			...
			$this->load->view('renewal/lampiran_a', $data);
		...
		function lampiran_a_excel(){
			...
			$this->load->view('renewal/lampiran_a_excel', $data);
		...
		function acknowledge(){
		   $wono = $this->uri->segment(3);
		   $data['woRow'] = $this->ModelAck->getWObyWOid($wono);
		   $this->load->view('ack/acknowledgeRegion', $data);
		}
	...
	/*	function form_new_labour(){
		...
		function newAckPt2(){
			...
			if(strlen($currentuserid) > 0){
				...
				$ctrRecord = $this->ModelAck->getCtrByClabNo($clabno);
				$data['ctrRow'] = $ctrRecord->row();
				$data['allCountries'] = $this->ModelAck->getAllCountries();
				$data['allAgencies'] = $this->ModelAck->getAllAgencies();
				
				$this->load->view('ack/new_ack_pt2', $data);
			}
			else{
				$this->load->view('session_expired.php');	
			}
		...
		function newWorkOrderPt2Submit(){
			$currentuserid = $this->session->userdata('emp_id');
			if(strlen($currentuserid) > 0){
				$wo_id = $this->ModelAck->getNextWONumber();
				$wonum_firstpart = "0000" . $wo_id;
				//echo $wonum_firstpart = "0000" . $wo_id;
				//$workorderno = "ACK" . substr($wonum_firstpart, -5) . "/" . date('m') . "/" . date('y');	//get the rightmost four digit
				$workorderno = "ACK" . substr($wonum_firstpart, -5);
				...
				if(isset($_POST['chkothers'])) $wo_others_details = '1';
				else $wo_others_details = '0';
				$wo_doc_others_date = $_POST['dtothers'];
				$doc_others_detail = $_POST['txtothers'];
				if($wo_doc_others_date != "") $wo_doc_others_date = date('Y-m-d', strtotime($wo_doc_others_date));
				else $wo_doc_others_date = '0000-00-00';
				...
				$wo_pay_imidetail = $_POST['txtImipay'];
				if(isset($_POST['chkclab'])) $wo_pay_clb = '1';
				else $wo_pay_clb = '0';
				$wo_pay_clabdetail = $_POST['txtclabpay'];
				if(isset($_POST['chkins_guarante'])) $wo_pay_insg = '1';
				else $wo_pay_insg = '0';
				$wo_pay_fwcsdetail = $_POST['txtfwcspay'];
				if(isset($_POST['chkins_hospital'])) $wo_pay_insh = '1';
				else $wo_pay_insh = '0';
				$wo_pay_fwhsdetail = $_POST['txtfwhspay'];
				if(isset($_POST['chkotherspay'])) $wo_pay_others = '1';
				else $wo_pay_others = '0';
				$wo_pay_othersdetail = $_POST['txtotherspay'];
				...
				/*$woQuery = "INSERT INTO workorders (wo_id, wo_num, wo_date, wo_clab_no, wo_fcl_total, wo_fcl_country, wo_agency, wo_isreplacement, wo_replace_date, wo_replace_reason, 
							...
							*/
				$woQuery = "INSERT INTO acknowledgement(wo_id, wo_num, wo_date, wo_clab_no, wo_fcl_total, wo_fcl_country,wo_ack_type,wo_iswithdrawal, wo_personincharge, wo_datesubmit, wo_keyin_by, wo_keyin_date,wo_spim_status,wo_sender_name,wo_sender_ic,wo_sender_contact,wo_attn_officer) VALUES ($wo_id, '$workorderno', '$wo_date', '$wo_clab_no',$wo_fcl_total, '$wo_fcl_country','$wo_ack_type','$wo_iswithdrawal','$wo_personincharge', '$wo_datesubmit', '$wo_keyin_by', '$wo_keyin_date','A','$wo_sendername','$wo_senderic','$wo_senderctc','$wo_attn_off')";
				
				$docQuery = "INSERT INTO ack_doc (doc_woid, doc_ori_passport, doc_fomema, doc_extplks, doc_details, doc_others_detail,doc_others,doc_others_date,
				             doc_ori_passport_date,doc_fomema_date,doc_extplks_date,doc_details_date,doc_datecomplete)
	            			 VALUES ($wo_id, '$wo_doc_oripass', '$wo_doc_fomema', '$wo_doc_extplks', '$wo_doc_details','$doc_others_detail','$wo_others_details', '$wo_doc_others_date',
							 '$wo_doc_oripass_date', '$wo_doc_fomema_date', '$wo_doc_extplks_date', 
							 '$wo_doc_details_date','$wo_doc_datecomplete')";

				$paymentQuery = "INSERT INTO ack_payment (pay_woid, pay_refno,pay_imigration,pay_imigration_detail,pay_clab,pay_clab_detail,
								 pay_ins_guarante,pay_fwcs_detail,pay_ins_hospital,pay_fwhs_detail,pay_others,pay_others_detail) 
	            			     VALUES ($wo_id, '$wo_pay_refno','$wo_pay_imi','$wo_pay_imidetail','$wo_pay_clb','$wo_pay_clabdetail','$wo_pay_insg','$wo_pay_fwcsdetail','$wo_pay_insh','$wo_pay_fwhsdetail','$wo_pay_others','$wo_pay_othersdetail')";	        
				$statusQuery = "INSERT INTO ack_status (status_woid) values ('$wo_id')";              			     
		...
		/*		$currentuserid = $this->session->userdata('emp_id');
				...
				redirect('contAck/updateWorkOrderPt2/'.$wo_id .'/add');
			}
			else{
				$this->load->view('session_expired.php');	
			}
		...
		function uploadDocView(){
			...
			if(strlen($currentuserid) > 0){
				...
				$this->load->view('renewal/new_workorder_upload', $data);	
			}
			else{
		...
		function uploadDoc(){
			...
			if(strlen($currentuserid) > 0){
				...
				//upload documents
				$config['upload_path'] = './uploads/renewal';
				...
				else
				{
					...
					$upload_id = $this->ModelRenewal->getNextUploadID();
		...
		function updateAcknowledgement(){
			$this->load->view('ack/update_acknowledgement_pt1');
		}
		
		function updateAcknowledgementSearch(){
			$searchby = $_POST['searchby'];
			$keyword = $_POST['txtkeyword'];
			
			//######later get the data based on workorderid#####
			$data['ctrRecord'] = $this->ModelAck->getWOByWoidOrCompname($searchby, $keyword);
			
			$this->load->view('ack/update_acknowledgement_pt1', $data);
		}
		...
		function updateWorkOrderPt2(){
			...
			if(strlen($currentuserid) > 0){
				$workorderid = $this->uri->segment(3);
				$data['successMsg'] = $this->uri->segment(4);
				$data['woRow'] = $this->ModelAck->getWObyWOid($workorderid);
				$data['allCountries'] = $this->ModelAck->getAllCountries();
				$data['allAgencies'] = $this->ModelAck->getAllAgencies();
				$data['allPhTrackLog'] = $this->ModelAck->getAllPhTrackLog($workorderid);
				$data['allFCLFiles'] = $this->ModelAck->getAllFCLFiles($workorderid);
				$data['allFCLUpload'] = $this->ModelAck->getAllFCLUpload($workorderid);
				$data['allLegalFiles'] = $this->ModelAck->getAllLegalList($workorderid);
				
			
				
				
				$this->load->view('ack/update_acknowledgement_pt2', $data);
			}
			else{
				$this->load->view('session_expired.php');	
			}
		...
		function updateAcknowledgementPt2Submit(){
			$currentuserid = $this->session->userdata('emp_id');
			if(strlen($currentuserid) > 0){
				$wo_id = $_POST['txtwoid'];
				...
				//PAYMENT
				$wo_pay_refno = $_POST['payrefno'];
				$wo_pay_imi_detail = $_POST['txtimidetail'];
				$wo_pay_clab_detail = $_POST['txtclabdetail'];
				$wo_pay_fwcs_detail = $_POST['txtfwcsdetail'];
				$wo_pay_fwhs_detail = $_POST['txtfwhsdetail'];
				...
				if(isset($_POST['chkpayothers'])){
					$wo_pay_others = '1';
					$pay_others_detail = $_POST['txtotherdetail'];
				}
				
			    //if(isset($_POST['chkpayothers'])) $wo_pay_others = '1';
				//$pay_others_detail = $_POST['txtotherdetail'];
				//else $wo_pay_others = '0';
				...
				$woQuery = "UPDATE acknowledgement SET wo_fcl_total = $wo_fcl_total, wo_fcl_country = '$wo_fcl_country', wo_isreplacement = '$wo_isreplacement',     
				            wo_iswithdrawal = '$wo_iswithdrawal', wo_withd_date = '$wo_withd_date', wo_withd_reason = '$wo_withd_reason', 
							wo_personincharge = '$wo_personincharge', wo_modifiedby = '$modifiedby', wo_modifieddate = NOW(),
				...
				$docQuery = "update ack_doc set doc_ori_passport = '$wo_doc_oripass', doc_fomema = '$wo_doc_fomema', 
				             doc_extplks='$wo_doc_extplks', doc_details = '$wo_doc_details', doc_others = '$wo_others_details' 			,doc_ori_passport_date = '$wo_doc_oripass_date', doc_fomema_date = '$wo_doc_fomema_date', 
				           doc_extplks_date='$wo_doc_extplks_date', doc_details_date = '$wo_doc_details_date', doc_others_date = '$wo_doc_others_date', doc_others_detail = '$doc_others_detail',
	        ...
	        /*    $paymentQuery = "UPDATE wo_payment SET pay_refno = '$wo_pay_refno', pay_adminfee = '$wo_pay_adminfee', pay_levy = '$wo_pay_levy', pay_plks = '$wo_pay_plks', 
				...
				$paymentQuery = "UPDATE ack_payment set pay_refno = '$wo_pay_refno',pay_imigration = '$wo_pay_imi',pay_imigration_detail = '$wo_pay_imi_detail',pay_clab_detail = '$wo_pay_clab_detail',pay_fwcs_detail = '$wo_pay_fwcs_detail',pay_fwhs_detail = '$wo_pay_fwhs_detail',pay_clab = '$wo_pay_clb', pay_others_detail = '$pay_others_detail' , pay_ins_guarante = '$wo_pay_insg',pay_ins_hospital = '$wo_pay_insh', pay_others = '$wo_pay_others' where pay_woid = '$wo_id'";
	            			  	            			     
				$legalQuery = "UPDATE wo_legal SET legal_agree_receivedate = '$wo_agree_receivedate' WHERE legal_woid = $wo_id";
	            			    
				$statusQuery = "UPDATE ack_status SET wo_latest_progress = '$wo_latest_progress'";
			...
			//	}
				...
				//echo $legalQuery;
				//echo $statusQuery;
				redirect('contAck/updateWorkOrderPt2/'.$wo_id .'/update');
			}
			else{
				$this->load->view('session_expired.php');	
			}
		...
		function newPhoneTrackLog(){
			...
			$this->load->view('ack/update_workorder_phtracklog', $data);	
		}
		...
		function newPhoneTrackLogSubmit(){
			...
			if(strlen($currentuserid) > 0){			
				...
				//phone track log
				$trackid = $this->ModelAck->getNextPhTrackLogID();
				...
				$sqlQuery = "INSERT INTO  ack_phonetrack (track_id, track_wo_id, track_datetime, track_attendby, track_remarks, track_action, track_actionby, track_compdate) 
		...
		function listAllAcknowledgement(){
			/********Grid for labor list*******/
			$css_src = $this->rapyd->get_elements_path("css/sippsstyle.css","css");
			$this->rapyd->load("datafilter","datagrid");
		    $this->rapyd->uri->keep_persistence();
		    ...
		    //grid
		    $grid = new DataGrid("", "acknowledgement");
		    //$grid->db->join("contractors","workorder.wo_clab_no = contractors.ctr_clab_no","LEFT");
		    //$grid->db->order_by("wo_id");
		    $grid->db->join("wo_agency","acknowledgement.wo_agency = wo_agency.agency_id", "LEFT");
		    $grid->db->join("ack_doc","acknowledgement.wo_id = ack_doc.doc_woid", "LEFT");
		    $grid->db->join("wo_legal","acknowledgement.wo_id = wo_legal.legal_woid", "LEFT");
		    $grid->db->join("ack_payment","acknowledgement.wo_id = ack_payment.pay_woid", "LEFT");
		    $grid->db->join("ack_status","acknowledgement.wo_id = ack_status.status_woid", "LEFT");
		    $grid->db->join("contractors","acknowledgement.wo_clab_no = contractors.ctr_clab_no","LEFT");
			$grid->db->join("mst_countries","acknowledgement.wo_fcl_country = mst_countries.cty_id","LEFT");
			$grid->db->join("pmt_type","acknowledgement.wo_ack_type = pmt_type.pmt_type_id","LEFT");
			$grid->db->where("wo_spim_status ='A'");
		    $grid->db->orderby("wo_id", "desc");
			...
			$grid->column("No.","$link_edit", 'width="50"');
			$grid->column("Acknowledgement Number","wo_num");
			$grid->column("COMPANY NAME","ctr_comp_name");
			$grid->column("CLAB NO.","wo_clab_no");
			$grid->column("TOTAL FCL","wo_fcl_total");
			$grid->column("FCL Country", "cty_desc");
			$grid->column("DATE SUBMIT","<callback_displaydate><#wo_datesubmit#></callback_displaydate>");
			$grid->column("P IN CHARGE", "wo_personincharge");
			$grid->column("Acknowledgement Type", "pmt_type_desc");
			...
			$this->load->view('ack/acknowledgement_lists', $data);	
		}
		
		function searchAcknowledgement(){
		    ...
		    $grid->db->join("contractors","Acknowledgement.wo_clab_no = contractors.ctr_clab_no","LEFT");
			$grid->db->join("mst_countries","Acknowledgement.wo_fcl_country = mst_countries.cty_id","LEFT");
			$grid->db->join("mst_replacement_type","Acknowledgement.wo_isreplacement = mst_replacement_type.replacement_type_id","LEFT");
			
			
			if ($searchby == "fcl_newpassno")
			$grid->db->join("wo_fcl_renew","Acknowledgement.wo_id = wo_fcl_renew.fcl_woid ","LEFT");	
			
					
		    if($searchby == "ctr_comp_name" || $searchby == "ctr_clab_no" || $searchby == "fcl_newpassno" || $searchby == "pay_refno" || $searchby == "wo_num")
			...
			$grid->column("P IN CHARGE", "wo_personincharge");
			$grid->column("Acknowledgement Type", "replacement_desc");
  			$grid->build();
