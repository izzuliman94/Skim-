<?php
	class contPayment extends Controller {
		...
		function taxinvlist(){
		   /********Grid for labor list*******/
			...
			$grid->db->join("pmt_type","pmt_tax_inv.payment_type = pmt_type.pmt_type_id", "LEFT");
		    ...
		    $grid->use_function("callback_displayuri");
		 	...
		 	$baseuri = "contPayment";
		 	$link_edit = anchor("$baseuri/GetPaymentReceipt/<#pmt_taxinv_no#>","<#pmt_taxinv_no#>");
			$link_print = anchor_popup("$baseuri/printinvoice/<#pmt_taxinv_no#>","Print");
	...
	function savetaxinvoice(){
		...
		function GetPaymentInvTax(){
		    $pmtno = $this->uri->segment(3);
			$data['successMsg'] = $this->uri->segment(4);
			$data['payment'] = $this->ModelPayment->getAllInvTaxByID($pmtno);
