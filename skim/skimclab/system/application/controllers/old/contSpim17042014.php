	...
	class ContSpim extends Controller{
		...
		function Send_Report(){
           ...
           // $data['allFCLlampiran'] = $this->ModelSpim->getLampiranFCLbyID($wono);	
            ...
            $this->email->to('akmar@clab.com.my , aida@clab.com.my , faizal@clab.com.my , hartini@clab.com.my');
		...
		$sql = "update wo_fcl set
				...
				fcl_wt = '$wt',
				fcl_country = '$country',
				fcl_next_of_kin = ".$this->db->escape($nok).",
				fcl_relationship = '$relationship',
				fcl_plksno = '$plksno'
                where fcl_id = '$id'"; 
				
		$sqllabour = "update workers set
		              wkr_name = ".$this->db->escape($name).",
					  wkr_dob = '$dob',
					  ...
					  wkr_country = '$country',
					  wkr_plksno = '$plksno',
					  wkr_currentemp = '$clabno',
					  wkr_next_of_kin = ".$this->db->escape($nok).",
		...
		$sql_workers = "insert into workers
		               (wkr_id,wkr_passno,wkr_name,wkr_currentemp ,wkr_dob,wkr_homeaddr,wkr_country,wkr_passexp,wkr_status,wkr_wtrade,wkr_plksno,wkr_next_of_kin,wkr_relationship)
					   values
					   ('$workerid','$passno',".$this->db->escape($name).",".$this->db->escape($companyname).",'$dob','$birthplace','$country','$passexp','1','$wt','$plksno',".$this->db->escape($nok).",'$relationship')";
	...
	/*			$statusQuery = "INSERT INTO wo_status (status_woid, wo_isreceive, wo_receiveby, wo_receivedate, wo_receive_comment, wo_isprocess, wo_processby, 
      ...
      $grid->column("Workorder Number","wo_num");
		...
		function printHOVDR(){
			...
			$this->load->view('spim/spim_print_SerahanVDR.php', $data);
		}
